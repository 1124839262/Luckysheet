...
function createFilter() {
  ...
  if (last['row'][0] == last['row'][1] && last['column'][0] == last['column'][1]) {
    ...
    for (let c = 0; c < Store.flowdata[curR].length; c++) {
      const cell = Store.flowdata[curR][c];
...
function createFilterOptions(luckysheet_filter_save, filterObj) {
  ...
  const newSelectedHTML = `<div id="luckysheet-filter-selected-sheet${ Store.currentSheetIndex }" class="luckysheet-cell-selected luckysheet-filter-selected"  style="left:${ col_pre }px;width:${ col - col_pre - 1 }px;top:${ row_pre }px;height:${ row - row_pre - 1 }px;display:block;border-color:#897BFF;z-index:20;background:none;"></div>`;
...
function initialFilterHandler(){
  ...
  $('.luckysheet-cols-menu .luckysheet-cols-submenu').hover(
    function () {
      const $t = $(this), attrid = $t.attr('id'), $attr = $(`#${  attrid  }_sub`), $con = $t.parent();
      const winW = $(window).width(), winH = $(window).height();
      const menuW = $con.width(), attrH = $attr.height() + 25, attrW = $attr.width() + 5;
    ...
    function () {
      const $t = $(this), attrid = $t.attr('id'), $attr = $(`#${  attrid  }_sub`);
  ...
  $('#luckysheet-filter-selected').click(function () {
    const $t = $(this), toffset = $t.offset(), $menu = $('#luckysheet-filter-submenu');
    $menu.hide();

    const winH = $(window).height(), winW = $(window).width();
    const menuW = $menu.width(), menuH = $menu.height();
  ...
  $('#luckysheet-cell-main').on('click', '.luckysheet-filter-options', function (e) {
    ...
    if (byconditiontype == '2') {
      const $input = $('#luckysheet-filter-menu .luckysheet-filter-selected-input2').show().find('input');
    ...
    setTimeout(function () {
      ...
      for (let r = st_r + 1; r <= ed_r; r++) {
        ...
        const cell = Store.flowdata[r][cindex];
  ...
  $('#luckysheet-filter-orderby-color').hover(
    function(){
      ...
      for (let r = st_r + 1; r <= ed_r; r++) {
        ...
        const checksAF = alternateformat.checksAF(r, cindex, af_compute);
        if(checksAF != null){//若单元格有交替颜色
          bg = checksAF[1];
        }

        const checksCF = conditionformat.checksCF(r, cindex, cf_compute);
      ...
      const $t = $('#luckysheet-filter-orderby-color-submenu').end();
      const $con = $(this).parent();
      const winW = $(window).width(), winH = $(window).height();
  ...
  $(document).off('click.orderbyColorConfirm').on('click.orderbyColorConfirm', '#luckysheet-filter-orderby-color-submenu #luckysheet-filter-orderby-color-confirm', function(){
    ...
    $('#luckysheet-filter-orderby-color-submenu .item').each(function(i, e){
      if($(e).find('input[type=\'checkbox\']').is(':checked')){
        const color = $(this).find('label').attr('title');
        const $id = $(this).closest('.box').attr('id');
    ...
    const $top = $(`#luckysheet-filter-options-sheet${  Store.currentSheetIndex  } .luckysheet-filter-options`).eq(cindex - st_c);

    const optionstate = Object.keys(rowhidden).length > 0;

    const rowhiddenall = $.extend(true, rowhiddenother, rowhidden),
  ...
  $(document).off('click.filterCheckbox3').on('click.filterCheckbox3', '#luckysheet-filter-byvalue-select .month',function(){
    ...
    const $yearDay = $(this).parents('.yearBox').find('.day');
  ...
  $(document).off('click.filterCheckbox4').on('click.filterCheckbox4', '#luckysheet-filter-byvalue-select .day',function(){
    ...
    const $monthDay = $(this).parents('.monthBox').find('.day');
  ...
  $('#luckysheet-filter-byvalue-btn-contra').click(function () {
    const $input = $('#luckysheet-filter-byvalue-select .ListBox input[type=\'checkbox\']');
    ...
    //天 对应的 月份
    const $month = $('#luckysheet-filter-byvalue-select .ListBox .monthBox');
    ...
    //天 对应的 年份
    const $year = $('#luckysheet-filter-byvalue-select .ListBox .yearBox');
  ...
  $('#luckysheet-filter-initial').click(function () {
    ...
    $(`#luckysheet-filter-options-sheet${  Store.currentSheetIndex  } .luckysheet-filter-options`).each(function () {
      ...
      const optionstate = $t.hasClass('luckysheet-filter-options-active');
      const rowhidden = json.parseJsonParm($t.data('rowhidden'));
      const caljs = json.parseJsonParm($t.data('caljs'));
    ...
    if(v != ''){
      $('#luckysheet-filter-byvalue-select .ListBox input[type=\'checkbox\']').each(function(i, e){
        if($(e).closest('.day').length > 0){
          const day = $(e).siblings('label').text().toString();
          const month = $(e).closest('.monthBox').find('.month label').text().toString();
          const year = $(e).closest('.yearBox').find('.year label').text().toString();
          ...
          if(itemV.indexOf(v) == -1){
            ...
            //天 对应的 月份
            const $monthDay = $(e).closest('.dayList').find('.day:visible');
            if($monthDay.length == 0){
              $(e).closest('.monthBox').find('.month').hide();
            }

            //天 对应的 年份
            const $yearDay = $(e).closest('.monthList').find('.day:visible');
            if($yearDay.length == 0){
        ...
        if($(e).closest('.textBox').length > 0){
          const itemV = $(e).siblings('label').text().toString();
  ...
  $('#luckysheet-filter-confirm').click(function () {
    ...
    if ($('#luckysheet-filter-bycondition').next().is(':visible') && $('#luckysheet-filter-byvalue').next().is(':hidden') && $('#luckysheet-filter-selected span').data('value') != 'null') {
      const $t = $('#luckysheet-filter-selected span');
      const type = $t.data('type'), value = $t.data('value');
      ...
      if (type == '0') {
        ...
        const $input = $('#luckysheet-filter-menu .luckysheet-filter-selected-input2 input');
        caljs['type'] = '2';
      ...
      for (let r = st_r + 1; r <= ed_r; r++) {
        ...
        if (value == 'cellnull') { //单元格为空
          ...
          const value1 = caljs['value1'], valuelen = value1.length;
          ...
          const value1 = parseFloat(caljs['value1']), value2 = parseFloat(caljs['value2']);
      ...
      $('#luckysheet-filter-byvalue-select .ListBox input[type=\'checkbox\']').each(function(i, e){
        ...
        if($(e).closest('.day').length > 0){
          ...
          const year = $(e).closest('.yearBox').find('.year label').text().replace(locale_filter.filiterYearText, '');
        ...
        if($(e).closest('.textBox').length > 0){
          const itemV = $(e).closest('.textBox').data('filter');
    ...
    const optionstate = $('#luckysheet-filter-byvalue-select .ListBox input[type=\'checkbox\']:visible:checked').length < $('#luckysheet-filter-byvalue-select .ListBox input[type=\'checkbox\']:visible').length || $('#luckysheet-filter-byvalue-input').val().length > 0 || ($('#luckysheet-filter-bycondition').next().is(':visible') && $('#luckysheet-filter-byvalue').next().is(':hidden') && $('#luckysheet-filter-selected span').data('value') != 'null');
