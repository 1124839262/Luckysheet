...
export default function mobileinit(){
  ...
  $(document).on('touchmove', '#luckysheet-grid-window-1', function(event){
    ...
    if(luckysheet_touchmove_status){//滚动
      const slideX = touch.pageX - luckysheet_touchmove_startPos.x;
      const slideY = touch.pageY - luckysheet_touchmove_startPos.y;
      ...
      const mouse = mouseposition(touch.pageX, touch.pageY);
      ...
      const last = $.extend(true, {}, Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1]);
