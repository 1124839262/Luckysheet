import {chatatABC, replaceHtml} from '../utils/util';
import {getSheetIndex} from '../methods/get';
import {keycode, modelHTML} from './constant';
...
const luckysheetSearchReplace = {
  createDialog: function(source) {
    ...
    const $t = $('#luckysheet-search-replace')
  ...
  init: function() {
    ...
    $(document)
      ...
      .on('click.SRsearchAllboxItem', '#luckysheet-search-replace #searchAllbox .boxItem', function() {
        ...
        const r = $(this).attr('data-row');
        const c = $(this).attr('data-col');
        const sheetIndex = $(this).attr('data-sheetIndex');
        ...
        const row = Store.visibledatarow[r],
          row_pre = r - 1 == -1 ? 0 : Store.visibledatarow[r - 1];
        const col = Store.visibledatacolumn[c],
  ...
  searchNext: function() {
    ...
    const searchText = $('#luckysheet-search-replace #searchInput input').val();
    ...
    const searchIndexArr = _this.getSearchIndexArr(searchText, range);
    ...
    const row = Store.visibledatarow[searchIndexArr[count].r],
      row_pre = searchIndexArr[count].r - 1 == -1 ? 0 : Store.visibledatarow[searchIndexArr[count].r - 1];
    const col = Store.visibledatacolumn[searchIndexArr[count].c],
  ...
  searchAll: function() {
    ...
    for (let i = 0; i < searchIndexArr.length; i++) {
      const value_ShowEs = valueShowEs(searchIndexArr[i].r, searchIndexArr[i].c, Store.flowdata).toString();
          ...
          if (cell != null) {
            ...
            if (value != null && value != '') {
              ...
              if (wordCheck) {
                ...
                //正则表达式
                const reg = new RegExp(func_methods.getRegExpStr(searchText), regExpFlags);
  ...
  replace: function() {
    ...
    const replaceText = $('#luckysheet-search-replace #replaceInput input').val();
  ...
  replaceAll: function() {
    ...
    if (wordCheck) {
      for (let i = 0; i < searchIndexArr.length; i++) {
        const r = searchIndexArr[i].r;
        const c = searchIndexArr[i].c;
    ...
    const succeedInfo = replaceHtml(locale_findAndReplace.successTip, {
