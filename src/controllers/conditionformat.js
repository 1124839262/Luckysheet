...
const conditionformat = {
  ...
  init: function(){
    ...
    $(document).off('change.CFchooseSheet').on('change.CFchooseSheet', '#luckysheet-administerRule-dialog .chooseSheet', function(){
      const index = $('#luckysheet-administerRule-dialog .chooseSheet option:selected').val();
    ...
    $(document).off('click.CFadministerRuleConfirm').on('click.CFadministerRuleConfirm', '#luckysheet-administerRule-dialog-confirm', function(){
      ...
      //保存之前的规则
      const fileH = $.extend(true, [], Store.luckysheetfile);
      const historyRules = _this.getHistoryRules(fileH);

      //保存当前的规则
      const fileClone = $.extend(true, [], _this.fileClone);
      for(let c = 0; c < fileClone.length; c++){
        const sheetIndex = fileClone[c]['index'];
      ...
      const fileC = $.extend(true, [], Store.luckysheetfile);
      const currentRules = _this.getCurrentRules(fileC);
      ...
      if(server.allowUpdate){
        const files = $.extend(true, [], Store.luckysheetfile);
    ...
    $(document).off('click.CFadministerRuleFa').on('click.CFadministerRuleFa', '#luckysheet-administerRule-dialog .item .fa-table', function(){
      ...
      const sheetIndex = $('#luckysheet-administerRule-dialog .chooseSheet select option:selected').val();
      if(sheetIndex != Store.currentSheetIndex){
        sheetmanage.changeSheetExec(sheetIndex);
      }

      const txt = $(this).siblings('input').val().trim();
      const dataItem = $(this).parents('.item').attr('data-item');

      _this.multiRangeDialog(dataItem, txt);

      _this.selectRange = [];

      const range = _this.getRangeByTxt(txt);
      if(range.length > 0){
        for(let s = 0; s < range.length; s++){
          const r1 = range[s].row[0], r2 = range[s].row[1];
          const c1 = range[s].column[0], c2 = range[s].column[1];

          const row = Store.visibledatarow[r2],
            row_pre = r1 - 1 == -1 ? 0 : Store.visibledatarow[r1 - 1];
          const col = Store.visibledatacolumn[c2],
    ...
    $(document).off('click.CFmultiRangeConfirm').on('click.CFmultiRangeConfirm', '#luckysheet-multiRange-dialog-confirm', function(){
      ...
      const v = $(this).parents('#luckysheet-multiRange-dialog').find('input').val();
      $(`#luckysheet-administerRule-dialog .item[data-item=${dataItem}] input`).val(v);

      const sheetIndex = $('#luckysheet-administerRule-dialog .chooseSheet option:selected').val();
    ...
    $(document).off('click.CFnewConditionRuleConfirm').on('click.CFnewConditionRuleConfirm', '#luckysheet-newConditionRule-dialog-confirm', function(){
      ...
      const index = $('#luckysheet-newConditionRule-dialog .ruleTypeItem.on').index();
      const type1 = $('#luckysheet-newConditionRule-dialog #type1 option:selected').val();
      const type2 = $(`#luckysheet-newConditionRule-dialog .${  type1  }Box #type2 option:selected`).val();

      let format, rule;
      if(index == 0){
        if(type1 == 'dataBar'){ //数据条
          const color = $(this).parents('#luckysheet-newConditionRule-dialog').find('.dataBarBox .luckysheet-conditionformat-config-color').spectrum('get').toHexString();
          ...
          const maxcolor = $(this).parents('#luckysheet-newConditionRule-dialog').find('.colorGradationBox .maxVal .luckysheet-conditionformat-config-color').spectrum('get').toRgbString();
          const midcolor = $(this).parents('#luckysheet-newConditionRule-dialog').find('.colorGradationBox .midVal .luckysheet-conditionformat-config-color').spectrum('get').toRgbString();
          const mincolor = $(this).parents('#luckysheet-newConditionRule-dialog').find('.colorGradationBox .minVal .luckysheet-conditionformat-config-color').spectrum('get').toRgbString();
          ...
          const len = $(this).parents('#luckysheet-newConditionRule-dialog').find('.iconsBox .model').attr('data-len');
          const leftMin = $(this).parents('#luckysheet-newConditionRule-dialog').find('.iconsBox .model').attr('data-leftmin');
          const top = $(this).parents('#luckysheet-newConditionRule-dialog').find('.iconsBox .model').attr('data-top');
        ...
        if(index == 1){
          if(type1 == 'number'){ //单元格值
            ...
            if(type2 == 'betweenness'){
              ...
              //条件值是否是选区
              const rangeArr1 = _this.getRangeByTxt(v1);
              if(rangeArr1.length > 1){
                _this.infoDialog(conditionformat_Text.onlySingleCell, '');
                return;
              }
              else if(rangeArr1.length == 1){
                const r1 = rangeArr1[0].row[0], r2 = rangeArr1[0].row[1];
                const c1 = rangeArr1[0].column[0], c2 = rangeArr1[0].column[1];
              ...
              const rangeArr2 = _this.getRangeByTxt(v2);
              if(rangeArr2.length > 1){
                _this.infoDialog(conditionformat_Text.onlySingleCell, '');
                return;
              }
              else if(rangeArr2.length == 1){
                const r1 = rangeArr2[0].row[0], r2 = rangeArr2[0].row[1];
                const c1 = rangeArr2[0].column[0], c2 = rangeArr2[0].column[1];
              ...
              //条件值是否是选区
              const rangeArr = _this.getRangeByTxt(v);
              if(rangeArr.length > 1){
                _this.infoDialog(conditionformat_Text.onlySingleCell, '');
                return;
              }
              else if(rangeArr.length == 1){
                const r1 = rangeArr[0].row[0], r2 = rangeArr[0].row[1];
                const c1 = rangeArr[0].column[0], c2 = rangeArr[0].column[1];
            ...
            //条件值
            const v = $('#luckysheet-newConditionRule-dialog #daterange-btn').val();
          ...
          //条件值
          const v = $('#luckysheet-newConditionRule-dialog #conditionVal input').val().trim();
          ...
          //条件值
          const v = $('#luckysheet-newConditionRule-dialog #formulaConditionVal input').val().trim();
      ...
      if(source == 0){
        ...
        //保存当前的规则
        const ruleArr = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] == undefined ? [] : Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'];
        ruleArr.push(rule);
        ...
        //临时存储新规则
        const ruleArr = _this.fileClone[getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] ? _this.fileClone[getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] : [];
        ruleArr.push(rule);
    ...
    $(document).off('click.CFeditorConditionRule').on('click.CFeditorConditionRule', '#editorConditionRule', function(){
      ...
      const itemIndex = $('#luckysheet-administerRule-dialog .ruleList .listBox .item.on').attr('data-item');
      const rule = {
    ...
    $(document).off('click.CFeditorConditionRuleConfirm').on('click.CFeditorConditionRuleConfirm', '#luckysheet-editorConditionRule-dialog-confirm',function(){
      const index = $('#luckysheet-editorConditionRule-dialog .ruleTypeItem.on').index();
      const type1 = $('#luckysheet-editorConditionRule-dialog #type1 option:selected').val();
      const type2 = $(`#luckysheet-editorConditionRule-dialog .${  type1  }Box #type2 option:selected`).val();

      const cellrange = _this.editorRule['data'].cellrange;

      let format, rule;
      if(index == 0){
        if(type1 == 'dataBar'){ //数据条
          const color = $(this).parents('#luckysheet-editorConditionRule-dialog').find('.dataBarBox .luckysheet-conditionformat-config-color').spectrum('get').toHexString();
          ...
          const maxcolor = $(this).parents('#luckysheet-editorConditionRule-dialog').find('.colorGradationBox .maxVal .luckysheet-conditionformat-config-color').spectrum('get').toRgbString();
          const midcolor = $(this).parents('#luckysheet-editorConditionRule-dialog').find('.colorGradationBox .midVal .luckysheet-conditionformat-config-color').spectrum('get').toRgbString();
          const mincolor = $(this).parents('#luckysheet-editorConditionRule-dialog').find('.colorGradationBox .minVal .luckysheet-conditionformat-config-color').spectrum('get').toRgbString();
          ...
          const len = $(this).parents('#luckysheet-editorConditionRule-dialog').find('.iconsBox .model').attr('data-len');
          const leftMin = $(this).parents('#luckysheet-editorConditionRule-dialog').find('.iconsBox .model').attr('data-leftmin');
          const top = $(this).parents('#luckysheet-editorConditionRule-dialog').find('.iconsBox .model').attr('data-top');
        ...
        if(index == 1){
          if(type1 == 'number'){ //单元格值
            ...
            //条件值
            const v = $('#luckysheet-editorConditionRule-dialog #daterange-btn').val();
          ...
          //条件值
          const v = $('#luckysheet-editorConditionRule-dialog #conditionVal input').val().trim();
          ...
          //条件值
          const v = $('#luckysheet-editorConditionRule-dialog #formulaConditionVal input').val().trim();
          console.log(v);
          if(v == ''){
            _this.infoDialog('Condition value cannot be empty!', '');
            return;
      ...
      //修改编辑的规则
      const sheetIndex = _this.editorRule['sheetIndex'];
      const itemIndex = _this.editorRule['itemIndex'];
    ...
    $(document).off('change.CFnewEditorRuleType2').on('change.CFnewEditorRuleType2', '.luckysheet-newEditorRule-dialog #type2', function(){
      const type1 = $(this).parents('.luckysheet-newEditorRule-dialog').find('#type1 option:selected').val();

      if(type1 == 'colorGradation'){
        const type2 = $(this).find('option:selected').val();
    ...
    $(document).off('click.CFiconsLi').on('click.CFiconsLi', '.luckysheet-newEditorRule-dialog .iconsBox li', function(){
      const len = $(this).find('div').attr('data-len');
      const leftmin = $(this).find('div').attr('data-leftmin');
      const top = $(this).find('div').attr('data-top');
      const title = $(this).find('div').attr('title');
      const position = $(this).find('div').css('background-position');
    ...
    $(document).off('click.CFdefault').on('click.CFdefault', '#luckysheet-conditionformat-dialog-confirm', function(){
      ...
      //条件名称
      const conditionName = $('#luckysheet-conditionformat-dialog .box').attr('data-itemvalue');
      ...
      if(conditionName == 'greaterThan' || conditionName == 'lessThan' || conditionName == 'equal' || conditionName == 'textContains'){
        ...
        const v = $('#luckysheet-conditionformat-dialog #daterange-btn').val();
        ...
        const v = $('#luckysheet-conditionformat-dialog #conditionVal').val().trim();
    ...
    $(document).off('click.CFicons').on('click.CFicons', '#luckysheet-CFicons-dialog .item', function(){
      ...
      if(Store.luckysheet_select_save.length > 0){
        const cellrange = $.extend(true, [], Store.luckysheet_select_save);
        const format = {
    ...
    $(document).on('click', '.range .fa-table', function(){
      const id = $(this).parents('.luckysheet-modal-dialog').attr('id');
      $(`#${  id}`).hide();
      //入口
      let source;
      ...
      if(id == 'luckysheet-conditionformat-dialog'){
        ...
        const $id = $(this).parents('.range').attr('id');
    ...
    $(document).on('click', '#luckysheet-singleRange-dialog-confirm', function(){
      ...
      const source = $(this).attr('data-source');
      const v = $(this).parents('#luckysheet-singleRange-dialog').find('input').val();
    ...
    $(document).on('click', '.luckysheet-modal-dialog-title-close', function(){
      ...
      if(id == 'luckysheet-newConditionRule-dialog'){
        const source = $(`#${  id}`).find('#luckysheet-newConditionRule-dialog-close').attr('data-source');
        //新建规则入口
        if(source == 1){
          $('#luckysheet-administerRule-dialog').show();
        }
      ...
      if(id == 'luckysheet-singleRange-dialog'){
        ...
        const source = $(this).parents('#luckysheet-singleRange-dialog').find('#luckysheet-singleRange-dialog-confirm').attr('data-source');
        if(source == '0_1' || source == '0_2'){
          $('#luckysheet-conditionformat-dialog').show();
        }
  ...
  singleRangeDialog: function(source, value){
    ...
    const $t = $('#luckysheet-singleRange-dialog')
    ...
    const scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
  ...
  multiRangeDialog: function(dataItem, value){
    ...
    const $t = $('#luckysheet-multiRange-dialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 300)
        .end(),
      myh = $t.outerHeight(),
  ...
  conditionformatDialog: function(title, content){
    ...
    const $t = $('#luckysheet-conditionformat-dialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 300)
        .end(),
      myh = $t.outerHeight(),
  ...
  CFiconsDialog: function(){
    ...
    const $t = $('#luckysheet-CFicons-dialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 400)
        .end(),
      myh = $t.outerHeight(),
  ...
  administerRuleDialog: function(){
    ...
    const content =  `<div class="chooseSheet">
    ...
    const $t = $('#luckysheet-administerRule-dialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 400)
        .end(),
      myh = $t.outerHeight(),
  ...
  getConditionRuleList: function(index){
    ...
    const ruleArr = _this.fileClone[getSheetIndex(index)].luckysheet_conditionformat_save; //条件格式规则集合
    if(ruleArr != null && ruleArr.length > 0){
      const conditionformat_Text = locale().conditionformat;

      for(let i = 0; i < ruleArr.length; i++){
        const type = ruleArr[i]['type'];            //规则类型
        const format = ruleArr[i]['format'];        //规则样式
        const cellrange = ruleArr[i]['cellrange'];  //规则应用范围
        ...
        for(let s = 0; s < cellrange.length; s++){
          const r1 = cellrange[s].row[0], r2 = cellrange[s].row[1];
          const c1 = cellrange[s].column[0], c2 = cellrange[s].column[1];

          rangeTxtArr.push(`${chatatABC(c1) + (r1 + 1)  }:${  chatatABC(c2)  }${r2 + 1}`);
        }

        //条件格式规则列表dom
        const itemHtml =  `<div class="item" data-item="${  i  }">` +
      ...
      $('#luckysheet-administerRule-dialog .ruleList .listBox .item canvas').each(function(i){
        const x = $(this).closest('.item').attr('data-item');

        const type = ruleArr[x]['type'];
        const format = ruleArr[x]['format'];

        const can = $(this).get(0).getContext('2d');
        if(type == 'dataBar'){
          if(format.length == 2){
            const my_gradient = can.createLinearGradient(0, 0, 46, 0);
          ...
          const l = format['leftMin'];
          const t = format['top'];

          const w1 = 32 * len + 10 * (len - 1);
          const h1 = 32;
          const w2 = 46;
          const h2 = 46 * 32 / w1;
  ...
  newConditionRuleDialog: function(source){
    ...
    //规则说明
    const ruleExplainHtml = _this.getRuleExplain(0);
    ...
    const $t = $('#luckysheet-newConditionRule-dialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 400)
        .end(),
      myh = $t.outerHeight(),
  ...
  editorConditionRuleDialog: function(){
    ...
    const rule = _this.editorRule.data;
    ...
    //规则说明
    const ruleExplainHtml = _this.getRuleExplain(index);
    ...
    const $t = $('#luckysheet-editorConditionRule-dialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 400)
        .end(),
      myh = $t.outerHeight(),
    ...
    if(ruleType == 'dataBar' || ruleType == 'colorGradation' || ruleType == 'icons'){
      if(type1 == 'dataBar'){
        ...
        const len = ruleFormat['len'];
        const l = ruleFormat['leftMin'];
        const t = ruleFormat['top'];
      ...
      if(type1 == 'number'){
        ...
        const val1 = rule.conditionValue[0];
  ...
  infoDialog: function(title, content){
    ...
    const $t = $('#luckysheet-conditionformat-info-dialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 300)
        .end(),
      myh = $t.outerHeight(),
  ...
  getRuleExplain: function(index){
    ...
    const textCellColorHtml = this.textCellColorHtml();
  ...
  CFSplitRange: function(range1, range2, range3, type){
    ...
    const r1 = range1['row'][0], r2 = range1['row'][1];
    const c1 = range1['column'][0], c2 = range1['column'][1];
  ...
  getcolorGradation: function(color1, color2, value1, value2, value){
    ...
    const r1 = parseInt(rgb1[0].split('(')[1]);
    const g1 = parseInt(rgb1[1]);
    const b1 = parseInt(rgb1[2].split(')')[0]);

    const rgb2 = color2.split(',');
    const r2 = parseInt(rgb2[0].split('(')[1]);
    const g2 = parseInt(rgb2[1]);
    const b2 = parseInt(rgb2[2].split(')')[0]);

    const r = Math.round(r1 - (r1 - r2) / (value1 - value2) * (value1 - value));
    const g = Math.round(g1 - (g1 - g2) / (value1 - value2) * (value1 - value));
    const b = Math.round(b1 - (b1 - b2) / (value1 - value2) * (value1 - value));
  ...
  getCFPartRange: function(sheetIndex, range1, range2){
    ...
    const cf = Store.luckysheetfile[getSheetIndex(sheetIndex)].luckysheet_conditionformat_save;
    if(cf != null && cf.length > 0){
      label:  for(let i = 0; i < cf.length; i++){
        const cellrange = cf[i].cellrange;

        for(let j = 0; j < cellrange.length; j++){
          const r1 = cellrange[j].row[0], r2 = cellrange[j].row[1];
          const c1 = cellrange[j].column[0], c2 = cellrange[j].column[1];
  ...
  getComputeMap: function(sheetIndex){
    ...
    const ruleArr = Store.luckysheetfile[index]['luckysheet_conditionformat_save'];
    const data = Store.luckysheetfile[index]['data'];

    if(data == null){
      return null;
    }

    const computeMap = this.compute(ruleArr, data);
  ...
  compute: function(ruleArr, d){
    ...
    if(ruleArr.length > 0){
      for(let i = 0; i < ruleArr.length; i++){
        const type = ruleArr[i]['type'];
        const cellrange = ruleArr[i]['cellrange'];
        const format = ruleArr[i]['format'];
        ...
        if(type == 'dataBar'){ //数据条
                  ...
                  for(let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c++){
                    ...
                    if(getObjType(cell) == 'object' && cell['ct'] != null && cell['ct'].t == 'n' && cell.v != null){
                      if(parseInt(cell.v) < 0){ //负数
                        const valueLen = Math.round(Math.abs(parseInt(cell.v)) / Math.abs(min) * 100) / 100;
                      ...
                      if(parseInt(cell.v) > 0){ //正数
                        const valueLen = Math.round(parseInt(cell.v) / max * 100) / 100;
          ...
          const len = parseInt(format['len']);
          const leftMin = parseInt(format['leftMin']);
          const top = parseInt(format['top']);
          ...
          for(let s = 0; s < cellrange.length; s++){
            ...
            if(conditionName == 'greaterThan' || conditionName == 'lessThan' || conditionName == 'equal' || conditionName == 'textContains'){
              ...
              const re = new RegExp(conditionValue0); // 外部传递过来的正则表达式
              ...
              for(let r = str; r <= edr; r++){
                for(let c = stc; c <= edc; c++){
                  ...
                  const funcV = formula.execfunction(func);
                  let v = funcV[1];
