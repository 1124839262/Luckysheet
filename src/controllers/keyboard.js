...
const luckysheet_shiftkeydown = false;
  ...
  if ($('#luckysheet-formula-search-c').is(':visible') && (dir=='up' || dir=='down') ) {
    ...
    if($('#luckysheet-formula-functionrange-select').is(':visible')){
      ...
      if(anchor.parent().next().text() == null || anchor.parent().next().text() == ''){
        const vText = $('#luckysheet-input-box #luckysheet-input-box-index').text();
...
export function keyboardInitial(){
      ...
      if(anchor.parent().is('#luckysheet-helpbox-cell') || anchor.is('#luckysheet-helpbox-cell')){
        if(kcode == keycode.ENTER){
          ...
          if(formula.iscelldata(helpboxValue)){
            ...
            const row = Store.visibledatarow[cellrange['row'][1]],
              row_pre = cellrange['row'][0] - 1 == -1 ? 0 : Store.visibledatarow[cellrange['row'][0] - 1];
            const col = Store.visibledatacolumn[cellrange['column'][1]],
  ...
  $('#luckysheet_info_detail_input').val(server.title).css('width', getByteLen(server.title) * 10).keydown(function(){
    ...
    const $t = $(this);
    const inputlen = getByteLen($t.val())*10;
    const updatelen = $('#luckysheet_info_detail_update').outerWidth();
    const savelen = $('#luckysheet_info_detail_save').outerWidth();
    const userlen = $('#luckysheet_info_detail_user').parent().outerWidth()+60;
    const containerlen = $(`#${  Store.container}`).outerWidth();
    const otherlen = 100;

    const minuslen = containerlen- savelen - updatelen - userlen - otherlen;
