...
const imageCtrl = {
  ...
  modelHtml: function(id, imgItem) {
    ...
    const imageUrlHandle = Store.toJsonOptions && Store.toJsonOptions['imageUrlHandle'];
    const src = typeof imageUrlHandle === 'function' ? imageUrlHandle(imgItem.src) : imgItem.src;
    const imgItemParam = _this.getImgItemParam(imgItem);

    const width = imgItemParam.width;
    const height = imgItemParam.height;
    const left = imgItemParam.left;
    const top = imgItemParam.top;
    const position = imgItemParam.position;

    const borderWidth = imgItem.border.width;
  ...
  getSliderHtml: function() {
    const imageText = locale().imageText;
  ...
  colorSelectDialog: function(currenColor){
    ...
    const $t = $('#luckysheet-imageCtrl-colorSelect-dialog')
  ...
  init: function() {
    ...
    $('#luckysheet-modal-dialog-slider-imageCtrl').off('click.color').on('click.color', '#imgItemBorderColor', function() {
      const currenColor = $(this).find('span').attr('title');
    ...
    $(document).off('click.selectColorConfirm').on('click.selectColorConfirm', '#luckysheet-imageCtrl-colorSelect-dialog-confirm', function(){
      const $parent = $(this).parents('#luckysheet-imageCtrl-colorSelect-dialog');
      $('#luckysheet-modal-dialog-mask').hide();
      $parent.hide();

      const currenColor = $parent.find('.currenColor span').attr('title');
    ...
    $('#luckysheet-image-showBoxs').off('mousedown.active').on('mousedown.active', '.luckysheet-modal-dialog-image', function(e) {
      ...
      const item = _this.images[id];
      const imgItemParam = _this.getImgItemParam(item);
      ...
      const imageUrlHandle = Store.toJsonOptions && Store.toJsonOptions['imageUrlHandle'];
      const imgUrl = typeof imageUrlHandle === 'function' ? imageUrlHandle(item.src) : item.src;
    ...
    $('#luckysheet-modal-dialog-activeImage').off('mousedown.move').on('mousedown.move', '.luckysheet-modal-dialog-content', function(e) {
      ...
      const offset = $('#luckysheet-modal-dialog-activeImage').offset();
    ...
    $('#luckysheet-modal-dialog-activeImage').off('mousedown.resize').on('mousedown.resize', '.luckysheet-modal-dialog-resize-item', function(e) {
      ...
      const mouse = mouseposition(e.pageX, e.pageY);
      const x = mouse[0] + scrollLeft;
      const y = mouse[1] + scrollTop;

      const position = $('#luckysheet-modal-dialog-activeImage').position();
      const width = $('#luckysheet-modal-dialog-activeImage').width();
      const height = $('#luckysheet-modal-dialog-activeImage').height();
  ...
  addImgItem: function(img) {
    ...
    const imgItem = $.extend(true, {}, _this.imgItem);
    imgItem.src = img.src;
    ...
    const id = _this.generateRandomId();
  ...
  moveImgItem: function() {
    ...
    const obj = $('#luckysheet-modal-dialog-activeImage')[0];
    const item = _this.images[_this.currentImgId];
  ...
  resizeImgItem: function() {
    ...
    const item = _this.images[_this.currentImgId];
    const scaleX = obj.clientWidth / item.crop.width;
    const scaleY = obj.clientHeight / item.crop.height;
  ...
  croppingEnter: function() {
    ...
    const imageUrlHandle = Store.toJsonOptions && Store.toJsonOptions['imageUrlHandle'];
    const imgSrc = typeof imageUrlHandle === 'function' ? imageUrlHandle(item.src) : item.src;
  ...
  copyImgItem: function(e) {
    ...
    const cpdata = '<table data-type="luckysheet_copy_action_image"><tr><td><td></tr></table>';

    if (!clipboardData) {
      const textarea = $('#luckysheet-copy-content');
  ...
  pasteImgItem: function() {
    ...
    const rowIndex = Store.luckysheet_select_save[0].row_focus || 0;
    const colIndex = Store.luckysheet_select_save[0].column_focus || 0;
    const left = colIndex == 0 ? 0 : Store.visibledatacolumn[colIndex - 1];
    const top = rowIndex == 0 ? 0 : Store.visibledatarow[rowIndex - 1];

    const img = $.extend(true, {}, _this.copyImgItemObj);
    ...
    const id = _this.generateRandomId();
    const modelHtml = _this.modelHtml(id, img);
  ...
  allImagesShow: function() {
    ...
    for(const imgId in _this.images){
      const imgItem = _this.images[imgId];
      const modelHtml = _this.modelHtml(imgId, imgItem);
      $('#luckysheet-image-showBoxs .img-list').append(modelHtml);
    }
  },
  moveChangeSize: function(rc, index, size) {
    const _this = this;
    const images = $.extend(true, {}, _this.images);

    if(rc == 'row'){
      const row = Store.visibledatarow[index],
        row_pre = index - 1 == -1 ? 0 : Store.visibledatarow[index - 1];
      const changeSize = size - (row - row_pre - 1);
          ...
          if(imgItemParam.top >= row){
            ...
            if(imgItemParam.top + imgItemParam.height >= row-2){
              if(imgItemParam.top < row + changeSize){
                const scaleY = (imgItemParam.height + changeSize) / imgItemParam.height;
                ...
                const scaleY = (imgItemParam.top + imgItemParam.height - row) / imgItemParam.height;
                imgItem.default.height = Math.round(imgItem.default.height * scaleY);
                imgItem.crop.height = Math.round(imgItem.crop.height * scaleY);
                imgItem.crop.offsetTop = Math.round(imgItem.crop.offsetTop * scaleY);
              ...
              if(imgItemParam.top > row + changeSize){
                const scaleY = 1 / imgItemParam.height;
                imgItem.default.height = Math.round(imgItem.default.height * scaleY);
                imgItem.crop.height = Math.round(imgItem.crop.height * scaleY);
                imgItem.crop.offsetTop = Math.round(imgItem.crop.offsetTop * scaleY);
                imgItem.default.top = row + changeSize - imgItem.crop.offsetTop;
                ...
                const scaleY = (row + changeSize - imgItemParam.top) / imgItemParam.height;
                imgItem.default.height = Math.round(imgItem.default.height * scaleY);
                imgItem.crop.height = Math.round(imgItem.crop.height * scaleY);
                imgItem.crop.offsetTop = Math.round(imgItem.crop.offsetTop * scaleY);
              }
      ...
      const col = Store.visibledatacolumn[index],
        col_pre = index - 1 == -1 ? 0 : Store.visibledatacolumn[index - 1];
      const changeSize = size - (col - col_pre - 1);
          ...
          if(imgItemParam.left >= col){
            ...
            if(imgItemParam.left + imgItemParam.width >= col-2){
              if(imgItemParam.left < col + changeSize){
                const scaleX = (imgItemParam.width + changeSize) / imgItemParam.width;
                ...
                const scaleX = (imgItemParam.left + imgItemParam.width - col) / imgItemParam.width;
                imgItem.default.width = Math.round(imgItem.default.width * scaleX);
                imgItem.crop.width = Math.round(imgItem.crop.width * scaleX);
                imgItem.crop.offsetLeft = Math.round(imgItem.crop.offsetLeft * scaleX);
              ...
              if(imgItemParam.left > col + changeSize){
                const scaleX = 1 / imgItemParam.width;
                imgItem.default.width = Math.round(imgItem.default.width * scaleX);
                imgItem.crop.width = Math.round(imgItem.crop.width * scaleX);
                imgItem.crop.offsetLeft = Math.round(imgItem.crop.offsetLeft * scaleX);
                imgItem.default.left = col + changeSize - imgItem.crop.offsetLeft;
                ...
                const scaleX = (col + changeSize - imgItemParam.left) / imgItemParam.width;
                imgItem.default.width = Math.round(imgItem.default.width * scaleX);
                imgItem.crop.width = Math.round(imgItem.crop.width * scaleX);
                imgItem.crop.offsetLeft = Math.round(imgItem.crop.offsetLeft * scaleX);
              }
