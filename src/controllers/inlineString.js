...
import {selectTextContent, selectTextContentCollapse, selectTextContentCross} from '../global/cursorPos';
...
export function isInlineStringCell(cell){
  const isIs = cell && cell.ct!=null && cell.ct.t=='inlineStr' && cell.ct.s!=null && cell.ct.s.length>0;
  return isIs;
}
...
export function updateInlineStringFormat(cell, attr, value, $input){
  ...
  const endContainer = range.endContainer, startContainer = range.startContainer;
  ...
  if($textEditor.length>0){
    if(startContainer===endContainer){
      ...
      let left='' , mid='' , right='';
      const s1=0, s2=startOffset, s3 = endOffset, s4=content.length;
      ...
      if(startContainer.parentNode.tagName=='SPAN' && endContainer.parentNode.tagName=='SPAN'){
        ...
        const startContent = startSpan.innerHTML, endContent = endSpan.innerHTML;
        let sleft='' , sright='', eleft='' , eright='';
        const s1=0, s2=startOffset, s3 = endOffset, s4=endContent.length;
        ...
        for(let i=0;i<startSpanIndex;i++){
          const span = spans.get(i), content = span.innerHTML;
        ...
        if(startSpanIndex<endSpanIndex){
          for(let i=startSpanIndex+1;i<endSpanIndex;i++){
            const span = spans.get(i), content = span.innerHTML;
            const cssText = getCssText(span.style.cssText, attr, value);
...
export function enterKeyControll(cell){
  ...
  if($textEditor.length>0){
    ...
    if(startContainer.parentNode.tagName=='SPAN'){
      ...
      const cssText = startSpan.style.cssText;
