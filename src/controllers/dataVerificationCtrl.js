...
const dataVerificationCtrl = {
  ...
  createDialog: function(){
    ...
    const $t = $('#luckysheet-dataVerification-dialog').find('.luckysheet-modal-dialog-content').css('min-width', 350).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
  ...
  init: function(){
    ...
    $(document).off('click.dvSaveConfirm').on('click.dvSaveConfirm', '#luckysheet-dataVerification-dialog-confirm', function(e) {
      const rangeTxt = $('#luckysheet-dataVerification-dialog #data-verification-range input').val().trim();
      const range = _this.getRangeByTxt(rangeTxt);
      ...
      const type = $('#luckysheet-dataVerification-dialog #data-verification-type-select').val();
      ...
      const remote = $('#luckysheet-dataVerification-dialog #data-verification-remote').is(':checked');
      const prohibitInput = $('#luckysheet-dataVerification-dialog #data-verification-prohibitInput').is(':checked');
      const hintShow = $('#luckysheet-dataVerification-dialog #data-verification-hint-show').is(':checked');
      let hintText = '';
      ...
      const historyDataVerification = $.extend(true, {}, _this.dataVerification);
      const currentDataVerification = $.extend(true, {}, _this.dataVerification);
  ...
  dataAllocation: function(){
    ...
    //focus单元格
    const rowIndex = range.row_focus || range.row[0];
    const colIndex = range.column_focus || range.column[0];
    const dataVerification = $.extend(true, {}, _this.dataVerification);
  ...
  rangeDialog: function(dataSource, txt){
    ...
    const $t = $('#luckysheet-dataVerificationRange-dialog')
  ...
  cellFocus: function(r, c, clickMode){
    ...
    const item = _this.dataVerification[`${r  }_${  c}`];
    ...
    if(item.type == 'dropdown'){
      ...
      if($('#luckysheet-dataVerification-dropdown-List').is(':visible')){
        const dataIndex = $('#luckysheet-dataVerification-dropdown-List').prop('data-index');
    ...
    //数据验证未通过
    const cellValue = getcellvalue(r, c, null);

    if(isRealNull(cellValue)){
      return;
    }

    const validate = _this.validateCellData(cellValue, item);
  ...
  validateCellData: function(cellValue, item){
    ...
    if(type == 'dropdown'){
      const list = _this.getDropdownList(value1);
  ...
  dropdownListShow: function(){
    ...
    const item = _this.dataVerification[`${rowIndex  }_${  colIndex}`];
    const list = _this.getDropdownList(item.value1);

    let optionHtml = '';
    if (item.type === 'dropdown' && item.type2) {
      // 下拉多选的情况下 将已经选择的标出来
      const cellValue = getcellvalue(rowIndex, colIndex, null);
      const valueArr = isRealNull(cellValue) ? [] : cellValue.split(',');
      list.forEach(i => {
        const checked = valueArr.indexOf(i) !== -1;
    ...
    const myh = $('#luckysheet-dataVerification-dropdown-List').outerHeight();
    const currentWinH = $('#luckysheet-cell-main')[0].scrollHeight;
  ...
  getDropdownList: function(txt){
    ...
    if(formula.iscelldata(txt)){
      const range = formula.getcellrange(txt);
      const d = Store.luckysheetfile[getSheetIndex(range.sheetIndex)].data;
