...
const luckysheetFreezen = {
  ...
  cutVolumn: function (arr, cutindex) {
    ...
    const pre = arr.slice(0, cutindex);
    const premax = pre[pre.length - 1];
  ...
  cancelFreezenVertical: function (sheetIndex) {
    ...
    const isvertical = $('#luckysheet-freezebar-vertical').is(':visible');
    ...
    const currentSheet = Store.luckysheetfile[getSheetIndex(sheetIndex)];
  ...
  createFreezenVertical: function (freezenverticaldata, left) {
    ...
    if (_this.initialVertical) {
      ...
      const gridheight = $('#luckysheet-grid-window-1').height();
  ...
  changeFreezenIndex: function (originindex, type) {
    ...
    if (type == 'v' && _this.freezenverticaldata != null) {
      const freezen_colindex = _this.freezenverticaldata[1];
      const offset = luckysheet_searcharray(Store.visibledatacolumn, $('#luckysheet-cell-main').scrollLeft());
      ...
      const freezen_rowindex = _this.freezenhorizontaldata[1];
      const offset = luckysheet_searcharray(Store.visibledatarow, $('#luckysheet-cell-main').scrollTop());
  ...
  scrollFreezen: function () {
    ...
    if (_this.freezenverticaldata != null) {
      ...
      const offset = luckysheet_searcharray(_this.freezenverticaldata[3], $('#luckysheet-cell-main').scrollLeft());

      const top = _this.freezenverticaldata[4];
      ...
      const column_px = Store.visibledatacolumn[column], freezen_px = Store.visibledatacolumn[freezen_colindex];
    ...
    if (_this.freezenhorizontaldata != null) {
      ...
      const offset = luckysheet_searcharray(_this.freezenhorizontaldata[3], $('#luckysheet-cell-main').scrollTop());

      const left = _this.freezenhorizontaldata[4];
      ...
      const row_px = Store.visibledatarow[row], freezen_px = Store.visibledatarow[freezen_rowindex];
  ...
  cancelFreezenHorizontal: function (sheetIndex) {
    ...
    const ishorizontal = $('#luckysheet-freezebar-horizontal').is(':visible');
  ...
  createFreezenHorizontal: function (freezenhorizontaldata, top) {
    ...
    if (_this.initialHorizontal) {
      ...
      const gridwidth = $('#luckysheet-grid-window-1').width();
  ...
  createCanvas: function(id, width, height, left, top){
    const c = $('<canvas/>').appendTo('#luckysheet-grid-window-1').attr({
      'id': id,
  ...
  scrollAdaptOfselect: function(){
    ...
    if (_this.freezenhorizontaldata != null && _this.freezenverticaldata != null) {
      const freezenTop = _this.freezenhorizontaldata[0];
      const freezen_rowindex = _this.freezenhorizontaldata[1];
      const offTop = scrollTop - _this.freezenhorizontaldata[2];

      const freezenLeft = _this.freezenverticaldata[0];
      const freezen_colindex = _this.freezenverticaldata[1];
      const offLeft = scrollLeft - _this.freezenverticaldata[2];

      for(let s = 0; s < Store.luckysheet_select_save.length; s++){
        const obj = $.extend(true, {}, Store.luckysheet_select_save[s]);
        ...
        if(s == Store.luckysheet_select_save.length - 1){
          const rf = obj.row_focus == null ? r1 : obj.row_focus;
          const cf = obj.column_focus == null ? c1 : obj.column_focus;
          ...
          const top = row_pre_f;
          const height = row_f - row_pre_f - 1;

          const left = col_pre_f;
          const width = col_f - col_pre_f - 1;
      ...
      for(let s = 0; s < Store.luckysheet_select_save.length; s++){
        ...
        if(s == Store.luckysheet_select_save.length - 1){
          const rf = obj.row_focus == null ? r1 : obj.row_focus;
          const cf = obj.column_focus == null ? obj.column[0] : obj.column_focus;
      ...
      for(let s = 0; s < Store.luckysheet_select_save.length; s++){
        ...
        if(s == Store.luckysheet_select_save.length - 1){
          const rf = obj.row_focus == null ? obj.row[0] : obj.row_focus;
          const cf = obj.column_focus == null ? c1 : obj.column_focus;
  ...
  scrollAdaptOfImage: function () {
    ...
    const images = imageCtrl.images;

    const scrollTop = $('#luckysheet-cell-main').scrollTop();
    const scrollLeft = $('#luckysheet-cell-main').scrollLeft();

    const freezenTop =  _this.freezenhorizontaldata != null ? (_this.freezenhorizontaldata[0] - _this.freezenhorizontaldata[2]) : -1;
    const freezenLeft = _this.freezenverticaldata != null ? (_this.freezenverticaldata[0] - _this.freezenverticaldata[2]) : -1;
    ...
    $.each(images, function (i) {
      ...
      const dialogImage = $(`#${  i}`);
      const x = dialogImage.position();

      const width = dialogImage.width();
      const height = dialogImage.height();
      const defaultTop = image.default.top * zoomRatio;
      const defaultLeft = image.default.left * zoomRatio;
  ...
  scrollAdaptOfchart: function(){
    ...
    if(_this.freezenhorizontaldata != null && _this.freezenverticaldata != null){
      const freezenTop = _this.freezenhorizontaldata[0] - _this.freezenhorizontaldata[2];
      const freezenLeft = _this.freezenverticaldata[0] - _this.freezenverticaldata[2];
      ...
      $('#luckysheet-cell-main .luckysheet-data-visualization-chart').each(function(i, e){
        ...
        const $canvas_width = $(e).find('canvas').width();
        const $canvas_height = $(e).find('canvas').height();

        const height_diff = $canvas_height - height;
        const width_diff = $canvas_width - width;

        if((x.top - height_diff) < freezenTop){
          const size = freezenTop - (x.top - height_diff);
        ...
        if((x.left - width_diff) < freezenLeft){
          const size = freezenLeft - (x.left - width_diff);
  ...
  scrollAdaptOfpostil: function(){
    ...
    if(_this.freezenhorizontaldata != null && _this.freezenverticaldata != null){
      ...
      $('#luckysheet-postil-showBoxs .luckysheet-postil-show').each(function(i, e){
        ...
        const postil_left = postil['left'] == null ? toX + 18 : postil['left'];
        let postil_top = postil['top'] == null ? toY - 18 : postil['top'];
        const postil_width = postil['width'] == null ? luckysheetPostil.defaultWidth : postil['width'];
        const postil_height = postil['height'] == null ? luckysheetPostil.defaultHeight : postil['height'];

        if(postil_top < 0){
          postil_top = 2;
        }

        const size = luckysheetPostil.getArrowCanvasSize(postil_left, postil_top, toX, toY);
        let show = true;
        let show2 = true;
  ...
  scrollAdaptOfdpicon: function(){
    ...
    const copy_r = luckysheetDropCell.copyRange['row'][1],
      copy_c = luckysheetDropCell.copyRange['column'][1];

    const apply_r = luckysheetDropCell.applyRange['row'][1],
    ...
    if(_this.freezenhorizontaldata != null && _this.freezenverticaldata != null){
      const freezen_rowindex = _this.freezenhorizontaldata[1];
      const offsetRow = luckysheet_searcharray(_this.freezenhorizontaldata[3], $('#luckysheet-cell-main').scrollTop() - _this.freezenhorizontaldata[2]);
      const freezen_colindex = _this.freezenverticaldata[1];
      const offsetColumn = luckysheet_searcharray(_this.freezenverticaldata[3], $('#luckysheet-cell-main').scrollLeft() - _this.freezenverticaldata[2]);
      ...
      if(row_index >= freezen_rowindex && col_index >= freezen_colindex){
        ...
        if(row_index < (freezen_rowindex + offsetRow - 1)){
          ...
          const col = colLocationByIndex(col_index + offsetColumn)[1];
        ...
        if(col_index < (freezen_colindex + offsetColumn - 1)){
          ...
          const row = rowLocationByIndex(row_index + offsetRow)[1];
      ...
      const row = rowLocationByIndex(row_index)[1],
  ...
  scrollAdaptOffilteroptions: function(){
    ...
    if(_this.freezenhorizontaldata != null && _this.freezenverticaldata != null){
      const freezen_rowindex = _this.freezenhorizontaldata[1];
      const freezen_top = _this.freezenhorizontaldata[0] + $('#luckysheet-cell-main').scrollTop();

      const freezen_colindex = _this.freezenverticaldata[1];
      const offsetColumn = luckysheet_searcharray(_this.freezenverticaldata[3], $('#luckysheet-cell-main').scrollLeft() - _this.freezenverticaldata[2]);

      $(`#luckysheet-filter-options-sheet${ Store.currentSheetIndex } .luckysheet-filter-options`).each(function(i, e){
        const row_index = $(e).data('str');
        const top = row_index - 1 == -1 ? 0 : Store.visibledatarow[row_index - 1];

        const col_index = $(e).data('cindex');
        ...
        if(row_index >= freezen_rowindex && col_index >= freezen_colindex){
          ...
          if(top < freezen_top){
            ...
            const left = Store.visibledatacolumn[col_index + offsetColumn] - 20;
  ...
  frozenTofreezen: function() {
    ...
    if(frozen.type === 'row'){
      ...
      const freezenhorizontaldata = [Store.visibledatarow[row_st], row_st + 1, scrollTop, luckysheetFreezen.cutVolumn(Store.visibledatarow, row_st + 1), top];
      ...
      const freezenverticaldata = [Store.visibledatacolumn[col_st], col_st + 1, scrollLeft, luckysheetFreezen.cutVolumn(Store.visibledatacolumn, col_st + 1), left];
      ...
      const row_focus = frozen.range['row_focus'];
      ...
      const column_focus = frozen.range['column_focus'];
