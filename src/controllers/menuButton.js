...
import {checkTheStatusOfTheSelectedCells, frozenFirstColumn, frozenFirstRow} from '../global/api';
...
const menuButton = {
  ...
  initialMenuButton: function() {
    ...
    $('#luckysheet-icon-paintformat').click(function(e) {
      ...
      e.stopPropagation();
      const _locale = locale();
      const locale_paint = _locale.paint;
      ...
      const r1 = Store.luckysheet_select_save[0].row[0],
        r2 = Store.luckysheet_select_save[0].row[1];

      const c1 = Store.luckysheet_select_save[0].column[0],
    ...
    $('#luckysheet-icon-currency').click(function() {
      const d = editor.deepCopyFlowData(Store.flowdata); //取数据
    ...
    $('#luckysheet-icon-fmt-other').click(function() {
      ...
      const menuButtonId = `${$(this).attr('id')  }-menuButton`;
      let $menuButton = $(`#${  menuButtonId}`);

      if ($menuButton.length == 0) {
        const itemdata = locale_defaultFmt;

        const itemset = _this.createButtonMenu(itemdata);

        // luckysheet-menuButton-sub
        const menu = replaceHtml(_this.menu, { id: 'fmt-other', item: itemset, subclass: '', sub: '' });
        ...
        const subitemset = _this.createButtonMenu(subitemdata);
        const submenu = replaceHtml(_this.menu, {
          id: 'fmtOtherSelf',
          item: subitemset,
        ...
        $('#luckysheet-icon-fmtOtherSelf-menuButton')
          ...
          .click(function() {
            ...
            const itemvalue = $(this).attr('itemvalue');
      ...
      const userlen = $(this).outerWidth();
      const tlen = $menuButton.outerWidth();
    ...
    $('#luckysheet-icon-font-family')
      ...
      .click(function() {
        ...
        if ($menuButton.length == 0) {
          ...
          const itemset = _this.createButtonMenu(_this.fontSelectList);

          const menu = replaceHtml(_this.menu, { id: 'font-family', item: itemset, subclass: '', sub: '' });
    ...
    $('#luckysheet-icon-text-color-menu')
      ...
      .click(function() {
        ...
        if ($menuButton.length == 0) {
          ...
          const itemset = _this.createButtonMenu(itemdata);
          const subid = 'text-color-self';
          const coloritem = replaceHtml(_this.coloritem, {
            class: 'luckysheet-icon-alternateformat',
            name: `${locale_toolbar.alternatingColors  }...`,
          });
          const menu = replaceHtml(_this.color, {
            id: menuButtonId,
            coloritem: coloritem,
            colorself: subid,
            sub: '',
          ...
          $menuButton.find('.luckysheet-color-reset').click(function() {
            ...
            const $input = $(`#${  menuButtonId}`).find('.luckysheet-color-selected');
            $input.val('#000000');
          ...
          $menuButton.find('.luckysheet-icon-alternateformat').click(function() {
            ...
            const range = $.extend(true, {}, Store.luckysheet_select_save[0]);

            const isExists = alternateformat.rangeIsExists(range)[0];
            if (!isExists) {
        ...
        const offsetTop = $(this).offset().top + 26;
        setTimeout(function() {
          const $input = $(`#${  menuButtonId}`).find('.luckysheet-color-selected');
    ...
    $('#luckysheet-icon-font-size')
      ...
      .click(function() {
        ...
        if ($menuButton.length == 0) {
          ...
          const menu = replaceHtml(_this.menu, { id: 'font-size', item: itemset, subclass: '', sub: '' });
      ...
      .keyup(function(e) {
        ...
        const itemvalue = parseInt($this.val());
        const $menuButton = $('#luckysheet-icon-font-size-menuButton');
        _this.focus($menuButton, itemvalue);
    ...
    $('#luckysheet-icon-border-all').click(function() {
      ...
      const cfg = $.extend(true, {}, Store.config);
    ...
    $('#luckysheet-icon-border-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'border-menu', item: itemset, subclass: '', sub: '' });
        ...
        const submenuid = 'luckysheet-icon-borderSize-menuButton';
        const subcolormenuid = 'luckysheet-icon-borderColor-menuButton';
        const colormenu = replaceHtml(_this.color, {
          id: subcolormenuid,
          coloritem: '',
          colorself: '',
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          const $icon = $('#luckysheet-icon-border-all').find('.luckysheet-icon-img-container');
        ...
        $(`#${  subcolormenuid}`)
          ...
          .click(function() {
            const $input = $(`#${  subcolormenuid}`).find('.luckysheet-color-selected');
            $input.val('#000');
            $('#luckysheet-icon-cell-color').attr('color', null);
    ...
    $('#luckysheet-icon-merge-button').click(function() {
      ...
      if (Store.config['merge'] != null) {
        ...
        for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
          const r1 = Store.luckysheet_select_save[s].row[0],
            r2 = Store.luckysheet_select_save[s].row[1];
          const c1 = Store.luckysheet_select_save[s].column[0],
    ...
    $('#luckysheet-icon-merge-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'merge-menu', item: itemset, subclass: '', sub: '' });
    ...
    $('#luckysheet-icon-align-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'align-menu', item: itemset, subclass: '', sub: '' });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          const $icon = $('#luckysheet-icon-align')
            .attr('type', itemvalue)
    ...
    $('#luckysheet-icon-valign-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'valign-menu', item: itemset, subclass: '', sub: '' });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          const $icon = $('#luckysheet-icon-valign')
            .attr('type', itemvalue)
            .find('.luckysheet-icon-img-container');
    ...
    $('#luckysheet-icon-textwrap-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'textwrap-menu', item: itemset, subclass: '', sub: '' });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          const $icon = $('#luckysheet-icon-textwrap')
            .attr('type', itemvalue)
            .find('.luckysheet-icon-img-container');
    ...
    $('#luckysheet-icon-rotation-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'rotation-menu', item: itemset, subclass: '', sub: '' });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          const $icon = $('#luckysheet-icon-rotation')
            .attr('type', itemvalue)
            .find('.luckysheet-icon-img-container');
    ...
    $('#luckysheet-icon-freezen-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'freezen-menu', item: itemset, subclass: '', sub: '' });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          if (itemvalue == 'freezenRow') {
            ...
            if (luckysheetFreezen.freezenRealFirstRowColumn) {
              const row_st = 0;
              const top = Store.visibledatarow[row_st] - 2 + Store.columnHeaderHeight;
              ...
              const col_st = 0;
              const left = Store.visibledatacolumn[col_st] - 2 + Store.rowHeaderWidth;
              ...
              const top = Store.visibledatarow[row_st] - 2 - scrollTop + Store.columnHeaderHeight;
              const freezenhorizontaldata = [
                Store.visibledatarow[row_st],
                row_st + 1,
                scrollTop,
              ...
              const left = Store.visibledatacolumn[col_st] - 2 - scrollLeft + Store.rowHeaderWidth;
              const freezenverticaldata = [
                Store.visibledatacolumn[col_st],
                col_st + 1,
                scrollLeft,
            ...
            // 固定超出屏幕范围
            const rangeTop = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1].top;
            if (
            ...
            const row_focus = last['row_focus'] == null ? last['row'][0] : last['row_focus'];
            ...
            // 固定超出屏幕范围
            const rangeLeft = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1].left;
            if (
              luckysheetFreezen.freezenRealFirstRowColumn &&
            ...
            const last = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1];
            const column_focus = last['column_focus'] == null ? last['column'][0] : last['column_focus'];
    ...
    $('#luckysheet-icon-autofilter').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'autofilter', item: itemset, subclass: '', sub: '' });
    ...
    $('#luckysheet-icon-seachmore').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'seachmore', item: itemset, subclass: '', sub: '' });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          if (itemvalue == 'search' || itemvalue == 'replace') {
            ...
            const last = Store.luckysheet_select_save[0];
    ...
    $('#luckysheet-icon-function-menu').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'function-menu', item: itemset, subclass: '', sub: '' });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          if (itemvalue == 'if') {
            const last = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1];
            const r = last['row_focus'] == null ? last['row'][0] : last['row_focus'];
            const c = last['column_focus'] == null ? last['column'][0] : last['column_focus'];

            if (!!Store.flowdata[r] && !!Store.flowdata[r][c] && !!Store.flowdata[r][c]['f']) {
              const fp = Store.flowdata[r][c]['f'].toString();
            ...
            const cell = Store.flowdata[row_index][col_index];
            if (cell != null && cell.f != null) {
              //单元格有计算
              const functionStr = luckysheetformula.getfunctionParam(cell.f);
    ...
    $('#luckysheet-icon-bold')
      ...
      .click(function(e) {
        ...
        const flag = checkTheStatusOfTheSelectedCells('bl', 1);
    ...
    $('#luckysheet-icon-italic')
      ...
      .click(function() {
        ...
        const flag = checkTheStatusOfTheSelectedCells('it', 1);
        const foucsStatus = flag ? 0 : 1;
    ...
    $('#luckysheet-icon-strikethrough')
      ...
      .click(function() {
        const d = editor.deepCopyFlowData(Store.flowdata);
        const flag = checkTheStatusOfTheSelectedCells('cl', 1);
        const foucsStatus = flag ? 0 : 1;
    ...
    $('#luckysheet-icon-underline')
      ...
      .click(function() {
        const d = editor.deepCopyFlowData(Store.flowdata);
        const flag = checkTheStatusOfTheSelectedCells('un', 1);
        const foucsStatus = flag ? 0 : 1;
    ...
    $('#luckysheet-icon-conditionformat').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const itemset = _this.createButtonMenu(itemdata);
        const menu = replaceHtml(_this.menu, { id: 'conditionformat', item: itemset, subclass: '', sub: '' });
        ...
        const subitemset2 = _this.createButtonMenu(subitemdata2);
        const submenu2 = replaceHtml(_this.menu, {
        ...
        //数据条子菜单
        const submenu3 = `<div id="luckysheet-icon-dataBar-menuButton" class="luckysheet-cols-menu luckysheet-rightgclick-menu luckysheet-menuButton luckysheet-menuButton-sub luckysheet-mousedown-cancel" style="width: 126px;padding: 5px;top: 118.5px;left: 1321.48px;display: none;">
        ...
        //色阶
        const submenu4 = `<div id="luckysheet-icon-colorGradation-menuButton" class="luckysheet-cols-menu luckysheet-rightgclick-menu luckysheet-menuButton luckysheet-menuButton-sub luckysheet-mousedown-cancel" style="width: 126px;padding: 5px;top: 143.5px;left: 1321.48px;display: none;">
                                    <div itemvalue="0" class="luckysheet-cols-menuitem luckysheet-mousedown-cancel" style="width: 28px; height: 26px;padding: 5px;float: left;">
        ...
        //清除规则子菜单
        const subitemdata6 = [{ text: conditionformat_text.deleteSheetRule, value: 'delSheet', example: '' }];
        const subitemset6 = _this.createButtonMenu(subitemdata6);
        const submenu6 = replaceHtml(_this.menu, {
          id: 'deleteRule',
          item: subitemset6,
          subclass: 'luckysheet-menuButton-sub',
        });
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          if (itemvalue == 'icons') {
            ...
            const loadSheetUrl = server.loadSheetUrl;
            ...
            if (loadSheetUrl != '' && loadSheetUrl != null) {
              ...
              $.post(loadSheetUrl, { gridKey: server.gridKey, index: sheetindex.join(',') }, function(d) {
                const dataset = new Function(`return ${  d}`)();
                ...
                for (const item in dataset) {
                  ...
                  const otherfile = file[getSheetIndex(item)];
          ...
          .on(
            ...
            function() {
              ...
              if (Store.luckysheet_select_save.length == 0) {
                ...
                const textCellColorHtml = conditionformat.textCellColorHtml();
        ...
        $(document)
          ...
          .on('click.CFdataBar', '#luckysheet-icon-dataBar-menuButton .luckysheet-cols-menuitem', function() {
            ...
            if (Store.luckysheet_select_save.length > 0) {
              ...
              const format = conditionformat.dataBarList[itemvalue]['format'];
        ...
        $(document)
          ...
          .on(
            ...
            function() {
              ...
              if (Store.luckysheet_select_save.length > 0) {
                const cellrange = $.extend(true, [], Store.luckysheet_select_save);
                const format = conditionformat.colorGradationList[itemvalue]['format'];
    ...
    $('#luckysheet-icon-postil').click(function() {
      ...
      const itemset = _this.createButtonMenu(itemdata);
      const menu = replaceHtml(_this.menu, { id: 'postil', item: itemset, subclass: '', sub: '' });
    ...
    $('#luckysheet-icon-print').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        const menu = replaceHtml(_this.menu, { id: 'print', item: itemset, subclass: '', sub: '' });
  ...
  updateFormat: function(d, attr, foucsStatus) {
    ...
    if (attr in inlineStyleAffectAttribute) {
      if (parseInt($('#luckysheet-input-box').css('top')) > 0) {
        const value = $('#luckysheet-input-box').text();
        if (value.substr(0, 1) != '=') {
          const cell = d[Store.luckysheetCellUpdate[0]][Store.luckysheetCellUpdate[1]];
    ...
    for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
      const row_st = Store.luckysheet_select_save[s]['row'][0],
        row_ed = Store.luckysheet_select_save[s]['row'][1];
      const col_st = Store.luckysheet_select_save[s]['column'][0],
  ...
  updateFormat_mc: function(d, foucsStatus) {
    ...
    if (foucsStatus == 'mergeCancel') {
      for (let i = 0; i < Store.luckysheet_select_save.length; i++) {
        const range = Store.luckysheet_select_save[i];
          ...
          for (let c = c1; c <= c2; c++) {
            ...
            if (cell != null && cell.mc != null) {
              ...
              if ('rs' in cell.mc) {
                ...
                const cell_clone = JSON.parse(JSON.stringify(fv[`${mc_r  }_${  mc_c}`]));
  ...
  changeMenuButtonDom: function(attr, foucsStatus, _locale) {
    ...
    if (attr == 'bl') {
      ...
      const menuButtonId = 'luckysheet-icon-font-family-menuButton';
      ...
      const $menuButton = $('#luckysheet-icon-font-size-menuButton');
      const itemvalue = foucsStatus,
        $input = $('#luckysheet-icon-font-size input');
      _this.focus($menuButton, itemvalue);
      $('#luckysheet-icon-font-size').attr('itemvalue', itemvalue);
      $input.val(itemvalue);
    } else if (attr == 'ht') {
      const $menuButton = $('#luckysheet-icon-align-menu-menuButton');
      ...
      const $menuButton = $('#luckysheet-icon-valign-menu-menuButton');
      ...
      const $menuButton = $('#luckysheet-icon-textwrap-menu-menuButton');
      ...
      const $menuButton = $('#luckysheet-icon-rotation-menu-menuButton');
      ...
      const $menuButton = $('#luckysheet-icon-fmt-other');
      const _locale = locale();
      const locale_defaultFmt = _locale.defaultFmt;
      if (!foucsStatus) {
  ...
  inputMenuButtonFocus: function(focusTarget) {
    ...
    if (startContainer.parentNode.tagName == 'SPAN') {
      const cssText = startContainer.parentNode.style.cssText;
      const stylelist = convertCssToStyleList(cssText);
  ...
  menuButtonFocus: function(d, r, c) {
    const _this = this;
    const foucsList = ['bl', 'it', 'cl', 'ff', 'ht', 'vt', 'fs', 'tb', 'tr', 'ct', 'un'];
    const _locale = locale();
    for (let i = 0; i < foucsList.length; i++) {
      const attr = foucsList[i];
      const foucsStatus = _this.checkstatus(d, r, c, attr);
  ...
  getRangeInMerge: function(st_r, rlen, st_c, clen, sheetIndex) {
    ...
    if (cfg != null && cfg['merge'] != null) {
      for (const key in cfg['merge']) {
        const mc = cfg['merge'][key];
        if (
  ...
  mergeMoveMain: function(columnseleted, rowseleted, s, top, height, left, width) {
    ...
    while (offloop) {
      ...
      for (let i = 0; i < mcset.length; i++) {
        ...
        const changeparam = _this.mergeMove(mc, columnseleted, rowseleted, s, top, height, left, width);
  ...
  mergeMove: function(mc, columnseleted, rowseleted, s, top, height, left, width) {
    ...
    if (
      ...
      const margeset = _this.mergeborer(Store.flowdata, mc.r, mc.c);
      if (margeset) {
  ...
  getCellRealSize: function(d, cell_r, cell_c) {
    ...
    const celldata = d[cell_r][cell_c];

    if (!!celldata && celldata['mc'] != null) {
      const mc = celldata['mc'];
      const margeset = _this.mergeborer(d, mc.r, mc.c);

      if (margeset) {
        const row = margeset.row[1];
        const row_pre = margeset.row[0];
        const row_index = margeset.row[2];
        const row_index_ed = margeset.row[3];

        const col = margeset.column[1];
        const col_pre = margeset.column[0];
        const col_index = margeset.column[2];
        const col_index_ed = margeset.column[3];

        width = col - col_pre - 1;
        height = row - row_pre - 1;
      }
    } else {
      const config = getluckysheetfile()[getSheetIndex(Store.currentSheetIndex)]['config'];
  ...
  getTextSize: function(text, font) {
    ...
    const o = $('#luckysheetTextSizeTest')
        .text(text)
        .css({ font: f }),
      w = o.innerWidth(),
      h = o.innerHeight();
  ...
  activeFormulaInput: function(row_index, col_index, rowh, columnh, formula, isnull) {
    ...
    if (isnull) {
      ...
      const $span = $('#luckysheet-rich-text-editor').find('span');
      luckysheetformula.setCaretPosition($span.get($span.length - 2), 0, 1);

      return;
    }

    const row_pre = rowLocationByIndex(rowh[0])[0],
  ...
  backFormulaInput: function(d, r, c, rowh, columnh, formula) {
    ...
    const v = luckysheetformula.execfunction(f, r, c);
  ...
  singleFormulaInput: function(d, _index, fix, st_m, ed_m, formula, type, noNum, noNull) {
    ...
    if (isNull && noNull) {
      const st_r_r = _this.getNoNullValue(d, _index, fix, type);
      ...
      if (st_r_r == null) {
        ...
        if (_index == st_m) {
          for (let c = st_m; c <= ed_m; c++) {
            const st_r_r = _this.getNoNullValue(d, c, fix, type);
  ...
  autoSelectionFormula: function(formula) {
    ...
    let isfalse = true;
    const select_save_len=Store.luckysheet_select_save.length;
    luckysheetformula.execFunctionExist = [];

    const execFormulaInput_c = function(d, st_r, ed_r, st_c, ed_c, formula) {
      const st_c_c = _this.getNoNullValue(d, st_r, ed_c, 'c');
    ...
    const execFormulaInput = function(d, st_r, ed_r, st_c, ed_c, formula) {
      const st_r_c = _this.getNoNullValue(d, st_c, ed_r, 'r');
    ...
    for(let s = 0; s < select_save_len; s++){
      const st_r = Store.luckysheet_select_save[s].row[0],
        ed_r = Store.luckysheet_select_save[s].row[1];
      const st_c = Store.luckysheet_select_save[s].column[0],
        ed_c = Store.luckysheet_select_save[s].column[1];
      const row_index = Store.luckysheet_select_save[s].row_focus,
  ...
  getStyleByCell: function(d, r, c) {
    ...
    for (const key in cell) {
      const value = _this.checkstatus(d, r, c, key);
  ...
  addFontTolist: function(fontName) {
    ...
    for (let a = 0; a < this.fontSelectList.length; a++) {
      const fItem = this.fontSelectList[a];
    ...
    if (isNone) {
      ...
      const $menuButton = $('#luckysheet-icon-font-family-menuButton');
      const itemset = this.createButtonMenu(this.fontSelectList);
  ...
  fontInitial: function(fontList) {
    ...
    const locale_fontarray = locale().fontarray;
    for (let a = 0; a < locale_fontarray.length; a++) {
      const fItem = locale_fontarray[a];
      const ret = {};
      ret.value = fItem;
      ret.index = a;
      ret.type = 'inner';
