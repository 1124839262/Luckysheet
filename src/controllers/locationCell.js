...
const luckysheetLocationCell = {
  createDialog: function(){
    ...
    const content = '<div class="listbox">'+
                        '<div class="listItem">'+
    ...
    const $t = $('#luckysheet-locationCell-dialog').find('.luckysheet-modal-dialog-content').css('min-width', 400).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
  ...
  init: function(){
    ...
    $(document).off('click.locationCellConfirm').on('click.locationCellConfirm', '#luckysheet-locationCell-dialog #luckysheet-locationCell-dialog-confirm', function(){
      ...
      const $radio = $('#luckysheet-locationCell-dialog .listItem input:radio:checked');
      const id = $radio.attr('id');

      if(id == 'locationConstant' || id == 'locationFormula'){
        const $checkbox = $radio.siblings('.subbox').find('input:checkbox:checked');
  ...
  apply: function(range, type, value){
    ...
    if(type == 'locationFormula' || type == 'locationConstant' || type == 'locationNull'){ //公式 常量 空值
      ...
      for(let s = 0; s < range.length; s++){
        ...
        const st_r = range[s].row[0], ed_r = range[s].row[1];
        const st_c = range[s].column[0], ed_c = range[s].column[1];
