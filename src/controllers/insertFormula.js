...
const insertFormula = {
  init: function(){
    ...
    $(document).off('change.fxFormulaTS').on('change.fxFormulaTS', '#formulaTypeSelect', function(){
      const type = $('#formulaTypeSelect option:selected').val();
    ...
    $(document).off('click.fxFormulaCf').on('click.fxFormulaCf', '#luckysheet-search-formula-confirm', function(){
      const formula = $('#luckysheet-search-formula .listBox.on').attr('name');
    ...
    $(document).off('focus.fxParamInput').on('focus.fxParamInput', '#luckysheet-search-formula-parm .parmBox input', function(){
      const parmIndex = $(this).parents('.parmBox').index();
      formula.data_parm_index = parmIndex;

      const formulatxt = $(this).parents('#luckysheet-search-formula-parm').find('.luckysheet-modal-dialog-title-text').text();
      const parmLen = Store.luckysheet_function[formulatxt].p.length;
      ...
      const parmName = $(this).parents('.parmBox').find('.name').text();
      $(`<span>${ parmName }:</span><span>${ parmDetail }</span>`).appendTo($('#luckysheet-search-formula-parm .parmDetailsBox'));

      //公式参数可自增（参数自增最多5个）
      if(parmRepeat == 'y'){
        const parmCount = $('#luckysheet-search-formula-parm .parmBox').length;
    ...
    $(document).off('click.fxParamI').on('click.fxParamI', '#luckysheet-search-formula-parm .parmBox i', function(){
      ...
      const $t = $('#luckysheet-search-formula-parm-select').find('.luckysheet-modal-dialog-content').css('min-width', 300).end(),
        myh = $t.outerHeight(),
        myw = $t.outerWidth();
    ...
    $(document).off('click.fxParamSelectCf').on('click.fxParamSelectCf', '#luckysheet-search-formula-parm-select-confirm', function(){
      const parmIndex = $('#luckysheet-search-formula-parm-select-input').attr('data_parm_index');
  ...
  formulaListDialog: function(){
    ...
    const $t = $('#luckysheet-search-formula').find('.luckysheet-modal-dialog-content').css('min-width', 300).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
    const winw = $(window).width(), winh = $(window).height();
    const scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
  ...
  formulaParmDialog: function(formulaTxt, parm){ //参数弹出框
    ...
    const $t = $('#luckysheet-search-formula-parm').find('.luckysheet-modal-dialog-content').css('min-width', 300).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
    const winw = $(window).width(), winh = $(window).height();
    const scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
    ...
    $('#luckysheet-search-formula-parm .parmBox').each(function(index,e){
      ...
      if(formula.getfunctionParam(parmtxt).fn == null){ //参数不是公式
        if(formula.iscelldata(parmtxt)){ //参数是选区
          const txtdata = luckysheet_getcelldata(parmtxt).data;
  ...
  parmTxtShow: function(parmtxt){
    if(formula.getfunctionParam(parmtxt).fn == null){ //参数不是公式
      if(formula.iscelldata(parmtxt)){ //参数是选区
        const cellrange = formula.getcellrange(parmtxt);
        const r1 = cellrange.row[0],
          r2 = cellrange.row[1],
      ...
      const cellrange = formula.getcellrange(txt);
      const r1 = cellrange.row[0],
        r2 = cellrange.row[1],
        c1 = cellrange.column[0],
        c2 = cellrange.column[1];
  ...
  functionStrCompute: function(){
    ...
    const formulatxt = $('#luckysheet-search-formula-parm').find('.luckysheet-modal-dialog-title-text').text();
    const p = Store.luckysheet_function[formulatxt].p;
    ...
    const function_str = formula.functionHTMLGenerate(functionHtmlTxt);
    $('#luckysheet-rich-text-editor').html(function_str);
    $('#luckysheet-functionbox-cell').html($('#luckysheet-rich-text-editor').html());

    if(isVal){ //公式计算
      const fp = $.trim(formula.functionParserExe($('#luckysheet-rich-text-editor').text()));
