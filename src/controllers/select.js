...
function selectHightlightShow(isRestore = false) {
  ...
  if (Store.luckysheet_select_save.length > 0) {
    for (let i = 0; i < Store.luckysheet_select_save.length; i++) {
      const r1 = Store.luckysheet_select_save[i].row[0],
        r2 = Store.luckysheet_select_save[i].row[1];
      const c1 = Store.luckysheet_select_save[i].column[0],
...
function selectTitlesShow(rangeArr, isRestore = false) {
  const s = $.extend(true, [], rangeArr);

  let rowTitleMap = {}, columnTitleMap = {};
  for (let i = 0; i < s.length; i++) {
    const r1 = s[i]['row'][0], r2 = s[i]['row'][1], c1 = s[i]['column'][0], c2 = s[i]['column'][1];
  ...
  for (let i = 0; i < rowTitleRange.length; i++) {
    const r1 = rowTitleRange[i][0], r2 = rowTitleRange[i][rowTitleRange[i].length - 1];
    const row = rowLocationByIndex(r2)[1], row_pre = rowLocationByIndex(r1)[0];
  ...
  for (let j = 0; j < columnTitleRange.length; j++) {
    const c1 = columnTitleRange[j][0], c2 = columnTitleRange[j][columnTitleRange[j].length - 1];
    const col = colLocationByIndex(c2)[1], col_pre = colLocationByIndex(c1)[0];
...
function selectTitlesMap(rangeMap, range1, range2) {
  const map = $.extend(true, {}, rangeMap);
...
function collaborativeEditBox() {
  const all_width = Store.visibledatacolumn;//当前操作页的所有列距离左边的距离
  const all_height = Store.visibledatarow;//当前操作页的所有列距离顶部的距离

  Store.cooperativeEdit.changeCollaborationSize.forEach(value => {
    if (value.i == Store.currentSheetIndex) {
      const count_col = value.v.column;//系统提示框所在的列范围
      ...
      const count_row = value.v.row;//系统提示框所在的行范围
      ...
      const range = Store.cooperativeEdit.merge_range; //获取单元格合并后的数据
      ...
      if (Store.config.columnlen !== null) {
        ...
        for (const k in Store.config.columnlen) {
      ...
      if (Store.config.rowlen !== null) {
        for (const k in Store.config.rowlen) {
      ...
      // 改变列宽的位置在提示框范围内
      const flag_width = value.v.column[0] <= Store.luckysheet_cols_change_size_start[1] && Store.luckysheet_cols_change_size_start[1] <= value.v.column[1];
      if (flag_width) {
        if (Store.luckysheet_cols_change_size_start[1] == 0) {
          change_width = all_width[0] - 1;
        } else {
          // 不在提示框范围内
          const counts = value.v.column;
          change_width = all_width[counts[1]] - all_width[counts[0] - 1] - (counts[1] - counts[0] + 1);
        }
      }
      const flag_height = value.v.row[0] <= Store.luckysheet_rows_change_size_start[1] && Store.luckysheet_rows_change_size_start[1] <= value.v.row[1];
      if (flag_height) {
        if (Store.luckysheet_rows_change_size_start[1] == 0) {
          change_height = all_height[0] - 1;
        } else {
          const counts = value.v.row;
      ...
      if (Object.keys(range).length > 0 ) {
        ...
        if(flag_sure_merge) {
          // 合并成一个时执行
          const flag_merge_width = range.column[0] <= value.v.column[0] && range.column[1] >= value.v.column[1];
          ...
          const flag_merge_height = range.row[0] <= value.v.row[0] && range.row[1] >= value.v.row[1];
          if (flag_merge_height) {
            if (range.row[0] !== 0) {
              const counts = range.row;
              change_height = all_height[counts[1]] - all_height[counts[0] - 1] - (counts[1] - counts[0] + 1);
            } else {
              change_top = 0;
            }
      ...
      const change_bottom = $(`#luckysheet-multipleRange-show-${  value.id}`)[0].offsetHeight - 1;
...
function selectionCopyShow(range) {
  ...
  if (range.length > 0) {
    for (let s = 0; s < range.length; s++) {
      ...
      const copyDomHtml = `<div class="luckysheet-selection-copy" style="display: block; left: ${  col_pre  }px; width: ${  col - col_pre - 1  }px; top: ${  row_pre  }px; height: ${  row - row_pre - 1  }px;">` +
...
function luckysheet_count_show(left, top, width, height, rowseleted, columnseleted) {
  const rowl = rowseleted[1] - rowseleted[0] + 1,
  ...
  const scrollWidth = $('#luckysheet-cell-main').scrollLeft(),
...
function selectHelpboxFill() {
  ...
  const rf = range['row_focus'], cf = range['column_focus'];
