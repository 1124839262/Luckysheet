...
import {$$, getObjType, luckysheetContainerFocus, showrightclickmenu} from '../utils/util';
import {getRangetxt, getSheetIndex} from '../methods/get';
import {colLocation, colLocationByIndex, mouseposition, rowLocation, rowLocationByIndex} from '../global/location';
import {checkIsAllowEdit, hasPartMC, isEditMode, isRealNull, isRealNum} from '../global/validate';
import {countfunc} from '../global/count';
import formula from '../global/formula';
import {luckysheetDeleteCell, luckysheetdeletetable, luckysheetextendtable} from '../global/extend';
import {jfrefreshgrid, jfrefreshgrid_rhcw, jfrefreshgridall} from '../global/refresh';
...
export function rowColumnOperationInitial() {
    ...
    .mousedown(function(event) {
      ...
      if (event.which == '3') {
        ...
        for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
          const obj_s = Store.luckysheet_select_save[s];
  ...
  $('#luckysheet-cols-h-c')
    .mousedown(function(event) {
      ...
      const mouse = mouseposition(event.pageX, event.pageY);
      const x = mouse[0] + $(this).scrollLeft();
  ...
  $('#luckysheet-top-left-add-selected').click(function(event) {
    ...
    const st_index = Store.luckysheet_select_save[0][Store.luckysheetRightHeadClickIs][0];
  ...
  $('#luckysheetColsRowsHandleAdd_column').click(function (event) {
    ...
    const st_index = Store.luckysheet_select_save[0].column[0];
  ...
  $('#luckysheet-bottom-right-add-selected').click(function(event) {
    ...
    const st_index = Store.luckysheet_select_save[0][Store.luckysheetRightHeadClickIs][1];
  ...
  $('#luckysheet-delCellsMoveLeft').click(function(event) {
    ...
    const str = Store.luckysheet_select_save[0].row[0],
  ...
  $('#luckysheet-column-row-width-selected').click(function(event) {
    ...
    if (Store.luckysheetRightHeadClickIs == 'row') {
      ...
      for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
        const r1 = Store.luckysheet_select_save[s].row[0];
        const r2 = Store.luckysheet_select_save[s].row[1];
      ...
      for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
        const c1 = Store.luckysheet_select_save[s].column[0];
        const c2 = Store.luckysheet_select_save[s].column[1];
...
function luckysheetcolsdbclick() {
  ...
  const colIndex = colLocation(x)[2];
  const d = editor.deepCopyFlowData(Store.flowdata);
  ...
  for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
    ...
    if (colIndex < c1 || colIndex > c2) {
      ...
      for (let r = dataset_row_st; r <= dataset_row_ed; r++) {
        ...
        const cellWidth = colLocationByIndex(colIndex)[1] - colLocationByIndex(colIndex)[0] - 2;
