...
import {datenum_local, update} from '../global/format';
import {setCellFormat, setCellValue} from '../global/api';
...
const cellDatePickerCtrl = {
  cellFocus: function (r, c, cell) {
    ...
    const type = cell.ct.fa || 'YYYY-MM-DD';
    const defaultDate = update('yyyy-MM-dd hh:mm:ss', cell.v);
    const dateFormat = fitFormat(type);
    ...
    const fp = flatpickr('#luckysheet-input-box', {
      ...
      onChange: function (selectedDates, dateStr) {
        const currentVal = datenum_local(new Date(selectedDates));
