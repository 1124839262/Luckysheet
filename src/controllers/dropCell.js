...
const luckysheetDropCell = {
  ...
  createIcon: function(){
    ...
    const copy_r = _this.copyRange['row'][1],
      copy_c = _this.copyRange['column'][1];
    const apply_r = _this.applyRange['row'][1],
    ...
    const col = colLocationByIndex(col_index)[1],
    ...
    $('#luckysheet-dropCell-icon').mouseover(function(){
      ...
      const typeItemHide = _this.typeItemHide();
      ...
      const menuW = $('#luckysheet-dropCell-typeList').width(),
  ...
  typeItemHide: function(){
    ...
    const copyRange = _this.copyRange;
    const str_r = copyRange['row'][0], end_r = copyRange['row'][1];
    const str_c = copyRange['column'][0], end_c = copyRange['column'][1];
  ...
  update: function(){
    ...
    const borderInfoCompute = getBorderInfoCompute();
    const dataVerification = $.extend(true, {}, file['dataVerification']);

    const direction = _this.direction;
    const type = _this.applyType;

    //复制范围
    const copyRange = _this.copyRange;
    const copy_str_r = copyRange['row'][0], copy_end_r = copyRange['row'][1];
    const copy_str_c = copyRange['column'][0], copy_end_c = copyRange['column'][1];
    const copyData = _this.getCopyData(d, copy_str_r, copy_end_r, copy_str_c, copy_end_c, direction);
    ...
    //应用范围
    const applyRange = _this.applyRange;
    const apply_str_r = applyRange['row'][0], apply_end_r = applyRange['row'][1];
    const apply_str_c = applyRange['column'][0], apply_end_c = applyRange['column'][1];
    ...
    if(direction == 'down' || direction == 'up'){
      ...
      for(let i = apply_str_c; i <= apply_end_c; i++){
        ...
        const applyData = _this.getApplyData(copyD, csLen, asLen);
        ...
        if(direction == 'down'){
          for(let j = apply_str_r; j <= apply_end_r; j++){
            ...
            if(cell.f != null){
              ...
              const v = formula.execfunction(f, j, i);
        ...
        if(direction == 'right'){
          for(let j = apply_str_c; j <= apply_end_c; j++){
            ...
            if(cell.f != null){
              ...
              const v = formula.execfunction(f, i, j);
    ...
    if(cdformat != null && cdformat.length > 0){
      for(let i = 0; i < cdformat.length; i++){
        ...
        for(let j = 0; j < cdformat_cellrange.length; j++){
          const range = conditionformat.CFSplitRange(cdformat_cellrange[j], {'row': copyRange['row'], 'column': copyRange['column']}, {'row': applyRange['row'], 'column': applyRange['column']}, 'operatePart');
          if(range.length > 0){
  ...
  getApplyData: function(copyD, csLen, asLen){
    ...
    if(copyD_number){
      for(let i = 0; i < copyD_number.length; i++){
        ...
        const arrIndex = _this.getDataIndex(csLen, asLen, copyD_number[i]['index']);
    ...
    if(copyD_extendNumber){
      for(let i = 0; i < copyD_extendNumber.length; i++){
        ...
        const arrIndex = _this.getDataIndex(csLen, asLen, copyD_extendNumber[i]['index']);
    ...
    if(copyD_date){
      for(let i = 0; i < copyD_date.length; i++){
        ...
        const arrIndex = _this.getDataIndex(csLen, asLen, copyD_date[i]['index']);
    ...
    if(copyD_chnNumber){
      for(let i = 0; i < copyD_chnNumber.length; i++){
        ...
        const arrIndex = _this.getDataIndex(csLen, asLen, copyD_chnNumber[i]['index']);
    ...
    if(copyD_chnWeek2){
      for(let i = 0; i < copyD_chnWeek2.length; i++){
        ...
        const arrIndex = _this.getDataIndex(csLen, asLen, copyD_chnWeek2[i]['index']);
    ...
    if(copyD_chnWeek3){
      for(let i = 0; i < copyD_chnWeek3.length; i++){
        ...
        const arrIndex = _this.getDataIndex(csLen, asLen, copyD_chnWeek3[i]['index']);
    ...
    if(copyD_other){
      for(let i = 0; i < copyD_other.length; i++){
        ...
        const arrIndex = _this.getDataIndex(csLen, asLen, copyD_other[i]['index']);
    ...
    if(type == '0'){ //复制单元格
      ...
      if(data.length == 1){
        ...
        for(let i = 1; i <= newLen; i++){
          ...
          const date = dayjs(d['m']).add(step * i, 'days').format('YYYY-MM-DD');
          d['m'] = date;
          d['v'] = genarate(date)[2];
          applyData.push(d);
        ...
        if(dayjs(data[1]['m']).date() == dayjs(data[0]['m']).date() && dayjs(data[1]['m']).diff(dayjs(data[0]['m']), 'months') != 0){
          ...
          for(let i = 1; i <= len; i++){
            ...
            const d = $.extend(true, {}, data[index]);
