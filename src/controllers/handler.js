...
import {checkIsAllowEdit, hasPartMC, isEditMode, isRealNull} from '../global/validate';
...
export default function luckysheetHandler() {
  ...
  $('#luckysheet-sheet-container-c').mousewheel(function(event, delta) {
    const scrollNum = event.deltaFactor < 40 ? 1 : event.deltaFactor < 80 ? 2 : 3;
  ...
  $('#luckysheet-grid-window-1').mousewheel(function(event, delta) {
    ...
    const col_st = luckysheet_searcharray(visibledatacolumn_c, scrollLeft);
  ...
  $(window).resize(function() {
    const luckysheetDocument = document.getElementById(Store.container);
  ...
  $('#luckysheet-cell-main, #luckysheetTableContent')
    .mousedown(function(event) {
      ...
      if (parseInt($input.css('top')) > 0) {
        if (
          ...
          //公式选区
          const rowseleted = [row_index, row_index_ed];
          const columnseleted = [col_index, col_index_ed];
          ...
          if (event.shiftKey) {
            ...
            if (vText.length > 0) {
              const lastWord = vText.substr(vText.length - 1, 1);
          ...
          setTimeout(function() {
            ...
            const $span = $editor.find(`span[rangeindex='${  formula.rangechangeindex  }']`);
      ...
      if ($('#luckysheet-multiRange-dialog').is(':visible')) {
        ...
        if (event.shiftKey) {
          const last = conditionformat.selectRange[conditionformat.selectRange.length - 1];
      ...
      if ($('#luckysheet-dataVerificationRange-dialog').is(':visible')) {
        ...
        if (event.shiftKey) {
          const last = dataVerificationCtrl.selectRange[dataVerificationCtrl.selectRange.length - 1];
    ...
    .mouseup(function(event) {
      if (event.which == '3') {
        ...
        const obj_s = Store.luckysheet_select_save[0];
        ...
        if (obj_s['row'] != null && obj_s['row'][0] == 0 && obj_s['row'][1] == Store.flowdata.length - 1) {
          ...
          for (let i = 0; i < Store.luckysheet_select_save.length; i++) {
            const s = Store.luckysheet_select_save[i];
            const c1 = s.column[0],
              c2 = s.column[1];

            for (let c = c1; c <= c2; c++) {
              const collen = cfg['columnlen'][c] == null ? Store.defaultcollen : cfg['columnlen'][c];
          ...
          for (let i = 0; i < Store.luckysheet_select_save.length; i++) {
            ...
            for (let r = r1; r <= r2; r++) {
              const rowlen = cfg['rowlen'][r] == null ? Store.defaultrowlen : cfg['rowlen'][r];
    ...
    .dblclick(function(event) {
      ...
      if (
        ...
        const column_focus = Store.luckysheet_select_save[0]['column_focus'];
        const row_focus = Store.luckysheet_select_save[0]['row_focus'];
  ...
  $(document).on('mousemove.luckysheetEvent', function(event) {
    ...
    if (formula.functionResizeStatus) {
      ...
      const left = x - Store.luckysheet_sheet_move_data.curleft - $('#luckysheet-sheet-container').offset().left;
      ...
      const winH = $(window).height(),
        winW = $(window).width();
      const myh = Store.luckysheet_model_move_obj.height(),
      ...
      function mouseRender() {
        if (
          ...
          const x = mouse[0];
          const y = mouse[1];
          const winH = $('#luckysheet-cell-main').height() - 20 * Store.zoomRatio,
        ...
        if (Store.luckysheet_select_status) {
          ...
          const row_location = rowLocation(y),
            ...
            row_pre = row_location[0],
            row_index = row_location[2];
          const col_index = Store.visibledatacolumn.length - 1,
          ...
          const row_index = Store.visibledatarow.length - 1,
          ...
          const winH = $(window).height() + scrollTop - Store.sheetBarHeight - Store.statisticBarHeight,
          ...
          const row_index_original = Store.luckysheet_cell_selected_move_index[0],
          ...
          const row_index_original = Store.luckysheet_cell_selected_extend_index[0],
        ...
        //image resize
          ...
          const resizeXY = imageCtrl.resizeXY;
          ...
          if (imgItem.isFixedPos) {
            const minTop =
                            ...
                            Store.calculatebarHeight +
                            Store.columnHeaderHeight;
            const minLeft = Store.rowHeaderWidth;
        ...
        //image cropChange
          ...
          const cropChangeXY = imageCtrl.cropChangeXY;

          const topchange = y - cropChangeXY[1],
            leftchange = x - cropChangeXY[0];

          const imgItem = imageCtrl.images[imageCtrl.currentImgId];
          const cropChange = imageCtrl.cropChange;
          ...
          const imageUrlHandle = Store.toJsonOptions && Store.toJsonOptions['imageUrlHandle'];
          const imgSrc = typeof imageUrlHandle === 'function' ? imageUrlHandle(imgItem.src) : imgItem.src;
          ...
          const resizeXY = luckysheetPostil.resizeXY;
          ...
          const resize = luckysheetPostil.resize;
  ...
  $(document).on('mouseup.luckysheetEvent', function(event) {
    ...
    if (!!pivotTable && pivotTable.movestate) {
      ...
      if (pivotTable.movesave.containerid != 'luckysheet-modal-dialog-pivotTable-list') {
        const $cur = $(event.target).closest('.luckysheet-modal-dialog-slider-config-list');
        if ($cur.length == 0) {
          ...
          $(
            ...
            .each(function() {
              ...
              $('#luckysheet-modal-dialog-pivotTable-list')
                .find('.luckysheet-modal-dialog-slider-list-item')
                .each(function() {
                  const $seleted = $(this).find('.luckysheet-slider-list-item-selected');
    ...
    if (Store.chartparam.luckysheetCurrentChartResize) {
      ...
      if (Store.chartparam.luckysheetInsertChartTosheetChange) {
        ...
        const myWidth1 = Store.chartparam.luckysheetCurrentChartResizeXy[2];
        const myHeight1 = Store.chartparam.luckysheetCurrentChartResizeXy[3];
    ...
    if (luckysheetPostil.move) {
      ...
      const ps_id = luckysheetPostil.currentObj.closest('.luckysheet-postil-show').attr('id');
    ...
    if (Store.luckysheet_rows_change_size) {
      ...
      const images = imageCtrl.moveChangeSize('row', Store.luckysheet_rows_change_size_start[1], size);
    ...
    if (Store.luckysheet_cols_change_size) {
      ...
      const images = imageCtrl.moveChangeSize('column', Store.luckysheet_cols_change_size_start[1], size);
    ...
    if (Store.luckysheet_cell_selected_move) {
      ...
      const borderInfoCompute = getBorderInfoCompute(Store.currentSheetIndex);
  ...
  $('#luckysheet-cell-main div.luckysheet-cs-fillhandle')
    .mousedown(function(event) {
      ...
      Store.luckysheet_cell_selected_extend_time = setTimeout(function() {
        ...
        const mouse = mouseposition(event.pageX, event.pageY);
        const x = mouse[0] + $('#luckysheet-cell-main').scrollLeft() - 5;
        const y = mouse[1] + $('#luckysheet-cell-main').scrollTop() - 5;
    ...
    .dblclick(function() {
      ...
      for (let r = r1 + 1; r < Store.flowdata.length; r++) {
        if (c0 - 1 >= 0 && c1 + 1 < Store.flowdata[0].length) {
          const cell1 = Store.flowdata[r][c0 - 1];
          const cell2 = Store.flowdata[r][c1 + 1];
          ...
          const cell = Store.flowdata[r][c0 - 1];
          ...
          const cell = Store.flowdata[r][c1 + 1];
  ...
  $('#luckysheet-copy-btn, #luckysheet-cols-copy-btn, #luckysheet-paste-btn-title').click(function(event) {
    ...
    //多重选区 有条件格式时 提示
    const cdformat = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)].luckysheet_conditionformat_save;
  ...
  $('#luckysheet-chart-btn-screenshot').click(function() {
    ...
    const st_r = Store.luckysheet_select_save[0].row[0],
      ed_r = Store.luckysheet_select_save[0].row[1];
    const st_c = Store.luckysheet_select_save[0].column[0],
    ...
    const ctx_newCanvas = newCanvas.get(0).getContext('2d');
    ...
    const url = newCanvas.get(0).toDataURL('image/png');
    image.src = url;
  ...
  $(document).on('click.luckysheetEvent', 'a.download', function() {
    const dataURI = $('#luckysheet-confirm-screenshot-save img').attr('src');
    const locale_screenshot = _locale.screenshot;
  ...
  $(document).on('mousedown.luckysheetEvent', 'div.luckysheet-modal-dialog', function(e) {
    ...
    const toffset = Store.luckysheet_model_move_obj.offset();
  ...
  $('#luckysheet-chart-rangeShow').on('mousedown.chartRangeShowMove', '.luckysheet-chart-rangeShow-move', function(
    ...
    const chart_json = Store.currentChart;

    const $id = $(this)
      .parent()
      .attr('id');
    if ($id == 'luckysheet-chart-rangeShow-content') {
      const row_s = chart_json.rangeArray[0].row[0] + chart_json.rangeSplitArray.content.row[0];
      const col_s = chart_json.rangeArray[0].column[0] + chart_json.rangeSplitArray.content.column[0];

      Store.chart_selection.rangeMoveIndex = [row_s, col_s];
    } else if ($id == 'luckysheet-chart-rangeShow-rowtitle') {
      const row_s = chart_json.rangeArray[0].row[0] + chart_json.rangeSplitArray.rowtitle.row[0];
      const col_s = chart_json.rangeArray[0].column[0] + chart_json.rangeSplitArray.rowtitle.column[0];

      Store.chart_selection.rangeMoveIndex = [row_s, col_s];
    } else if ($id == 'luckysheet-chart-rangeShow-coltitle') {
      const row_s = chart_json.rangeArray[0].row[0] + chart_json.rangeSplitArray.coltitle.row[0];
      const col_s = chart_json.rangeArray[0].column[0] + chart_json.rangeSplitArray.coltitle.column[0];
  ...
  $('#luckysheet-chart-rangeShow').on(
    ...
    function(event) {
      ...
      if ($id == 'luckysheet-chart-rangeShow-content') {
        ...
        const row_s = chart_json.rangeArray[0].row[0] + chart_json.rangeSplitArray.rowtitle.row[0];
        const row_e = chart_json.rangeArray[0].row[0] + chart_json.rangeSplitArray.rowtitle.row[1];

        const col_s = chart_json.rangeArray[0].column[0] + chart_json.rangeSplitArray.rowtitle.column[0];
        const col_e = chart_json.rangeArray[0].column[0] + chart_json.rangeSplitArray.rowtitle.column[1];

        Store.chart_selection.rangeResizeIndex = { row: [row_s, row_e], column: [col_s, col_e] };
      } else if ($id == 'luckysheet-chart-rangeShow-coltitle') {
        const row_s = chart_json.rangeArray[0].row[0] + chart_json.rangeSplitArray.coltitle.row[0];
        const row_e = chart_json.rangeArray[0].row[0] + chart_json.rangeSplitArray.coltitle.row[1];

        const col_s = chart_json.rangeArray[0].column[0] + chart_json.rangeSplitArray.coltitle.column[0];
        const col_e = chart_json.rangeArray[0].column[0] + chart_json.rangeSplitArray.coltitle.column[1];
  ...
  const dpi_x = document.getElementById('testdpidiv').offsetWidth * Store.devicePixelRatio;
  const dpi_y = document.getElementById('testdpidiv').offsetHeight * Store.devicePixelRatio;
  ...
  $(document).on('paste.luckysheetEvent', function(e) {
    ...
    if (selection.isPasteAction) {
      ...
      if (
        ...
        const reg = new RegExp('<tr.*?>(.*?)</tr>', 'gs');
        const reg2 = new RegExp('<td.*?>(.*?)</td>', 'gs');
        ...
        for (let i = 0; i < regArr.length; i++) {
          ...
          if (reg2Arr != null) {
            for (let j = 0; j < reg2Arr.length; j++) {
              const cpValue = reg2Arr[j].replace(/<td.*?>/gs, '').replace(/<\/td>/gs, '');
        ...
        //当前页面复制区内容
        const copy_r1 = Store.luckysheet_copy_save['copyRange'][0].row[0],
        ...
        const copy_index = Store.luckysheet_copy_save['dataSheetIndex'];
      ...
      if (
        ...
        if (txtdata.indexOf('table') > -1) {
          ...
          const data = new Array($('#luckysheet-copy-content').find('table tr').length);
          let colLen = 0;
          ...
          $('#luckysheet-copy-content')
            ...
            .each(function() {
              ...
              $tr.find(cellElements).each(function() {
                ...
                // 检测下划线
                const un = $td.css('text-decoration');
                if (un.indexOf('underline') != -1) {
                  cell.un = 1;
                }

                const it = $td.css('font-style');
                if (it == 'normal') {
                  cell.it = 0;
                } else {
                  cell.it = 1;
                ...
                const fs = Math.round((parseInt($td.css('font-size')) * 72) / 96);
                cell.fs = fs;

                const fc = $td.css('color');
                cell.fc = fc;

                // 水平对齐属性
                const ht = $td.css('text-align');
                if (ht == 'center') {
                  cell.ht = 0;
                } else if (ht == 'right') {
                  cell.ht = 2;
                ...
                // 垂直对齐属性
                const vt = $td.css('vertical-align');
                if (vt == 'middle') {
                  cell.vt = 0;
                ...
                if (data[r][c] == null) {
                  ...
                  const r_ab = Store.luckysheet_select_save[0]['row'][0] + r;
                  const c_ab = Store.luckysheet_select_save[0]['column'][0] + c;

                  for (let rp = 0; rp < rowspan; rp++) {
                    for (let cp = 0; cp < colspan; cp++) {
                      if (rp == 0) {
                        const bt = $td.css('border-top');
                        if (
                          bt != null &&
                                                    bt.length > 0 &&
                                                    bt.substr(0, 3).toLowerCase() != '0px'
                        ) {
                          const width = $td.css('border-top-width');
                          const type = $td.css('border-top-style');
                          const color = $td.css('border-top-color');
                          const borderconfig = menuButton.getQKBorder(width, type, color);
                      ...
                      if (rp == rowspan - 1) {
                        const bb = $td.css('border-bottom');
                        if (
                          bb != null &&
                                                    bb.length > 0 &&
                                                    bb.substr(0, 3).toLowerCase() != '0px'
                        ) {
                          const width = $td.css('border-bottom-width');
                          const type = $td.css('border-bottom-style');
                          const color = $td.css('border-bottom-color');
                          const borderconfig = menuButton.getQKBorder(width, type, color);
                      ...
                      if (cp == 0) {
                        const bl = $td.css('border-left');
                        if (
                          bl != null &&
                                                    bl.length > 0 &&
                                                    bl.substr(0, 3).toLowerCase() != '0px'
                        ) {
                          const width = $td.css('border-left-width');
                          const type = $td.css('border-left-style');
                          const color = $td.css('border-left-color');
                          const borderconfig = menuButton.getQKBorder(width, type, color);
                      ...
                      if (cp == colspan - 1) {
                        const br = $td.css('border-right');
                        if (
                          br != null &&
                                                    br.length > 0 &&
                                                    br.substr(0, 3).toLowerCase() != '0px'
                        ) {
                          const width = $td.css('border-right-width');
                          const type = $td.css('border-right-style');
                          const color = $td.css('border-right-color');
                          const borderconfig = menuButton.getQKBorder(width, type, color);
  ...
  if (luckysheetConfigsetting.enablePage) {
    $('#luckysheet-bottom-page-next')
      .click(function() {
        const queryExps = luckysheetConfigsetting.pageInfo.queryExps;
        const reportId = luckysheetConfigsetting.pageInfo.reportId;
        const fields = luckysheetConfigsetting.pageInfo.fields;
        const mobile = luckysheetConfigsetting.pageInfo.mobile;
        const frezon = luckysheetConfigsetting.pageInfo.frezon;
        const currentPage = luckysheetConfigsetting.pageInfo.currentPage;
        const totalPage = luckysheetConfigsetting.pageInfo.totalPage;
        const pageUrl = luckysheetConfigsetting.pageInfo.pageUrl;
        ...
        method.addDataAjax(
          ...
          function() {
            ...
            if (
              ...
              const pageInfoFull = replaceHtml(locale_info.pageInfoFull, {
              ...
              const pageInfo = replaceHtml(locale_info.pageInfo, {
                total: luckysheetConfigsetting.total,
                totalPage: luckysheetConfigsetting.pageInfo.totalPage,
