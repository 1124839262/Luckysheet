...
const ifFormulaGenerator = {
  ...
  init: function(){
    ...
    $(document).off('click.IFsingRange').on('click.IFsingRange', '#luckysheet-ifFormulaGenerator-dialog .singRange', function(){
      const value = $('#luckysheet-ifFormulaGenerator-dialog #compareValue').val().trim();
    ...
    $(document).off('click.IFsingRangeConfirm').on('click.IFsingRangeConfirm', '#luckysheet-ifFormulaGenerator-singleRange-confirm', function(){
      ...
      const value = $(this).parents('#luckysheet-ifFormulaGenerator-singleRange-dialog').find('input').val().trim();
    ...
    $(document).off('click.IFmultiRangeConfirm').on('click.IFmultiRangeConfirm', '#luckysheet-ifFormulaGenerator-multiRange-confirm', function(){
      ...
      const value = $(this).parents('#luckysheet-ifFormulaGenerator-multiRange-dialog').find('input').val().trim();
      const cellrange = formula.getcellrange(value);
      const str_r = cellrange['row'][0],
      ...
      const smallNum = arr[arr.length - 1];
    ...
    $(document).off('click.IFcreateBtn').on('click.IFcreateBtn', '#luckysheet-ifFormulaGenerator-dialog #createBtn', function(){
      const compareValue = $(this).parents('#luckysheet-ifFormulaGenerator-dialog').find('#compareValue').val().trim();
      if(compareValue == ''){
        _this.info(locale_formula.ifGenTipNotNullValue);
        return;
      }

      const method = $(this).parents('#luckysheet-ifFormulaGenerator-dialog').find('#DivisionMethod option:selected').val();
      if(method == '2'){
        ...
        const smallRange = $(this).parents('#luckysheet-ifFormulaGenerator-dialog').find('#smallRange').val().trim();
        const largeRange = $(this).parents('#luckysheet-ifFormulaGenerator-dialog').find('#largeRange').val().trim();
        const DivisionMethodVal = $(this).parents('#luckysheet-ifFormulaGenerator-dialog').find('#DivisionMethodVal').val().trim();
    ...
    $(document).off('click.IFconfirmBtn').on('click.IFconfirmBtn', '#luckysheet-ifFormulaGenerator-dialog-confirm', function(){
      const $item = $(this).parents('#luckysheet-ifFormulaGenerator-dialog').find('.ifList .item');
      let str = '';

      $($item.toArray().reverse()).each(function(i, e){
        const smallNum = $(e).find('.smallNum').val().trim();
        const largeNum = $(e).find('.largeNum').val().trim();
        const operator = $(e).find('.operator option:selected').val();
        const operator2 = $(e).find('.operator2 option:selected').val();
        const compareValue = $(e).find('.compareValue').text();
  ...
  ifFormulaDialog: function(fp){
    ...
    if(fp){
      const arr = fp.split('if(');

      for(let i = 1; i < arr.length; i++){
        const txt = arr[i].replace('and(','').replace(/\)/g,'').replace(/\"/g,'');
        let arr2 = txt.split(',');
    ...
    const content = '<div class="ifAttr">'+
                        '<div class="attrBox">'+
    ...
    const $t = $('#luckysheet-ifFormulaGenerator-dialog').find('.luckysheet-modal-dialog-content').css('min-width', 590).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
  ...
  singleRangeDialog: function(value){
    ...
    const $t = $('#luckysheet-ifFormulaGenerator-singleRange-dialog').find('.luckysheet-modal-dialog-content').css('min-width', 400).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
    const winw = $(window).width(), winh = $(window).height();
    const scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
  ...
  multiRangeDialog: function(){
    ...
    const $t = $('#luckysheet-ifFormulaGenerator-multiRange-dialog').find('.luckysheet-modal-dialog-content').css('min-width', 400).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
    const winw = $(window).width(), winh = $(window).height();
    const scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
  ...
  getIfList: function(compareValue, smallRange, largeRange, method, methodVal){
    ...
    if(method == '0'){
      const len = Math.ceil((largeRange - smallRange) / methodVal);
      for(let i = 0; i <= len; i++){
        const num = smallRange + methodVal * i;
      ...
      const addnum = Math.ceil((largeRange - smallRange) / methodVal);
      for(let i = 0; i <= methodVal; i++){
        const num = smallRange + addnum * i;
        if(i == 0 || num >= largeRange){
          arr.push('');
        }
        else{
  ...
  info: function(title){
    ...
    const $t = $('#luckysheet-ifFormulaGenerator-info').find('.luckysheet-modal-dialog-content').css('min-width', 300).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
    const winw = $(window).width(), winh = $(window).height();
    const scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
