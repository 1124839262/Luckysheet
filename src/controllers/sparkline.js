//sparkline设置
const createClass = function (/* [baseclass, [mixin, ...]], definition */) {
  let args;
...
const clipval = function (val, min, max) {
  if (val < min) {
    return min;
  }
  if (val > max) {
...
const quartile = function (values, q) {
  let vl;
  if (q === 2) {
...
const all = function (val, arr, ignoreNull) {
  let i;
  for (i = arr.length; i--; ) {
...
const formatNumber = function (num, prec, groupsize, groupsep, decsep) {
  let p, i;
...
const luckysheetSparkline = {
  ...
  _genShape: function (shapetype, shapeargs) {
    ...
    // this.type = type;
    // this.args = args;
    const shape = { id:id, type:shapetype, args:shapeargs };
...
const _luckysheetSparkLineOptions = {
  get:function(type){
    return luckysheetSparkline.mergedOptions[type];
  }
};

const _luckysheetSparkLineTarget = {
  drawLine:function(x1, y1, x2, y2, lineColor, lineWidth){
...
for(const item in luckysheetSparkline){
