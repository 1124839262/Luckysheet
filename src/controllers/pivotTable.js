...
const pivotTable = {
  ...
  getCellData: function(cursheetindex, datasheetindex, data_select_save) {
    ...
    const realIndex = getSheetIndex(sheetIndex);
    ...
    const pivotrealIndex = getSheetIndex(_this.pivotDataSheetIndex);

    const otherfile = Store.luckysheetfile[pivotrealIndex];
    ...
    if (_this.filterparm != null) {
      for (const f in _this.filterparm) {
        // 目的是取出rowhidden
        for (const h in _this.filterparm[f]) {
    ...
    for (let c = 0; c < _this.celldata[1].length; c++) {
      const type = isdatatype(_this.celldata[1][c]);
  ...
  resetOrderby: function(obj) {
    const orderby = $('#luckysheet-modal-dialog-config-value .luckysheet-modal-dialog-slider-config-item').index(obj);
  ...
  luckysheetsliderlistclearfilter: function($filter) {
    ...
    const $t = $filter.parent();
    const cindex = $t.data('index');
  ...
  luckysheetsliderlistitemfilter: function($filter) {
    ...
    const byconditiontype = $t.data('byconditiontype');
    $('#luckysheet-pivotTableFilter-selected span')
    ...
    if (byconditiontype == '2') {
      const $input = $('#luckysheet-pivotTableFilter-menu .luckysheet-pivotTableFilter-selected-input2')
        .show()
        .find('input');
    ...
    const rowhiddenother = {}; //其它筛选列的隐藏行
    ...
    setTimeout(function() {
      ...
      for (let r = 1; r < data.length; r++) {
        ...
        const cell = data[r][cindex];

        if (cell != null && cell.ct != null && cell.ct.t == 'd') {
          //单元格是日期
          const v = update('YYYY-MM-DD', cell.v);
      ...
      if (JSON.stringify(dvmap).length > 2) {
        for (const y in dvmap) {
          let ysum = 0;
          let monthHtml = '';

          for (const m in dvmap[y]) {
            let msum = 0;
            let dayHtml = '';

            for (const d in dvmap[y][m]) {
              const dayL = dvmap[y][m][d];
              msum += dayL;
  ...
  setDatatojsfile: function(attr, value, cindex) {
    ...
    const index = getSheetIndex(_this.pivotSheetIndex);
  ...
  createPivotTable: function(e) {
    ...
    const datasheetindex = Store.currentSheetIndex;
    ...
    const select_save = $.extend(true, {}, Store.luckysheet_select_save[0]);
  ...
  changePivotTable: function(index) {
    ...
    const pivotDataSheetIndex = Store.luckysheetfile[getSheetIndex(index)].pivotTable.pivotDataSheetIndex;
    const real_pivotDataSheetIndex = getSheetIndex(pivotDataSheetIndex);
  ...
  refreshPivotTable: function(isRefreshCanvas = true) {
    ...
    const ret = _this.dataHandler(_this.column, _this.row, _this.values, _this.showType, _this.celldata);
    _this.setDatatojsfile('pivotDatas', ret);

    const d = $.extend(true, [], sheetmanage.nulldata);
    let data = d;
  ...
  pivotclick: function(row_index, col_index, index) {
    ...
    const pivotDataSheetIndex = file.pivotTable.pivotDataSheetIndex;
    const real_pivotDataSheetIndex = getSheetIndex(pivotDataSheetIndex);
    ...
    const slider = $('#luckysheet-modal-dialog-slider-pivot');

    const isRangeClick = this.isPivotRange(row_index, col_index);
  ...
  storePivotTableParam: function() {
    ...
    const showtype = $('#luckysheetpivottablevaluecolrow:checked, #luckysheetpivottablevaluecolrow1:checked').val();
    _this.setDatatojsfile('showType', showtype == '0' ? 'row' : 'column');

    const pivotTable = _this.getPivotTableData();
  ...
  getPivotTableData: function(dataindex) {
    ...
    const index = getSheetIndex(dataindex);
  ...
  addValuesToTitle: function(titles, values) {
    const rowLen = titles.length * values.length,
    ...
    if (_this.initial) {
      ...
      $(document)
        ...
        .on(
          ...
          function() {
            const $p = $(this).parents('.luckysheet-mousedown-cancel');
            if ($p.hasClass('year')) {
              $(this)
                .parents('.yearBox')
                .find('.monthList')
      ...
      $('#luckysheet-pivotTableFilter-byvalue-btn-contra').click(function() {
        const $input = $('#luckysheet-pivotTableFilter-byvalue-select .ListBox input[type=\'checkbox\']');
        ...
        //天 对应的 月份
        const $month = $('#luckysheet-pivotTableFilter-byvalue-select .ListBox .monthBox');
        $month.each(function(index, event) {
          let monthDayAllCheck = true;
          const $monthDay = $(event).find('.day input[type=\'checkbox\']');
          $monthDay.each(function(i, e) {
            if ($(e).is(':checked')) {
            } else {
              monthDayAllCheck = false;
        ...
        //天 对应的 年份
        const $year = $('#luckysheet-pivotTableFilter-byvalue-select .ListBox .yearBox');
        $year.each(function(index, event) {
          let yearDayAllCheck = true;
          const $yearDay = $(event).find('.day input[type=\'checkbox\']');
          $yearDay.each(function(i, e) {
            if ($(e).is(':checked')) {
            } else {
              yearDayAllCheck = false;
      ...
      $('#luckysheet-pivotTableFilter-byvalue-input').on('input propertychange', function() {
        ...
        if (v != '') {
          const $check = $('#luckysheet-pivotTableFilter-byvalue-select .ListBox input[type=\'checkbox\']');
          $check.each(function(i, e) {
            const $p = $(e).parents('.luckysheet-mousedown-cancel');
            if ($p.hasClass('day')) {
              //日期
              const day = $(e)
                .siblings('label')
      ...
      $('#luckysheet-dialog-pivotTable-range-seleted').click(function() {
        $('#luckysheet-modal-dialog-slider-pivot').hide();
        luckysheetsizeauto();
        const $t = $('#luckysheet-data-pivotTable-selection'),
          myh = $t.outerHeight(),
          myw = $t.outerWidth();
      ...
      $('#luckysheet-modal-dialog-config-value').on(
        ...
        function(e) {
          ...
          const type = _this.pivot_data_type[cindex.toString()];
          if (sumtype == null) {
            if (type == 'num') {
              sumtype = 'SUM';
            } else {
          ...
          const $menu = $('#luckysheet-pivotTable-config-option-sumtype');
      ...
      $('#luckysheet-pivotTable-config-option-sumtype .luckysheet-cols-menuitem').click(function() {
        const $item = $('#luckysheet-pivotTable-config-option-sumtype').data('item');
        const sumtype = $(this).attr('sumtype');
        $item.data('sumtype', $(this).attr('sumtype'));
        const name = `${_this.getSumTypeName(sumtype)  }:${  $item.data('name')}`;
        $item
          .attr('title', name)
          .find('.luckysheet-modal-dialog-slider-config-item-txt')
          .html(name);
      ...
      $('#luckysheet-pivotTableFilter-confirm').click(function() {
        const $menu = $('#luckysheet-pivotTableFilter-menu');
        const cindex = $menu.data('index');
        ...
        if (
          ...
          const $t = $('#luckysheet-pivotTableFilter-selected span');
          ...
          for (let r = 1; r < d.length; r++) {
            ...
            if (value == 'cellnull') {
              ...
              //日期等于
              const value1 = genarate(caljs['value1'])[2];
              ...
              //大于
              const value1 = parseFloat(caljs['value1']);
          ...
          $('#luckysheet-pivotTableFilter-byvalue-select .ListBox input[type=\'checkbox\']').each(function(
            ...
            if ($(e).closest('.day').length > 0) {
              ...
              const itemV = `${locale_filter.filterDateFormatTip  }#$$$#${  year  }-${  month  }-${  day}`;
          ...
          for (let r = 1; r < d.length; r++) {
            ...
            if (cell == null || isRealNull(cell.v)) {
              ...
              const fmt = update('YYYY-MM-DD', cell.v);
        ...
        const $top = $('#luckysheet-modal-dialog-pivotTable-list .luckysheet-modal-dialog-slider-list-item').eq(
          cindex,
        );
        if (
      ...
      $('#luckysheet-pivotTable-selection-confirm').click(function() {
        const $input = $('#luckysheet-pivotTable-range-selection-input'),
          val = $input.val();
        ...
        if (
          ...
          if (val1.length > 1) {
            ...
            for (const i in Store.luckysheetfile) {
        ...
        .mousemove(function(e) {
          if (_this.movestate) {
            ...
            const x = event.pageX,
              y = event.pageY,
              $container = $(this);
            const curtop = y - $container.offset().top + $container.scrollTop();
            const position = _this.moveitemposition;
            const row_index = luckysheet_searcharray(position, curtop);
          ...
          if (_this.movestate) {
            ...
            if (_this.movesave.containerid == $t.attr('id')) {
              ...
              if ($t.attr('id') == 'luckysheet-modal-dialog-config-value') {
                const type = _this.pivot_data_type[_this.movesave.index.toString()];
                ...
                $('#luckysheet-modal-dialog-config-value')
                  ...
                  .each(function() {
                    if (
                      ...
                      const ni = parseFloat($(this).data('nameindex')) + 1;
        ...
        function() {
          ...
          if ($t.find('i').length == 0) {
            ...
            if (type == 'num') {
              ...
              const $column = $('#luckysheet-modal-dialog-config-column'),
                $row = $('#luckysheet-modal-dialog-config-row');
              const columnitem = $column.find('.luckysheet-modal-dialog-slider-config-item'),
    ...
    for (let i = 0; i < _this.celldata[0].length; i++) {
      ...
      if (restore && _this.filterparm != null) {
        if (_this.filterparm[i.toString()] != null) {
          const itemset = _this.filterparm[i.toString()];
  ...
  getTitleFromGroup: function(group, config, dataposition) {
    const _this = this;
    const orderbygroup = _this.orderbygroup(group, config, dataposition);
  ...
  generategrouparraymain: function(group, config) {
    ...
    for (let i = 0; i < group.length; i++) {
      const name = group[i].name;
      const arr = _this.generategrouparray(group[i].children, config, 1);
  ...
  dataHandler: function(column, row, values, showType, celldata) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const v_str = '';
      for (let v = 0; v < values.length; v++) {
        const d_value = getcellvalue(values[v].index, null, d);
        ...
        for (let r = 0; r < rowtitle_c.length; r++) {
          for (let c = 0; c < coltitle_c.length; c++) {
            const indicator = rowtitle_c[r] + coltitle_c[c];
    ...
    for (const indicator in dataposition) {
      const json = dataposition[indicator];
      ...
      if (json.sumtype == 'SUM') {
        ...
        const numArr = json.digitaldata.sort(function(a, b) {
          return a - b;
        });
        const numLen = numArr.length;
        const numindex = parseInt(numLen / 2);
        ...
        const mean = json.sum / json.count;
      ...
      const newAcc = numfloatlen(json.result);
      if (newAcc > json.acc) {
        json.acc = newAcc;
      }
    ...
    const datacoltitle_index = datacoltitle;
    datacoltitle = luckysheetArray.transpose(datacoltitle, false);

    const valuenslen = values.length == 0 ? 0 : 1;
    const rowLen =
    ...
    const rowOver = datacoltitle.length,
    ...
    for (let r = 0; r < rowLen; r++) {
      ...
      for (let c = 0; c < colLen; c++) {
        const drt = datarowtitle[r - rowOver];
        ...
        if (r < rowOver && c < colOver) {
          ...
          let suffix = '';
          const dct = datacoltitle_index[c - colOver];
          if (dct != null) {
    ...
    if (values.length == 1 && column.length > 0 && row.length > 0) {
      ...
      const titleRow = retdata.splice(column.length, 1);
      const newRetdata = [];
  ...
  drillDown: function(row_index, col_index) {
    ...
    const cell = _this.pivotDatas[row_index][col_index];
    const d = $.extend(true, [], sheetmanage.nulldata);
