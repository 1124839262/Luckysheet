...
const alternateformat = {
  ...
  getModelBox: function(hasRowHeader, hasRowFooter){
    ...
    for(let i = 0; i < _this.FixedModelColor.length; i++){
      const obj = _this.FixedModelColor[i];
    ...
    //自定义 模板
    const modelCustom = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)]['luckysheet_alternateformat_save_modelCustom'];
    if(modelCustom != null && modelCustom.length > 0){
      let modelCustomHtml = '';

      for(let i = 0; i < modelCustom.length; i++){
        const obj = modelCustom[i];
  ...
  init: function(){
    ...
    $(document).off('keydown.AFrangeInput').on('keydown.AFrangeInput', '#luckysheet-alternateformat-range input', function(e){
      const rangeValue = $(this).val().trim();
      if(e.keyCode == 13){
        _this.update();
      }
    });
    $(document).off('click.AFrangeIcon').on('click.AFrangeIcon', '#luckysheet-alternateformat-range .fa-table', function(){
      ...
      const rangeValue = $(this).parents('#luckysheet-alternateformat-range').find('input').val().trim();
    ...
    $(document).off('click.AFrDCf').on('click.AFrDCf', '#luckysheet-alternateformat-rangeDialog-confirm', function(){
      const rangeValue = $(this).parents('#luckysheet-alternateformat-rangeDialog').find('input').val().trim();
    ...
    $(document).off('click.AFselectColor').on('click.AFselectColor', '#luckysheet-modal-dialog-slider-alternateformat .luckysheet-color-menu-button-indicator', function(){
      const $parent = $(this).closest('.toningbox');
    ...
    $(document).off('click.AFselectColorConfirm').on('click.AFselectColorConfirm', '#luckysheet-alternateformat-colorSelect-dialog-confirm', function(){
      const $parent = $(this).parents('#luckysheet-alternateformat-colorSelect-dialog');
      ...
      const source = $parent.find('.currenColor').attr('data-source');
      ...
      const index = _this.modelfocusIndex;
      const len = _this.FixedModelColor.length;
      ...
      if(index < len){
        ...
        const modelCustom = file['luckysheet_alternateformat_save_modelCustom'];
    ...
    $(document).off('click.AFremove').on('click.AFremove', '#luckysheet-alternateformat-remove', function(){
      ...
      //保存之前的规则
      const historyRules = $.extend(true, [], ruleArr);
      ...
      const currentRules = $.extend(true, [], ruleArr);
  ...
  perfect: function(){
    ...
    const existsIndex = _this.rangeIsExists(range)[1];

    const obj = $.extend(true, {}, Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)]['luckysheet_alternateformat_save'][existsIndex]);

    //应用范围
    const cellrange = obj['cellrange'];
    ...
    //页眉、页脚
    const hasRowHeader = obj['hasRowHeader'];
    const hasRowFooter = obj['hasRowFooter'];
  ...
  modelToningColor: function(){
    const format = this.getFormatByIndex();
  ...
  colorSelectDialog: function(currenColor, colorType, source){
    ...
    const $t = $('#luckysheet-alternateformat-colorSelect-dialog')
  ...
  rangeDialog: function(value){
    ...
    const $t = $('#luckysheet-alternateformat-rangeDialog')
        .find('.luckysheet-modal-dialog-content')
        .css('min-width', 300)
        .end(),
      myh = $t.outerHeight(),
  ...
  rangeIsExists: function(range, index){
    ...
    if(AFarr.length > 0){
      ...
      //获取当前选区
      const rangeMap = _this.getRangeMap(range['row'], range['column']);

      //遍历
      for(const x in rangeMap){
        if(isExists){
          break;
        }
  ...
  getRangeMap: function(row, column){
    const map = {};
    const st_r = row[0], ed_r = row[1], st_c = column[0], ed_c = column[1];
  ...
  getIndexByFormat: function(format){
    ...
    //格式样式 模板
    const modelList = _this.FixedModelColor;
    for(let i = 0; i < modelList.length; i++){
      const obj = modelList[i];
  ...
  new: function(cellrange){
    ...
    const format = _this.getFormatByIndex();
  ...
  update: function(){
    ...
    const alternatingColors =_locale.alternatingColors;
    //获取标识
    const dataIndex = $('#luckysheet-alternateformat-remove').data('index');

    //应用范围
    const rangeValue = $('#luckysheet-modal-dialog-slider-alternateformat #luckysheet-alternateformat-range input').val().trim();
    ...
    const cellrange = formula.getcellrange(rangeValue);
    const isExists = _this.rangeIsExists(cellrange, dataIndex)[0];
  ...
  getComputeMap: function(){
    const file = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)];
    const ruleArr = file['luckysheet_alternateformat_save'];

    const computeMap = this.compute(ruleArr);
  ...
  compute: function(obj){
    ...
    if(obj != null && obj.length > 0){
      for(let i = 0; i < obj.length; i++){
        const cellrange = obj[i]['cellrange'];
        const format = obj[i]['format'];
        const hasRowHeader = obj[i]['hasRowHeader'];
        const hasRowFooter = obj[i]['hasRowFooter'];
        const st_r = cellrange['row'][0],
