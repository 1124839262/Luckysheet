...
const luckysheetSplitColumn = {
  createDialog: function(){
    ...
    const $t = $('#luckysheet-splitColumn-dialog').find('.luckysheet-modal-dialog-content').css('min-width', 400).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
    ...
    const dataArr = _this.getDataArr();
  ...
  init: function(){
    ...
    $(document).off('change.SPCinpcheckbox').on('change.SPCcheckbox', '#luckysheet-splitColumn-dialog .box input[type=\'checkbox\']', function(){
      const regStr = _this.getRegStr();
      const dataArr = _this.getDataArr(regStr);
      _this.dataPreview(dataArr);
    });
    ...
    $(document).off('click.SPCconfirm').on('click.SPCconfirm', '#luckysheet-splitColumn-dialog #luckysheet-splitColumn-dialog-confirm', function(){
      ...
      for(let i = 0; i < dataArr.length; i++){
        for(let j = 1; j < dataArr[0].length; j++){
          const cell = Store.flowdata[r + i][c + j];
  ...
  update: function(r, c, dataArr){
    ...
    const range = [{ 'row': [st_r, st_r + dataArr.length - 1], 'column': [st_c, st_c + dataArr[0].length - 1] }];
  ...
  dataPreview: function(dataArr){
    ...
    const tableHtml = `<table>${  trHtml  }</table>`;
  ...
  getRegStr: function(){
    ...
    $('#luckysheet-splitColumn-dialog .box input[type=\'checkbox\']:checked').each(function(i, e){
      ...
      if($id === 'splitColumn_type_01'){ //Tab键
        ...
        const txt = $(e).siblings('input[type=\'text\']').val().trim();
  ...
  getDataArr: function(regStr){
    ...
    const r2 = Store.luckysheet_select_save[0].row[1];
    const c = Store.luckysheet_select_save[0].column[0];
