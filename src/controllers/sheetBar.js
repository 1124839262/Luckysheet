...
import {keycode, sheetselectlistHTML, sheetselectlistitemHTML} from './constant';
import {mouseclickposition, replaceHtml,} from '../utils/util';
...
const luckysheetsheetrightclick = function ($t, $cur, e) {
  //引用单元格范围时，禁止切换sheer
...
export function refreshProtectionContent() {
  //刷新页面
  const sheetFile = sheetmanage.getSheetByIndex();//获取到的数据是正确的
...
export function initialSheetBar(){
  ...
  $('#luckysheet-sheet-area').on('mousedown', 'div.luckysheet-sheets-item', function (e) {
    ...
    if ($item.hasClass('luckysheet-sheets-item-active') && $item.find('.luckysheet-sheets-item-name').attr('contenteditable') == 'false') {
      jfdbclicklagTimeout = setTimeout(function () {
        ...
        const $itemclone = $item.clone().css('visibility', 'hidden').attr('id', 'luckysheet-sheets-item-clone');
    ...
    const $t = $(this), $cur = $(e.target);
    luckysheetsheetrightclick($t, $cur, e);
    server.keepHighLightBox();
  });

  const luckysheetsheetnameeditor = function ($t) {
  ...
  $('#luckysheet-sheet-area').on('input', 'span.luckysheet-sheets-item-name', function () {
    ...
    const maxLength = Store.defaultSheetNameMaxLength;
    if(maxLength  === 0){
      return;
    }
  ...
  $('#luckysheet-sheet-area').on('blur', 'span.luckysheet-sheets-item-name', function (e) {
    ...
    const $t = $(this);
    const txt = $t.text(), oldtxt = $t.data('oldtxt');
  ...
  $('#luckysheetsheetconfigrename').click(function () {
    const $name = luckysheetcurrentSheetitem.find('span.luckysheet-sheets-item-name');
  ...
  $('#luckysheet-sheets-m').click(function (e) {
    ...
    if (initialOpenSheet) {
      ...
      $('#luckysheet-sheet-list').on('click', '.luckysheet-cols-menuitem', function (e) {
        ...
        const $item = $(this), index = $item.data('index');
    ...
    const $t = $('#luckysheet-sheet-list');

    const left = $(this).offset().left - $(`#${  Store.container}`).offset().left;
    const bottom = $(this).height() + $('#luckysheet-sta-content').height() + 12;
