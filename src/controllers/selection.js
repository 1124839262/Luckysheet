...
const selection = {
  clearcopy: function(e) {
    ...
    if (!clipboardData) {
      const textarea = $('#luckysheet-copy-content').css('visibility', 'hidden');
      textarea.val(cpdata);
      textarea.focus();
      textarea.select();
  ...
  copy: function(e) {
    ...
    for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
      const range = Store.luckysheet_select_save[s];
      ...
      for (let copyR = r1; copyR <= r2; copyR++) {
        ...
        for (let copyC = c1; copyC <= c2; copyC++) {
          ...
          const cell = Store.flowdata[copyR][copyC];
  ...
  pasteHandler: function(data, borderInfo) {
    ...
    if (typeof data === 'object') {
      ...
      const minh = Store.luckysheet_select_save[0].row[0], //应用范围首尾行
        maxh = minh + copyh - 1;
      const minc = Store.luckysheet_select_save[0].column[0], //应用范围首尾列
        maxc = minc + copyc - 1;
  ...
  pasteHandlerOfCutPaste: function(copyRange) {
    ...
    const borderInfoCompute = getBorderInfoCompute(copySheetIndex);
    const c_dataVerification = $.extend(
      true,
      {},
      Store.luckysheetfile[getSheetIndex(copySheetIndex)]['dataVerification'],
    );
    const dataVerification = $.extend(
      true,
      {},
    ...
    if (Store.currentSheetIndex != copySheetIndex) {
      //跨表操作
      const sourceData = $.extend(true, [], Store.luckysheetfile[getSheetIndex(copySheetIndex)]['data']);
      const sourceConfig = $.extend(true, {}, Store.luckysheetfile[getSheetIndex(copySheetIndex)]['config']);

      const sourceCurData = $.extend(true, [], sourceData);
      ...
      const curCdformat = $.extend(true, [], cdformat);
      if (curCdformat != null && curCdformat.length > 0) {
        for (let i = 0; i < curCdformat.length; i++) {
          const cellrange = curCdformat[i].cellrange;
          let emptyRange = [];
  ...
  pasteHandlerOfCopyPaste: function(copyRange) {
    ...
    const copyData = $.extend(true, [], arr);
    ...
    let cdformat = null;
    if (copyRange['copyRange'].length == 1) {
      const c_file = Store.luckysheetfile[getSheetIndex(copySheetIndex)];
      const a_file = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)];

      const ruleArr_cf = $.extend(true, [], c_file['luckysheet_conditionformat_save']);
