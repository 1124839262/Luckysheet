...
import {openSelfModel, replaceHtml} from '../utils/util';
...
function recycleSeletion(cycleFunction, dataIsNullFunction){
  if(Store.luckysheet_select_save != null && Store.luckysheet_select_save.length > 0){
    const sheetFile = sheetmanage.getSheetByIndex(), data=sheetFile.data;
    if(data!=null){

      for(let i=0;i<Store.luckysheet_select_save.length;i++){
        const selection = Store.luckysheet_select_save[i];
        const row = selection.row, column = selection.column;
        for(let r=row[0];r<=row[1];r++){
          for(let c=column[0];c<=column[1];c++){
            let cell;

            const margeset = menuButton.mergeborer(data, r, c);
            if (margeset) {
              // row = margeset.row[1];
