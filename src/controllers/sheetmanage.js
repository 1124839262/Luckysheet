...
import {datagridgrowth, getcellFormula, getcellvalue, getdatabyselectionD} from '../global/getdata';
...
const sheetmanage = {
  ...
  generateCopySheetName: function(file, name) {
    ...
    const copyWord = `(${  locale_info.copy}`;
    ...
    let index = '';
    const nameCopy = name + copyWord;
    const sheetNames = [];

    for (let i = 0; i < file.length; i++) {
      const fileName = file[i].name.toString();
  ...
  getSheetByIndex: function(index) {
    ...
    const i = _this.getSheetIndex(index);
  ...
  getSheetByName: function(name) {
    ...
    for (let i = 0; i < Store.luckysheetfile.length; i++) {
      const file = Store.luckysheetfile[i];
      if (file.name == name) {
        return file;
      }
    }
  ...
  addNewSheet: function(e, isPivotTable) {
    ...
    const order = Store.luckysheetfile.length;
    const index = _this.generateRandomSheetIndex();

    const sheetname = _this.generateRandomSheetName(Store.luckysheetfile, isPivotTable);
    ...
    let sheetconfig = {};
    const sheet_defaullt_config = this.getCustomSheet();
    if (
  ...
  setSheetHide: function(index, isDelete) {
    const _this = this;
    const currentIdx = _this.getSheetIndex(index);
    // 钩子 sheetHideBefore
    ...
    const luckysheetcurrentSheetitem = $(`#luckysheet-sheets-item${  index}`);
    ...
    if (luckysheetConfigsetting.showsheetbarConfig.sheet) {
      ...
      const len = showSheetIdxs.length;
      if (1 === len) {
  ...
  reOrderAllSheet: function() {
    ...
    Store.luckysheetfile.sort((x, y) => {
      const order_x = x.order;
      const order_y = y.order;
  ...
  createSheet: function() {
    ...
    for (let i = 0; i < Store.luckysheetfile.length; i++) {
      let display = '';
      const sheetIndex = Store.luckysheetfile[i].index;
  ...
  locationSheet: function() {
    const $c = $('#luckysheet-sheet-container-c'),
      winW = $(`#${  Store.container}`).width();
    const $cursheet = $('#luckysheet-sheet-container-c > div.luckysheet-sheets-item-active').eq(0);
  ...
  copySheet: function(copyindex, e) {
    ...
    const copyarrindex = _this.getSheetIndex(copyindex);
    const copyjson = $.extend(true, {}, Store.luckysheetfile[copyarrindex]);
    ...
    const copyobject = $(`#luckysheet-sheets-item${  copyindex}`);
    $('#luckysheet-sheet-container-c').append(
      replaceHtml(sheetHTML, {
  ...
  deleteSheet: function(index) {
    ...
    const arrIndex = _this.getSheetIndex(index);
    ...
    const removedsheet = Store.luckysheetfile.splice(arrIndex, 1);
    _this.reOrderAllSheet();
  ...
  sheetParamRestore: function(file, data) {
    ...
    if (file['showGridLines'] != null) {
      const showGridLines = file['showGridLines'];
  ...
  initialjfFile: function(menu, title) {
    ...
    _this.getCurSheet();
    const file = Store.luckysheetfile[_this.getSheetIndex(Store.currentSheetIndex)];
    _this.nulldata = datagridgrowth([], Store.defaultrowNum, Store.defaultcolumnNum);
    const data = _this.buildGridData(file);
    ...
    setTimeout(function() {
      ...
      const key = server.gridKey;
      const cahce_key = `${key  }__qkcache`;
      ...
      const ini = function() {
        ...
        if (loadSheetUrl == '') {
          ...
          const sheetindexset = _this.checkLoadSheetIndex(file);
          const sheetindex = [];

          for (let i = 0; i < sheetindexset.length; i++) {
            const item = sheetindexset[i];
          ...
          $.post(loadSheetUrl, { gridKey: server.gridKey, index: sheetindex.join(',') }, function(d) {
            ...
            for (const item in dataset) {
              ...
              const otherfile = Store.luckysheetfile[_this.getSheetIndex(item)];
  ...
  storeSheetParam: function() {
    const index = this.getSheetIndex(Store.currentSheetIndex);
  ...
  storeSheetParamALL: function() {
    ...
    _this.storeSheetParam();
    const index = _this.getSheetIndex(Store.currentSheetIndex);
  ...
  mergeCalculation: function(index) {
    const file = Store.luckysheetfile[this.getSheetIndex(index)];
    ...
    for (const x in mergeConfig) {
      const r = parseInt(x.substr(0, x.indexOf('_')));
      const c = parseInt(x.substr(x.indexOf('_') + 1));
      const mcInfo = mergeConfig[x];
      if (data[r][c] == null) {
        data[r][c] = {};
      }
  ...
  loadOtherFile: function(file) {
    ...
    for (let i = 0; i < Store.luckysheetfile.length; i++) {
      const otherfile = Store.luckysheetfile[i];
      if (otherfile.index == file.index) {
        continue;
      }
      // let otherfile = Store.luckysheetfile[_this.getSheetIndex(item)];
  ...
  changeSheet: function(index, isPivotInitial, isNewSheet, isCopySheet) {
    ...
    if (load != null) {
      ...
      if (loadSheetUrl == '' || Store.luckysheetcurrentisPivotTable || !!isNewSheet) {
        ...
        const sheetindex = _this.checkLoadSheetIndex(file);
  ...
  refreshPivotTableByFile:function(file) {
    const pivotTableConfig = file.pivotTable;

    const column = pivotTableConfig.column;
    const row = pivotTableConfig.row;
    const values = pivotTableConfig.values;
    const showType = pivotTableConfig.showType;
    const filterparm = pivotTableConfig.filterparm;
    const pivotDataSheetIndex = pivotTableConfig.pivotDataSheetIndex;

    const pivotrealIndex = this.getSheetIndex(pivotDataSheetIndex);
    ...
    let rowhidden = {};
    if (filterparm != null) {
      for (const f in filterparm) {
        // 目的是取出rowhidden
        for (const h in filterparm[f]) {
    ...
    const ret = pivotTable.dataHandler(column, row, values, showType, newdata);

    pivotTableConfig.pivotDatas = ret;


    const d = $.extend(true, [], this.nulldata);
    let data = d;
  ...
  checkLoadSheetIndex: function(file) {
    const calchain = formula.getAllFunctionGroup(); //file.calcChain; //index
    const chart = file.chart; //dataSheetIndex
    const pivotTable = file.pivotTable; //pivotDataSheetIndex
    ...
    if (calchain != null) {
      ...
      for (let i = 0; i < calchain.length; i++) {
        const f = calchain[i];
        const dataindex = f.index;
        let formulaTxt = getcellFormula(f.r, f.c, dataindex);

        if (formulaTxt == null) {
          const file = Store.luckysheetfile[this.getSheetIndex(dataindex)];
        ...
        if (formulaTxt.indexOf('!') == -1) {
          ...
          for (const dataSheetIndex in formula.formulaContainSheetList[formulaTxt]) {
          ...
          formula.functionParser(formulaTxt, (str) => {
            ...
            if (str.indexOf('!') > -1) {
              ...
              if (sheet != null) {
                const dataSheetIndex = sheet.index;
                dataIndexList[dataSheetIndex] = 1;
      ...
      for (const index in dataIndexList) {
    ...
    if (chart != null) {
      for (let i = 0; i < chart.length; i++) {
        const cc = chart[i];
        const dataindex = cc.dataSheetIndex;
    ...
    if (pivotTable != null) {
      const dataindex = pivotTable.pivotDataSheetIndex;
  ...
  changeSheetExec: function(index, isPivotInitial, isNewSheet, isCopySheet) {
    const $sheet = $(`#luckysheet-sheets-item${  index}`);
  ...
  delChart: function(chart_id, sheetIndex) {
    const index = this.getSheetIndex(sheetIndex);
    const file = Store.luckysheetfile[index];
  ...
  saveChart: function(json) {
    ...
    const index = this.getSheetIndex(json.sheetIndex);
    const file = Store.luckysheetfile[index];
    ...
    if (file.chart == null) {
      ...
      for (let i = 0; i < file.chart.length; i++) {
        if (file.chart[i].chart_id == json.chart_id) {
          const old = $.extend(true, {}, file.chart[i]);
  ...
  getSheetConfig: function(sheetIndex) {
    ...
    const config = Store.luckysheetfile[_this.getSheetIndex(sheetIndex)].config;
  ...
  restoreCache: function() {
    ...
    const data = _this.CacheNotLoadControll;
    _this.CacheNotLoadControll = [];
  ...
  execCache: function(item) {
    ...
    if (type == 'sha') {
      ...
      const copyjson = $.extend(true, {}, Store.luckysheetfile[_this.getSheetIndex(value.copyindex)]);
      copyjson.index = index;
    ...
    if (type == 'v') {
      const r = item.r,
        c = item.c,
        v = item.v;
      const data = _this.getSheetData(index);
      file.data[r][c] = v;
    } else if (type == 'fc') {
      const op = item.op,
        pos = item.pos;
      ...
      const v = value,
        k = item.k;
      const config1 = _this.getSheetConfig(index);
      ...
      if (op == 'add') {
        ...
        for (let i = 0; i < file.chart.length; i++) {
          if (file.chart[i].chart_id == cid) {
            for (const item in file.chart[i]) {
              for (const vitem in value) {
                if (item == vitem) {
