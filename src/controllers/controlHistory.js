...
import {zoomNumberDomBind, zoomRefreshView} from './zoom';
...
function formulaHistoryHanddler(ctr, type='redo'){
  ...
  for(let s = 0; s < ctr.range.length; s++){
    const st_r = ctr.range[s].row[0];
    const ed_r = ctr.range[s].row[1];
    const st_c = ctr.range[s].column[0];
    const ed_c = ctr.range[s].column[1];
...
const controlHistory = {
  redo: function (e) {
    ...
    if (ctr.type == 'datachange') {
      ...
      const images = $.extend(true, {}, ctr.images);
      ...
      const ctrlValue1 = $.extend(true, {}, ctr.ctrlValue);
      ...
      const ctrlValue = $.extend(true, {}, ctr.ctrlValue);
      ...
      const $top = $(`#luckysheet-filter-options-sheet${  Store.currentSheetIndex  } .luckysheet-filter-options`).eq(ctr['optionsindex']);
      const st_r = $top.data('str'),
        ed_r = $top.data('edr'),
      ...
      const luckysheetcurrentSheetitem = $(`#luckysheet-sheets-item${  ctr.sheetIndex}`);
      ...
      const historyRules = ctr['data']['historyRules'];

      for(let i = 0; i < historyRules.length; i++){
        //条件规则
        const sheetIndex = historyRules[i]['sheetIndex'];
      ...
      const index = getSheetIndex(ctr['sheetIndex']);
      ...
      for(let i = 0; i < ctr.rc.length; i++){
        const r = ctr.rc[i].split('_')[0];
        const c = ctr.rc[i].split('_')[1];
    ...
    const newCtr = {...ctr, ...{data: ctr.curdata, curdata: ctr.data}};
    // 钩子函数
  ...
  undo: function () {
    ...
    if (ctr.type == 'datachange') {
      ...
      const images = $.extend(true, {}, ctr.curImages);
      Store.luckysheetfile[getSheetIndex(ctr.sheetIndex)].images = images;
      server.saveParam('all', ctr.sheetIndex, images, { 'k': 'images' });
      imageCtrl.images = images;
      imageCtrl.allImagesShow();
      ...
      const currentRules = ctr['data']['currentRules'];

      for(let i = 0; i < currentRules.length; i++){
        //条件规则
        const sheetIndex = currentRules[i]['sheetIndex'];
