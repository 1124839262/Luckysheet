...
const luckysheetMoreFormat = {
  ...
  createDialog: function(type){
    ...
    if(type == 'morecurrency'){ //货币
      ...
      for(let i = 0; i < _this.moneyFmtList.length; i++){
        const name = _this.moneyFmtList[i]['name'];
        const pos = _this.moneyFmtList[i]['pos'];
        const value = _this.moneyFmtList[i]['value'];
      ...
      for(let i = 0; i < _this.dateFmtList.length; i++){
        const name = _this.dateFmtList[i]['name'];
        const value = _this.dateFmtList[i]['value'];
      ...
      for(let i = 0; i < _this.numFmtList.length; i++){
        const name = _this.numFmtList[i]['name'];
        const value = _this.numFmtList[i]['value'];
    ...
    const $t = $('#luckysheet-moreFormat-dialog').find('.luckysheet-modal-dialog-content').css('min-width', 400).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
  ...
  init: function(){
    ...
    $(document).off('click.moreFormatConfirm').on('click.moreFormatConfirm', '#luckysheet-moreFormat-dialog #luckysheet-moreFormat-dialog-confirm', function(){
      ...
      const value = $('#luckysheet-moreFormat-dialog .listbox .listItem.on .value').text();
      const id = $(this).parents('#luckysheet-moreFormat-dialog').find('.box').attr('id');
      ...
      if(id == 'morecurrency'){ //货币
        ...
        const decimal = parseInt($('#luckysheet-moreFormat-dialog .decimal input').val().trim());
        ...
        const pos = $('#luckysheet-moreFormat-dialog .listbox .listItem.on input:hidden').val();
