...
function luckysheetMoveEndCell(postion, type, isScroll, terminal, onlyvalue) {
  ...
  const datarowlen = Store.flowdata.length,
...
function luckysheetMoveHighlightCell(postion, index, type, isScroll) {
  ...
  if(type == 'rangeOfSelect'){
    ...
    //focus单元格 是否是合并单元格
    const margeset = menuButton.mergeborer(Store.flowdata, curR, curC);
    if(margeset){
    ...
    //移动的下一个单元格是否是合并的单元格
    const margeset2 = menuButton.mergeborer(Store.flowdata, curR, curC);
    if(margeset2){
      row = margeset2.row[1];
  ...
  const winH = $('#luckysheet-cell-main').height(), winW = $('#luckysheet-cell-main').width();
...
function luckysheetMoveHighlightCell2(postion, type, isScroll) {
  ...
  if(type == 'rangeOfSelect'){
    ...
    const changeparam = menuButton.mergeMoveMain(columnseleted, rowseleted, last, row_pre, row - row_pre - 1, col_pre, col - col_pre - 1);
    if(changeparam != null){
    ...
    const changeparam = menuButton.mergeMoveMain(columnseleted, rowseleted, last, top, height, left, width);
    if(changeparam != null){
      columnseleted = changeparam[0];
      rowseleted= changeparam[1];
      top = changeparam[2];
...
function luckysheetMoveHighlightRange(postion, index, type, isScroll) {
  ...
  if(type == 'rangeOfSelect'){
    ...
    const rf = last['row_focus'], cf = last['column_focus'];
    ...
    if(postion == 'down'){ //选区上下变动
      if(rowHasMerge(rf, curC, endC)){ //focus单元格所在行有合并单元格
        const rfMerge = getRowMerge(rf, curC, endC);
        const rf_str = rfMerge[0], rf_end = rfMerge[1];
      ...
      if(colHasMerge(cf, curR, endR)){ //focus单元格所在列有合并单元格
        const cfMerge = getColMerge(cf, curR, endR);
        const cf_str = cfMerge[0], cf_end = cfMerge[1];
  ...
  if(type == 'rangeOfSelect'){
    ...
    if(postion == 'down'){
      ...
      if(rowHasMerge(rf, c1, c2)){ //focus所在行有合并单元格
        const rfMerge = getRowMerge(rf, c1, c2);
        const rf_str = rfMerge[0], rf_end = rfMerge[1];
      ...
      if(colHasMerge(cf, r1, r2)){ //focus所在行有合并单元格
        const cfMerge = getColMerge(cf, r1, r2);
        const cf_str = cfMerge[0], cf_end = cfMerge[1];
...
function getRowMerge(rIndex, c1, c2){
  const r1 = 0, r2 = Store.flowdata.length - 1;
  ...
  if(rIndex > r1){
    for(let r = rIndex; r >= r1; r--){
      for(let c = c1; c <= c2; c++){
        ...
        if(getObjType(cell) == 'object' && ('mc' in cell)){
          const mc = Store.config['merge'][`${cell['mc'].r  }_${  cell['mc'].c}`];
...
function getColMerge(cIndex, r1, r2){
  const c1 = 0, c2 = Store.flowdata[0].length - 1;
...
function getNextIndex(direction, focusIndex, strIndex, endIndex) {
  ...
  let stNull;
  if(direction == 'down'){
    const stValue = Store.flowdata[strIndex][focusIndex];
    ...
    for(let r = strIndex + 1; r <= endIndex; r++){
      const cell = Store.flowdata[r][focusIndex];
    ...
    const stValue = Store.flowdata[endIndex][focusIndex];
    ...
    const stValue = Store.flowdata[focusIndex][strIndex];
    ...
    for(let c = strIndex + 1; c <= endIndex; c++){
      const cell = Store.flowdata[focusIndex][c];
    ...
    const stValue = Store.flowdata[focusIndex][endIndex];
