...
import {openSelfModel, replaceHtml, transformRangeToAbsolute} from '../utils/util';
...
function addRangeItem(item){
  ...
  const title = item.name, sqref = item.sqref, password = item.password;
...
export function initialEvent(file){
  ...
  $('#luckysheet-slider-protection-ok').unbind('click').click(function () {
    const password = $('#protection-password').val();
    const sheet = $('#protection-swichProtectionState').is(':checked');
    const hint = $('#protection-hint').val();
    ...
    for(let i=0;i<authorityItemArr.length;i++){
      const name = authorityItemArr[i];
      const checkId = `luckysheet-protection-check-${  name}`;
      const authorityValue =  $(`#${checkId}`).is(':checked');

      authorityData[name] = authorityValue==true?1:0;
    }

    const allowRangeListTemp = JSON.parse(JSON.stringify(rangeItemListCache));
  ...
  $(document).off('click.luckysheetProtection.rangeItemUpdate').on('click.luckysheetProtection.rangeItemUpdate','#luckysheet-protection-rangeItem-container .luckysheet-protection-rangeItem-update', function(e){
    ...
    const $rangeItem = $(e.target).closest('.luckysheet-protection-rangeItem');

    const $rangeItemContainer =  $('#luckysheet-protection-rangeItem-container');

    const index = $rangeItemContainer.find('> div.luckysheet-protection-rangeItem').index($rangeItem);

    const item = rangeItemListCache[index];
  ...
  $(document).off('click.luckysheetProtection.rangeItemConfirm').on('click.luckysheetProtection.rangeItemConfirm','#luckysheet-protection-rangeItem-confirm', function(){
    ...
    const range = dataVerificationCtrl.getRangeByTxt(rangeText);
    ...
    if(isAddRangeItemState){
      ...
      const index = updateRangeItemIndex;
      const item = rangeItemListCache[index];
      ...
      const $rangeitem = $rangeItemContainer.find('> div.luckysheet-protection-rangeItem').eq(index);

      const $name = $rangeitem.find('.luckysheet-protection-rangeItem-name');
      ...
      const $range = $rangeitem.find('.luckysheet-protection-rangeItem-range');
  ...
  $(document).off('click.luckysheetProtection.validationConfirm').on('click.luckysheetProtection.validationConfirm','#luckysheet-protection-sheet-validation-confirm', function(e){
    const $validation = $('#luckysheet-protection-sheet-validation');
    const aut = validationAuthority;
    ...
    if(aut.algorithmName!=null && aut.algorithmName!='None'){
      if(aut.saltValue!=null && aut.saltValue.length>0){
        const hasher = CryptoApi.getHasher(aut.algorithmName);
  ...
  $('#luckysheet-protection-check-selectLockedCells').change(function() {
    const $selectLockedCells = $('#luckysheet-protection-check-selectLockedCells'), $selectunLockedCells = $('#luckysheet-protection-check-selectunLockedCells');

    const selectLockedCellsChecked = $selectLockedCells.is(':checked'), selectunLockedCellsChecked = $selectunLockedCells.is(':checked');
  ...
  $(document).off('click.luckysheetProtection.dvRange').on('click.luckysheetProtection.dvRange', '#protection-allowRangeAdd-range .fa-table', function(e) {
    ...
    const range = dataVerificationCtrl.getRangeByTxt(txt);
    if(range.length > 0){
  ...
  $(document).off('click.luckysheetProtection.dvRangeConfirm').on('click.luckysheetProtection.dvRangeConfirm', '#luckysheet-dataVerificationRange-dialog-confirm', function(e) {
    const txt = $(this).parents('#luckysheet-dataVerificationRange-dialog').find('input').val();

    const $input = $('#protection-allowRangeAdd-range input'), inputValue = $input.val();
...
export function openProtectionModal(file){
  ...
  if(file!=null && file.config!=null && file.config.authority!=null){
    const aut = file.config.authority;
...
function checkProtectionLockedSqref(r, c, aut, local_protection, isOpenAlert=true, isLock=true){
  let isPass = false;
  const rangeAut = aut.allowRangeList;
  if(rangeAut!=null && rangeAut.length>0){
    let isExists = false;
    for(let i=0;i<rangeAut.length;i++){
      const ra = rangeAut[i];
      const sqref = ra.sqref;
      const range = dataVerificationCtrl.getRangeByTxt(sqref);
...
function openRangePasswordModal(rangeAut) {
  ...
  const $hint = $('#luckysheet-protection-range-validation-hint');
  ...
  const $rangeV = $('#luckysheet-protection-range-validation');
  const $input = $rangeV.find('input');
  $input.val('');
  ...
  $('#luckysheet-protection-range-validation-confirm').off('click').on('click', function(){
    ...
    if(rangeAut.algorithmName!=null && rangeAut.algorithmName!='None'){
      ...
      if(rangeAut.saltValue!=null && rangeAut.saltValue.length>0){
        const hasher = CryptoApi.getHasher(rangeAut.algorithmName);
...
export function checkProtectionNotEnable(sheetIndex){
  const sheetFile = sheetmanage.getSheetByIndex(sheetIndex);
  if(sheetFile==null){
    return true;
  }
...
export function checkProtectionLocked(r, c, sheetIndex, isOpenAlert=true, isLock=true){
  ...
  const data=sheetFile.data, cell=data[r][c], aut = sheetFile.config.authority;
...
export function checkProtectionLockedRangeList(rangeList, sheetIndex){
  ...
  for(let s = 0; s < rangeList.length; s++){
    const r1 = rangeList[s].row[0], r2 = rangeList[s].row[1];
    const c1 = rangeList[s].column[0], c2 = rangeList[s].column[1];

    for(let r=r1;r<=r2;r++){
      for(let c=c1;c<=c2;c++){
        const cell = sheetFile.data[r][c] || {};
        const isLock = cell.lo === undefined || cell.lo === 1, // 单元格是否锁定
...
export function checkProtectionSelectLockedOrUnLockedCells(r, c, sheetIndex){
  ...
  if(cell && cell.lo === 0){ // lo为0的时候才是可编辑
    ...
    const isAllEdit = checkProtectionLockedSqref(r, c , aut, local_protection, false);//dont alert password model
