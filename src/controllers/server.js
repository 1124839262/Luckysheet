...
const server = {
  ...
  historyParam: function(data, sheetIndex, range) {
	    ...
	    if(r1 == r2 && c1 == c2){ //单个单元格更新
	        ...
	        const timeR = Math.floor(1000 / collen);
	        const n = Math.ceil(rowlen / timeR); //分批次更新，一次最多1000个单元格

	        for(let i = 0; i < n; i++){
	            const str = r1 + timeR * i;
  ...
  saveParam: function (type, index, value, params) {
    ...
    if (JSON.stringify(customImageUpdateMethodConfig) !== '{}') {
      if ('images' != d.k) {
        const msg = pako.gzip(encodeURIComponent(JSON.stringify(d)), {to: 'string'});
    ...
    if('WebSocket' in window){
      ...
      if(_this.updateUrl.indexOf('?') > -1){
        ...
        if(message === '用户退出') {
          ...
          const chang_data = JSON.parse(data.data);
          ...
          const flag = Store.cooperativeEdit.changeCollaborationSize.some(value1 => {
            return value1.id == id;
          });
          if(flag) {
          ...
          const checkoutFlag = Store.cooperativeEdit.checkoutData.some(item => {
            return item.id == id;
          });
          if(checkoutFlag) {
          ...
          if($(`#luckysheet-multipleRange-show-${  id}`)[0]) {
            const change_bottom = $(`#luckysheet-multipleRange-show-${  id}`)[0].offsetHeight - 1;
          ...
          const items = data.data === '' ?  data.data : JSON.parse(data.data);
  ...
  wsUpdateMsg: function(item) {
      ...
      if(Object.keys(item.range).length > 0) {
        ...
        collaborativeEditBox();
	        ...
	        const r1 = item.range.row[0], r2 = item.range.row[1];
	        const c1 = item.range.column[0], c2 = item.range.column[1];
        ...
        if(value && (typeof value === 'object')){
          file['config'][k] = value;
	        ...
	        if(k == 'name'){ //工作表名
	            ...
	            const currentSheetItem = $(`#luckysheet-sheet-container-c #luckysheet-sheets-item${  index}`);
        ...
        if(index == Store.currentSheetIndex){
          hyperlinkCtrl.hyperlink = value;
	        ...
	        const op = item.op, pos = item.pos;

	        if(getObjType(value) != 'object'){
        value = new Function(`return ${  value}`)();
	        }

	        const r = value.r, c = value.c;

	        const calcChain = file['calcChain'] == null ? [] : file['calcChain'];
        ...
        direction = value.direction,
	        	mc = value.mc,
	        	borderInfo = value.borderInfo;
	        const data = $.extend(true, [], file.data);
      ...
      sheetmanage.locationSheet();
	        ...
	        const copyindex = value.copyindex, name = value.name;

	        const copyarrindex = getSheetIndex(copyindex);
      ...
      sheetmanage.locationSheet();
	        ...
	        for(let i = 0; i < server.sheetDeleSave.length; i++){
	            if(server.sheetDeleSave[i].index == value.reIndex){
	                const datav = server.sheetDeleSave[i];
      ...
      sheetmanage.locationSheet();
	        ...
	        if(op == 'add'){ //插入
	            ...
	            for(let i = 0; i < file.chart.length; i++){
	                const chartjson = file.chart[i];

	                if(chartjson.chart_id == cid){
	                    for(const item in chartjson){
  ...
  request: function () {
    ...
    _this.cachelocaldata(function(cahce_key, params){
      ...
      const compressBeginLen = params.length;
      const iscommpress = false;
      ...
      if(_this.updateUrl != ''){
        $.post(_this.updateUrl, { compress: iscommpress, gridKey: _this.gridKey, data: params }, function (data) {
          const re = new Function(`return ${  data}`)();
          if(re.status){
  ...
  imageRequest: function(){
    ...
    html2canvas($(`#${  container}`).find('.luckysheet-grid-window').get(0), {
      onrendered: function(canvas) {
        ...
        const old = $(canvas).appendTo('body');
        old.hide();
        const newwidth = old.width();
        const newheight = old.height();
        const imageData = old.get(0).getContext('2d').getImageData(0, 0, newwidth, newheight);
        ...
        const newCanvas = $('<canvas>').attr('width', cutW).attr('height', cutH)[0];
        ...
        const base64 = old.get(0).toDataURL('image/jpeg', 0.9);

        //console.log(base64);
        //console.log("压缩：", pako.gzip(base64, { to: "string" }));
        //console.log("imageRequest");
        const curindex = luckysheet.sheetmanage.getCurSheetnoset();
        ...
        const data1 = encodeURIComponent(JSON.stringify({'t':'thumb', 'img': base64, 'curindex':curindex }));
        old.remove();
    ...
    if(value instanceof Array){
      for(let i = 0; i < value.length; i++){
        ...
        for(let a = 0; a < d.length; a++){
          const ditem = data[i]; //let ditem = data[a];?
  ...
  restorecachelocaldata: function(func){
    ...
    localforage.getItem(cahce_key).then(function(readValue) {
      const updatedata = readValue;
      _this.getlocaldata(function(data){
        if(data==null){
          data = [];
        }
        const newdata = updatedata.concat(data);
