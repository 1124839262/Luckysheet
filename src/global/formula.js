...
const luckysheetformula = {
  ...
  sparklinesColorMap: function(args, len) {
    ...
    if (args.length > len) {
      for (let i = len; i < args.length; i++) {
        const colorMap = args[i];
        const colorListArray = _this.readCellDataToOneArray(colorMap);

        for (let a = 0; a < colorListArray.length; a++) {
          const ca = colorListArray[a];
  ...
  cellOffset: function(range, rows, cols, height, width) {
    ...
    const rowl = range.rowl;
    const coll = range.coll;
    const startCellRow = parseInt(startCell.replace(/[^0-9]/g, ''));
    const startCellCol = ABCatNum(startCell.replace(/[^A-Za-z]/g, ''));
    ...
    const cellF = col[0] + row[0];
    const cellL = col[1] + row[1];
  ...
  isCompareOperator: function(str) {
    ...
    const strOne = str.substr(0, 1);
    const strTwo = str.substr(1, 1);
    let flag = false;
    let ret;
  ...
  acompareb: function(a, b) {
    ...
    if (isRealNum(b)) {
      ...
      if (b.indexOf('*') != -1 || b.indexOf('?') != -1) {
        ...
        //"黑糖"
        const ope = _this.isCompareOperator(b).ope;
        const num = _this.isCompareOperator(b).num;
  ...
  compareParams: function(fp, sp, sym) {
    ...
    //判断a和b的数据类型
    const classNameA = toString.call(fp),
    ...
    if (classNameA == '[object Object]' && classNameB == '[object Object]') {
      //获取a和b的属性长度
      const propsA = Object.getOwnPropertyNames(fp),
  ...
  createRangeHightlight: function() {
    ...
    const $span = $('#luckysheet-rich-text-editor').find('span.luckysheet-formula-functionrange-cell');
  ...
  getrangeseleciton: function() {
    ...
    if (anchor.parent().is('span') && anchorOffset != 0) {
      ...
      if (anchor.prev().length > 0) {
        const txt = $.trim(anchor.prev().text());
        const lasttxt = txt.substr(txt.length - 1, 1);
        return anchor.prev();
      }
    }
  ...
  searchFunction: function($editer) {
    const _this = this;
    const functionlist = Store.functionlist;
    ...
    const inputContent = $editer.text();
    const searchtxt = $cell.text().toUpperCase();
  ...
  helpFunction: function($editer, funcname, paramIndex) {
    ...
    const $func = functionlist[_this.functionlistPosition[$.trim(funcname).toUpperCase()]];
    if ($func == null) {
      return;
    }
    ...
    const helpformulaItem = '<span class="luckysheet-arguments-help-parameter" dir="auto">${param}</span>';
    const helpformulaArg =
    ...
    if (paramIndex == null) {
      ...
      const index = paramIndex >= $func.p.length ? $func.p.length - 1 : paramIndex;
      $(
  ...
  helpFunctionExe: function($editer, currSelection) {
    ...
    if ($('#luckysheet-formula-help-c').length == 0) {
      ...
      $('#luckysheet-formula-help-c .luckysheet-formula-help-collapse').click(function() {
        const $content = $('#luckysheet-formula-help-c .luckysheet-formula-help-content');
        $content.slideToggle(100, function() {
          const $c = _this.rangeResizeTo.parent(),
            offset = $c.offset();
          _this.searchFunctionPosition($('#luckysheet-formula-help-c'), $c, offset.left, offset.top, true);
        });
  ...
  rangeHightlightselected: function($editer, kcode) {
    ...
    const currSelection = _this.getrangeseleciton();
    ...
    const $anchorOffset = $(currSelection).closest('.luckysheet-formula-functionrange-cell');
  ...
  updatecell: function(r, c, value, isRefresh = true) {
    ...
    const $input = $('#luckysheet-rich-text-editor');
    ...
    const cfg = $.extend(true, {}, getluckysheetfile()[getSheetIndex(Store.currentSheetIndex)]['config']);
    if (cfg['rowlen'] == null) {
      cfg['rowlen'] = {};
    }
    ...
    if (
      ...
      //自动换行
      const defaultrowlen = Store.defaultrowlen;

      const canvas = $('#luckysheetTableContent')
        .get(0)
        .getContext('2d');
      ...
      if (cfg['customHeight'] && cfg['customHeight'][r] == 1) {
        ...
        const cellWidth = colLocationByIndex(c)[1] - colLocationByIndex(c)[0] - 2;

        const textInfo = getCellTextInfo(d[r][c], canvas, {
          r: r,
          c: c,
          cellWidth: cellWidth,
        });
  ...
  iscellformat: function(txt) {
    const re_abc = /[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ][123456789]/;
  },
  ...
  setfreezonFuceExe: function(rangetxt) {
    const row = parseInt(rangetxt.replace(/[^0-9]/g, ''));
    const col = ABCatNum(rangetxt.replace(/[^A-Za-z]/g, ''));
    const $row = '$',
      $col = '$';
  ...
  setfreezonFuc: function(event) {
    ...
    const obj = _this.getrangeseleciton();
    ...
    let newtxt = '',
      newpos = '';
    const rangetxtIndex = rangetxt.indexOf(':');
    ...
    if (rangetxtIndex == -1) {
      ...
      if (pos > rangetxtIndex) {
        const ret = `${prefix + rangetxt[0]  }:${  _this.setfreezonFuceExe(rangetxt[1])}`;
        newtxt = ret;
        newpos = ret.length;
      } else {
        const firsttxt = prefix + _this.setfreezonFuceExe(rangetxt[0]);
        const ret = `${firsttxt  }:${  rangetxt[1]}`;
        newtxt = ret;
  ...
  updateparam: function(orient, txt, step) {
    ...
    if (rangetxt.indexOf(':') == -1) {
      ...
      const freezonFuc = _this.isfreezonFuc(rangetxt);
      ...
      const freezonFuc0 = _this.isfreezonFuc(rangetxt[0]);
      const freezonFuc1 = _this.isfreezonFuc(rangetxt[1]);
  ...
  rangeSetValue: function(selected, obj) {
    ...
    if (_this.rangestart || _this.rangedrag_column_start || _this.rangedrag_row_start) {
      if (
        ...
        const function_str = _this.functionHTMLGenerate(functionHtmlTxt);
        $('#luckysheet-rich-text-editor').html(function_str);
        $('#luckysheet-functionbox-cell').html($('#luckysheet-rich-text-editor').html());

        if (isVal) {
          //公式计算
          const fp = $.trim(_this.functionParserExe($('#luckysheet-rich-text-editor').text()));
        ...
        const $span = $editor.find(`span[rangeindex='${  _this.rangechangeindex  }']`).html(range);
      ...
      const $t = $(function_str).insertAfter(_this.rangeSetValueTo);
  ...
  rangeResizeDraging: function(
    ...
    const topchange = row_pre - luckysheetCurrentChartResizeXy[1],
    ...
    const rangeindex = _this.rangeResizeIndex;
    const selected = {
      top: top,
      left: left,
      height: height,
    ...
    const range = _this.getSelectedFromRange(selected);
    const rangetxt = getRangetxt(Store.currentSheetIndex, range, _this.rangetosheet);
    const $span = _this.rangeResizeTo.find(`span[rangeindex='${  rangeindex  }']`).html(rangetxt);
  ...
  getSelectedFromRange: function(obj) {
    const row_st = obj.top + 2,
      row_ed = obj.top + obj.height - 2;
    const col_st = obj.left + 2,
  ...
  rangeMoveDraging: function(
    ...
    const winH = $(window).height() + scrollTop - sheetBarHeight - statisticBarHeight,
      winW = $(window).width() + scrollLeft;

    const row_index_original = luckysheet_cell_selected_move_index[0],
    ...
    const col_pre = col_s - 1 == -1 ? 0 : visibledatacolumn[col_s - 1],
      col = visibledatacolumn[col_e];
    const row_pre = row_s - 1 == -1 ? 0 : visibledatarow[row_s - 1],
      row = visibledatarow[row_e];
    const rangeindex = _this.rangeMoveIndex;
    const selected = {
      left: col_pre,
      width: col - col_pre - 2,
      top: row_pre,
      ...
      if (value.length > 0 && value.substr(0, 1) == '=' && (kcode != 229 || value.length == 1)) {
        ...
        if (window.getSelection) {
          ...
          if ($(currSelection.anchorNode).is('div')) {
            const editorlen = $('#luckysheet-rich-text-editor span').length;
  ...
  functionParser: function(txt, cellRangeFunction) {
    ...
    while (i < funcstack.length) {
      ...
      if (s == '(' && matchConfig.squote == 0 && matchConfig.dquote == 0 && matchConfig.braces == 0) {
        ...
        let s_next = '';
        const op = _this.operatorPriority;
  ...
  insertUpdateDynamicArray: function(dynamicArrayItem) {
    ...
    const luckysheetfile = getluckysheetfile();
    const file = luckysheetfile[getSheetIndex(index)];
  ...
  updateFunctionGroup: function(r, c, index) {
    ...
    const calcChain = file.calcChain;
    if (calcChain != null) {
  ...
  isFunctionRangeSelect: function(txt, r, c, index, dynamicArray_compute) {
    ...
    const _this = this;
    const txt1 = txt.toUpperCase();
    const isOffsetFunc =
            txt1.indexOf('INDIRECT(') > -1 || txt1.indexOf('OFFSET(') > -1 || txt1.indexOf('INDEX(') > -1;
    if (txt in this.formulaContainCellList) {
      const cellList = this.formulaContainCellList[txt];
      if (isOffsetFunc) {
        const isoff = cellList['__LuckyisOff__'];
        if (isoff == true) {
          for (const cellStr in cellList) {
  ...
  isFunctionRangeSaveChange: function(str, r, c, index, dynamicArray_compute) {
    const _this = this;
    if (r != null && c != null) {
      const range = _this.getcellrange($.trim(str), index);
      if (range == null) {
        return;
      }
      const row = range.row,
  ...
  execFunctionGroup: function(origin_r, origin_c, value, index, data, isForce = false) {
    ...
    const calcChains = _this.getAllFunctionGroup(),
      formulaObjects = {};

    const sheets = getluckysheetfile();
    const sheetData = {};
    ...
    if (_this.execFunctionExist == null) {
      ...
      for (let x = 0; x < _this.execFunctionExist.length; x++) {
        const cell = _this.execFunctionExist[x];
    ...
    const arrayMatch = function(formulaArray, formulaObjects, updateValueOjects, func) {
    ...
    for (let i = 0; i < calcChains.length; i++) {
      ...
      const calc_funcStr = getcellFormula(formulaCell.r, formulaCell.c, formulaCell.index);
      ...
      if (isOffsetFunc) {
        this.isFunctionRange(calc_funcStr, null, null, formulaCell.index, null, function(str_nb) {
          const range = _this.getcellrange($.trim(str_nb), formulaCell.index);
          if (range != null) {
        ...
        for (let i = 0; i < formulaTextArray.length; i++) {
          ...
          const range = _this.getcellrange($.trim(t), formulaCell.index);
    ...
    for (let i = 0; i < formulaRunList.length; i++) {
      ...
      window.luckysheet_getcelldata_cache = null;
      const calc_funcStr = formulaCell.calc_funcStr;

      const v = _this.execfunction(calc_funcStr, formulaCell.r, formulaCell.c, formulaCell.index);
  ...
  execFunctionGroup1: function(origin_r, origin_c, value, index, data, isForce = false) {
    ...
    if (_this.execFunctionExist == null) {
      for (let i = 0; i < group.length; i++) {
        let item = group[i];
        const file = luckysheetfile[getSheetIndex(item['index'])];
        if (file == null) {
          continue;
        }
        const cell = file.data[item.r][item.c];
        const calc_funcStr = getcellFormula(item.r, item.c, item.index);
    ...
    while (stack.length > 0) {
      ...
      for (const name in vertex1) {
        const item = vertex1[name];
        if (item == null) {
          continue;
        }

        const ukey = `r${  u.r  }c${  u.c  }i${  u.index}`;
    ...
    while (i < count) {
      for (const name in _this.execvertex) {
        const u = _this.execvertex[name];
  ...
  functionDFS: function(u) {
    ...
    u.color = 'g';
    u.times += 1;
    for (const chd in u.chidren) {
      const v = _this.execvertex[chd];
      if (v.color == 'w') {
    ...
    u.color = 'b';
    window.luckysheet_getcelldata_cache = null;
    const calc_funcStr = getcellFormula(u.r, u.c, u.index);

    const v = _this.execfunction(calc_funcStr, u.r, u.c, u.index);
  ...
  groupValuesRefresh: function() {
    ...
    if (_this.groupValuesRefreshData.length > 0) {
      for (let i = 0; i < _this.groupValuesRefreshData.length; i++) {
        const item = _this.groupValuesRefreshData[i];
  ...
  execfunction: function(txt, r, c, index, isrefresh, notInsertFunc) {
    ...
    const fp = $.trim(_this.functionParserExe(txt));
    //console.log(fp)
    ...
    try {
      if (fp.indexOf('luckysheet_getcelldata') > -1) {
        ...
        for (let i = 1; i < funcg.length; i++) {
          ...
          const funcgRange = _this.getcellrange(funcgStr);
  ...
  addButtonListener:function(txt, r, c){
    const listener =  $('#luckysheet-formula-refresh').data('listener');
