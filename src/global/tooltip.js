import {luckysheetchartpointconfigHTML, luckysheetToolHTML, modelHTML} from '../controllers/constant';
...
const tooltip = {
  info: function (title, content) {
    ...
    const $t = $('#luckysheet-info').find('.luckysheet-modal-dialog-content').css('min-width', 300).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
  ...
  confirm: function (title, content, func1, func2, name1, name2) {
    ...
    const $t = $('#luckysheet-confirm').find('.luckysheet-modal-dialog-content').css('min-width', 300).end(),
      myh = $t.outerHeight(),
      myw = $t.outerWidth();
    const winw = $(window).width(), winh = $(window).height();
    const scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
  ...
  createHoverTip: function (obj, to) {
    ...
    $(obj).on('mouseover', to, function (e) {
      ...
      _this.hoverTipshowTimeOut = setTimeout(function(){
        ...
        const toolwidth = $toolup.outerWidth();
  ...
  popover: function(content, position, close, style, btntxt, exitsFuc){
    ...
    const htmldiv = `<div id="luckysheetpopover" class="luckysheetpopover"><div class="luckysheetpopover-content">${locale_paint.start}</div><div class="luckysheetpopover-btn">${ btntxt }</div></div>`;
    $('#luckysheetpopover').remove();
    $('body').append(htmldiv);
    $('#luckysheetpopover .luckysheetpopover-content').html(content);

    const w = $('#luckysheetpopover').outerWidth(),
