...
function setcellvalue(r, c, d, v) {
  ...
  if (!server.allowUpdate && !luckysheetConfigsetting.pointEdit) {
    if (
      ...
      const autoFormatw = luckysheetConfigsetting.autoFormatw.toString().toUpperCase();
      const accuracy = luckysheetConfigsetting.accuracy;
...
function setAccuracy(autoFormatw, accuracy) {
  ...
  if (autoFormatw == 'TRUE') {
    if (accuracy == undefined) {
      ...
      const alength = parseInt(accuracy);
