...
import {luckysheetDrawgridColumnTitle, luckysheetDrawgridRowTitle, luckysheetDrawMain} from './draw';
...
import {collaborativeEditBox, selectHightlightShow, selectionCopyShow} from '../controllers/select';
...
function jfrefreshgrid(data, range, allParam, isRunExecFunction = true, isRefreshCanvas = true) {
  ...
  const cfg = allParam['cfg'];  //config
  const calc = allParam['calc'];
  const RowlChange = allParam['RowlChange'];  //行高改变
  const cdformat = allParam['cdformat'];  //条件格式
  const dataVerification = allParam['dataVerification'];  //数据验证
  const dynamicArray = allParam['dynamicArray'];  //动态数组
  const hyperlink = allParam['hyperlink'];
  ...
  for(let s = 0; s < range.length; s++){
    ...
    if(Store.flowdata[r1][c1] != null && Store.flowdata[r1][c1].spl != null){
      ...
      const fp = $.trim(formula.functionParserExe(Store.flowdata[r1][c1].f));
      const sparklines = new Function(`return ${ fp}`)();
...
function jfrefreshgrid_adRC(data, cfg, ctrlType, ctrlValue, calc, filterObj, cf, af, freezen, dataVerification, hyperlink){
  ...
  //merge改变对应的单元格值改变
  const mcData = [];
  for(const m in cfg['merge']){
    const mc = cfg['merge'][m];
  ...
  const index = ctrlValue.index,
    len = ctrlValue.len,
    rc = ctrlValue.rc;

  if(ctrlType == 'addRC'){
    const direction = ctrlValue.direction,
  ...
  if(calc.length > 0){
    ...
    for(let i = 0; i < calc.length; i++){
      const clc = calc[i];
      const clc_r = clc.r, clc_c = clc.c, clc_i = clc.index, clc_funcStr =  getcellFormula(clc_r, clc_c, clc_i, data);

      const clc_result = formula.execfunction(clc_funcStr, clc_r, clc_c, clc_i,null, true);
      clc.func = clc_result;
...
function jfrefreshgrid_pastcut(source, target, RowlChange){
  ...
  if(RowlChange){
    ...
    if(Store.currentSheetIndex == source['sheetIndex']){
      const rowlenArr = computeRowlenArr(target['curData'].length, target['curConfig']);
      Store.luckysheetfile[getSheetIndex(target['sheetIndex'])]['visibledatarow'] = rowlenArr;
    }
    else if(Store.currentSheetIndex == target['sheetIndex']){
      const rowlenArr = computeRowlenArr(source['curData'].length, source['curConfig']);
...
function jfrefreshgrid_rhcw(rowheight, colwidth, isRefreshCanvas=true){
  ...
  const calcChain = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)].calcChain;
  ...
  if(calcChain != null && calcChain.length > 0){
    ...
    for(let i = 0; i < calcChain.length; i++){
      const r = calcChain[i].r, c = calcChain[i].c, index = calcChain[i].index;
      ...
      if(index == Store.currentSheetIndex && Store.flowdata[r][c] != null && Store.flowdata[r][c].spl != null && ((r in Store.config['rowlen']) || (c in Store.config['columnlen']))){
        ...
        const fp = $.trim(formula.functionParserExe(Store.flowdata[r][c].f));
        const sparklines = new Function(`return ${  fp}`)();
  ...
  if(luckysheetFreezen.freezenhorizontaldata != null && luckysheetFreezen.freezenverticaldata != null){
    const row_st = luckysheetFreezen.freezenhorizontaldata[1] - 1;
    const col_st = luckysheetFreezen.freezenverticaldata[1] - 1;

    const scrollTop = luckysheetFreezen.freezenhorizontaldata[2];
    const scrollLeft = luckysheetFreezen.freezenverticaldata[2];
    ...
    if($(`#luckysheet-filter-options-sheet${  Store.currentSheetIndex  } .luckysheet-filter-options`).length > 0){
      $(`#luckysheet-filter-options-sheet${  Store.currentSheetIndex  } .luckysheet-filter-options`).each(function(i, e){
        const str = $(e).data('str'), cindex = $(e).data('cindex');

        const left = Store.visibledatacolumn[cindex] - 20;
        const top = str - 1 == -1 ? 0 : Store.visibledatarow[str - 1];
  ...
  if($(`#luckysheet-filter-selected-sheet${  Store.currentSheetIndex}`).length > 0){
    const luckysheet_filter_save = Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)].filter_select;
...
function luckysheetrefreshgrid(scrollWidth, scrollHeight) {
  ...
  if (luckysheetFreezen.freezenverticaldata != null || luckysheetFreezen.freezenhorizontaldata != null) {
    ...
    const drawWidth = Store.luckysheetTableContentHW[0], drawHeight = Store.luckysheetTableContentHW[1];
