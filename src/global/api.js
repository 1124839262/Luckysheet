...
export function getCellValue(row, column, options = {}) {
  ...
  const curSheetOrder = getSheetIndex(Store.currentSheetIndex);
  const {
    type = 'v',
    order = curSheetOrder
  } = { ...options };
  const targetSheetData = Store.luckysheetfile[order].data;
  ...
  if(getObjType(cellData) == 'object'){
    ...
    if (type == 'f' && return_v != null) {
      ...
      // 修复当单元格内有换行获取不到值的问题
        ...
        const inlineStrValueArr = cellData.ct.s;
...
export function clearCell(row, column, options = {}) {
  ...
  const targetSheetData = $.extend(true, [], Store.luckysheetfile[order].data);
...
export function deleteCell(move, row, column, options = {}) {
  ...
  const moveType = `move${  move.replace(move[0], move[0].toUpperCase())}`; // left-moveLeft;  up-moveUp
...
export function setCellFormat(row, column, attr, value, options = {}) {
  ...
  const cellData = targetSheetData[row][column] || {};
  const cfg = $.extend(true, {}, file.config);
  ...
  for (let i = 0; i < targetSheetData.length; i++) {
    ...
    for (let j = 0; j < rowArr.length; j++) {
      ...
      if (isWholeWord) {
        ...
        const reg = new RegExp(func_methods.getRegExpStr(content), 'g');
        if (reg.test(cell[type])) {
          result.push(cell);
        }
      } else {
        const reg = new RegExp(func_methods.getRegExpStr(content), 'ig');
        if (reg.test(cell[type])) {
          result.push(cell);
        }
      }
...
export function replace(content, replaceContent, options = {}) {
  const matchCells = find(content, options);
  const curSheetOrder = getSheetIndex(Store.currentSheetIndex);
  const {
    order = curSheetOrder,
  } = {...options};
  ...
  const sheetData = $.extend(true, [], file.data);
  ...
  const fileData = $.extend(true, [], file.data);
  file.data.length = 0;
...
export function enterEditMode(options = {}){
  ...
  if($('#luckysheet-conditionformat-dialog').is(':visible')){
    ...
    const row_index = last['row_focus'], col_index = last['column_focus'];
...
export function frozenRowRange(range, order) {
  ...
  if (!order || order == getSheetIndex(Store.currentSheetIndex)) {
    ...
    const row_focus = range.row_focus;
    if(row_focus > row_st){
      row_st = row_focus;
    }
    if(row_st == -1){
...
export function frozenColumnRange(range, order) {
  const locale_frozen = locale().freezen;
  const isStringRange = typeof range === 'string' && formula.iscelldata(range);
  ...
  if (!order || order == getSheetIndex(Store.currentSheetIndex)) {
    ...
    const column_focus = range.column_focus;
...
export function setBothFrozen(isRange, options = {}) {
  ...
  const isCurrentSheet = !order || order == getSheetIndex(Store.currentSheetIndex);
  const locale_frozen = locale().freezen;
...
export function hideRowOrColumn(type, startIndex, endIndex, options = {}) {
  ...
  const file = Store.luckysheetfile[order];
  const cfgKey = type === 'row' ? 'rowhidden': 'colhidden';
  const cfg = $.extend(true, {}, file.config);
  if(cfg[cfgKey] == null) {
    cfg[cfgKey] = {};
  }
...
export function setColumnWidth(columnInfo, options = {}) {
  ...
  for(const c in columnInfo){
    if(parseInt(c) >= 0){
      const len = columnInfo[c];
...
export function getRowHeight(rowInfo, options = {}) {
  ...
  rowInfo.forEach((item) => {
    if(parseInt(item) >= 0){
      const size = rowlen[parseInt(item)] || Store.defaultrowlen;
...
export function getColumnWidth(columnInfo, options = {}) {
  ...
  columnInfo.forEach((item) => {
    if(parseInt(item) >= 0){
      const size = columnlen[parseInt(item)] || Store.defaultcollen;
...
export function getRange() {
  const rangeArr = JSON.parse(JSON.stringify(Store.luckysheet_select_save));
...
export function getRangeAxis() {
  const result = [];
  const rangeArr = JSON.parse(JSON.stringify(Store.luckysheet_select_save));
  const sheetIndex = Store.currentSheetIndex;

  rangeArr.forEach(ele=>{
    const axisText = getRangetxt(sheetIndex, {column:ele.column,row:ele.row});
...
export function getRangeValue(options = {}) {
  const curOrder = getSheetIndex(Store.currentSheetIndex);
  const {
    range,
    order = curOrder
  } = {...options};
...
export function getRangeHtml(options = {}) {
  ...
  if(getObjType(range) == 'string'){
    ...
    const cellrange = formula.getcellrange(range);
    range = [{
      'row': cellrange.row,
  ...
  //多重选区 有条件格式时 提示
  const cdformat = $.extend(true, [], file.luckysheet_conditionformat_save);
  if (range.length > 1 && cdformat.length > 0) {
    let hasCF = false;
    const cf_compute = conditionformat.getComputeMap(file.index);
        ...
        if(getObjType(d[r][c]) == 'object' && ('mc' in d[r][c])){
          if('rs' in d[r][c]['mc']){
            ...
            if(borderInfoCompute && borderInfoCompute[`${r  }_${  c}`]){
              ...
              const rowlen = d[r][c]['mc'].rs, collen = d[r][c]['mc'].cs;

              if(JSON.stringify(bl_obj).length > 23){
                let bl_color = null, bl_style = null;

                for(const x in bl_obj.color){
                  if(bl_obj.color[x] >= (rowlen / 2)){
                    bl_color = x;
                  }
                }

                for(const x in bl_obj.style){
              ...
              if(JSON.stringify(br_obj).length > 23){
                ...
                for(const x in br_obj.color){
                  if(br_obj.color[x] >= (rowlen / 2)){
                    br_color = x;
                  }
                }

                for(const x in br_obj.style){
              ...
              if(JSON.stringify(bt_obj).length > 23){
                ...
                for(const x in bt_obj.color){
                  if(bt_obj.color[x] >= (collen / 2)){
                    bt_color = x;
                  }
                }

                for(const x in bt_obj.style){
              ...
              if(JSON.stringify(bb_obj).length > 23){
                ...
                for(const x in bb_obj.color){
                  if(bb_obj.color[x] >= (collen / 2)){
                    bb_color = x;
                  }
                }

                for(const x in bb_obj.style){
...
export function getRangeArray(dimensional, options = {}) {
  const dimensionalValues = ['oneDimensional', 'twoDimensional'];
  ...
  const cfg = $.extend(true, {}, file.config);
  if(cfg['merge'] != null){
    const has_PartMC = hasPartMC(cfg, r1, r2, c1, c2);
...
export function getRangeJson(isFirstRowTitle, options = {}) {
  const curRange = Store.luckysheet_select_save[0];
  const curSheetOrder = getSheetIndex(Store.currentSheetIndex);
  let {
    range = curRange,
    order = curSheetOrder
  ...
  const getdata = getdatabyselection(range, file.index);
  const arr = [];
  if (getdata.length === 0) {
    return;
  }
...
export function getRangeDiagonal(type, options = {}) {
  ...
  const getdata = getdatabyselection(range, order);
  const arr = [];
  if (getdata.length === 0) {
    return;
  }

  const clen = getdata[0].length;
  switch (type) {
  case 'normal':
    for (let r = 0; r < getdata.length; r++) {
      if (r >= clen) {
...
export function setRangeShow(range, options = {}) {
  ...
  if(getObjType(range) == 'array'){
    for(let i = 0; i < range.length; i++){
      if(getObjType(range[i]) === 'string'){
        ...
        const cellrange = formula.getcellrange(range[i]);
        range[i] = {
          'row': cellrange.row,
          'column': cellrange.column
        };
  ...
  for(let i = 0; i < range.length; i++){
    const changeparam = menuButton.mergeMoveMain(range[i].column, range[i].row, range[i]);
    if(changeparam) {
      range[i] = {
        'row': changeparam[1],
...
export function setRangeValue(data, options = {}) {
  const curSheetOrder = getSheetIndex(Store.currentSheetIndex);
  const curRange = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1];
  let {
    range = curRange,
    isRefresh = true,
    order = curSheetOrder,
...
export function setRangeFilter(type, options = {}) {
  ...
  if(type == 'open'){
    ...
    const luckysheet_filter_save = $.extend(true, {}, file.filter_select);
...
export function setRangeMerge(type, options = {}) {
  const typeValues = ['all', 'horizontal', 'vertical'];
  if (typeValues.indexOf(type) < 0) {
  ...
  for(let i = 0; i < range.length; i++){
    const r1 = range[i].row[0],
      r2 = range[i].row[1];
    const c1 = range[i].column[0],
      c2 = range[i].column[1];
...
export function cancelRangeMerge(options = {}) {
  const curRange = Store.luckysheet_select_save,
    ...
    for(let r = r1; r <= r2; r++){
      for(let c = c1; c <= c2; c++){
        ...
        if(cell != null && cell.mc != null){
          const mc_r = cell.mc.r, mc_c = cell.mc.c;
...
export function setRangeConditionalFormatDefault(conditionName, conditionValue, options = {}) {
  ...
  const conditionRange = [], conditionValue2 = [];
  ...
  if(conditionName == 'betweenness'){
    ...
    //条件值是否是选区
    const rangeArr1 = conditionformat.getRangeByTxt(v1);
    ...
    const rangeArr2 = conditionformat.getRangeByTxt(v2);
    ...
    //条件值是否是选区
    const rangeArr = conditionformat.getRangeByTxt(v);
    ...
    const v1 = conditionValue[0];
    const v2 = conditionValue[1];
  ...
  const historyRules = conditionformat.getHistoryRules(fileH);

  //保存当前的规则
  const ruleArr = file['luckysheet_conditionformat_save'] || [];
  ruleArr.push(rule);
  file['luckysheet_conditionformat_save'] = ruleArr;

  const fileC = $.extend(true, [], Store.luckysheetfile);
  const currentRules = conditionformat.getCurrentRules(fileC);
...
export function deleteRangeConditionalFormat(itemIndex, options = {}) {
  ...
  const cdformatItem = cdformat.splice(itemIndex, 1);
...
export function matrixOperation(type, options = {}) {
  ...
  const getdata = getdatabyselection(range);
  const arr = [];
  if (getdata.length === 0) {
    return;
  }
...
export function setSheetAdd(options = {}) {
  const lastOrder = Store.luckysheetfile.length - 1;
  let {
    sheetObject = {},
    order = lastOrder,
    success
  ...
  const index = sheetmanage.generateRandomSheetIndex();
  ...
  if(order <= 0){
    const beforeIndex = Store.luckysheetfile[0].index;
    const beforeObj = $(`#luckysheet-sheets-item${  beforeIndex}`);
    ...
    const afterIndex = Store.luckysheetfile[order - 1].index;
    const afterObj = $(`#luckysheet-sheets-item${  afterIndex}`);
...
export function setSheetCopy(options = {}) {
  ...
  const copyindex = file.index;
  const index = sheetmanage.generateRandomSheetIndex();

  const copyjson = $.extend(true, {}, file);
  ...
  if (Store.clearjfundo) {
    ...
    const jfredostr = Store.jfredo[Store.jfredo.length - 1];
...
export function setSheetMove(type, options = {}) {
  ...
  if(type == 'left'){
    ...
    const prevIndex = Store.luckysheetfile[order - 1].index;
    ...
    const nextIndex = Store.luckysheetfile[order + 1].index;
    ...
    if(type < order){
      const prevIndex = Store.luckysheetfile[type].index;
      $(`#luckysheet-sheets-item${  sheetIndex}`).insertBefore($(`#luckysheet-sheets-item${  prevIndex}`));
    }
    else{
      const nextIndex = Store.luckysheetfile[type].index;
      $(`#luckysheet-sheets-item${  sheetIndex}`).insertAfter($(`#luckysheet-sheets-item${  nextIndex}`));
    }
...
export function setSheetOrder(orderList, options = {}) {
  ...
  Store.luckysheetfile.sort((x, y) => {
    const order_x = orderListMap[x.index.toString()];
    const order_y = orderListMap[y.index.toString()];
...
export function scroll(options = {}){
  ...
  if(scrollLeft != null){
    ...
    const col = Store.visibledatacolumn[targetColumn],
  ...
  if(scrollTop != null){
    ...
    const row = Store.visibledatarow[targetRow],
...
export function getScreenshot(options = {}) {
  ...
  const has_PartMC = hasPartMC(Store.config, str, edr, stc, edc);
...
export function getWorkbookName(options = {}) {
  ...
  let name = '';
  const element = $('#luckysheet_info_detail_input');
...
export function undo(options = {}) {
  const ctr = $.extend(true, {}, Store.jfredo[Store.jfredo.length - 1]);
...
export function redo(options = {}) {
  const ctr = $.extend(true, {}, Store.jfundo[Store.jfundo.length - 1]);
...
export function getAllSheets() {
  const data = $.extend(true, [], Store.luckysheetfile);
...
export function getConfig(options = {}) {
  ...
  const config = $.extend(true, {}, file.config);
...
export function setDataVerification(optionItem, options = {}) {
  ...
  const typeValues = ['dropdown', 'checkbox', 'number', 'number_integer', 'number_decimal', 'text_content', 'text_length', 'date', 'validity'];
  const type2Values_1 = ['bw', 'nb', 'eq', 'ne', 'gt', 'lt', 'gte', 'lte'];
  const type2Values_2 = ['include', 'exclude', 'equal'];
  const type2Values_3 = ['bw', 'nb', 'eq', 'ne', 'bf', 'nbf', 'af', 'naf'];
  const type2Values_4 = ['card', 'phone'];
  ...
  const currentDataVerification = $.extend(true, {}, file.dataVerification);
  ...
  if(file.index == Store.currentSheetIndex){
    const historyDataVerification = $.extend(true, {}, file.dataVerification);
...
export function insertImage(src, options = {}){
  ...
  if(file.index == Store.currentSheetIndex){
    ...
    const visibledatarow = file.visibledatarow || [];
    ...
    const visibledatacolumn = file.visibledatacolumn || [];
...
export function updataSheet (options = {}) {
  ...
  const file = files[sheetmanage.getSheetIndex(Store.currentSheetIndex)],
...
export function checkTheStatusOfTheSelectedCells(type,status){
  ...
  /* 获取选区内所有的单元格-扁平后的处理 */
  const cells = getRangeWithFlatten();

  const flag = cells.every(({r,c})=>{
