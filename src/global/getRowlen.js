...
function getMeasureText(value, ctx, fontset){
  ...
  if(mtc != null){
    ...
    const measureText = ctx.measureText(value), cache = {};
    // var regu = "^[ ]+$";
    ...
    if(ctx.textBaseline == 'alphabetic'){
      const descText = 'gjpqy', matchText='abcdABCD';
...
function getCellTextInfo(cell , ctx, option){
  ...
  if(tr=='3'){//vertical text
    ...
    if(tb=='2' || isInline){//wrap
      ...
      const width = textWidthAll, height = textHeightAll;
...
function drawLineInfo(wordGroup, cancelLine,underLine,option){
  const left = option.left, top = option.top, width=option.width, height = option.height, asc = option.asc,desc = option.desc,fs = option.fs;
