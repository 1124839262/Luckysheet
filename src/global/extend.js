...
function luckysheetextendtable(type, index, value, direction, sheetIndex) {
  ...
  const curOrder = getSheetIndex(sheetIndex);
  const file = Store.luckysheetfile[curOrder];
  const d = $.extend(true, [], file.data);
  ...
  if (calcChain != null && calcChain.length > 0) {
    for (let i = 0; i < calcChain.length; i++) {
      const calc = $.extend(true, {}, calcChain[i]);
      const calc_r = calc.r,
        calc_c = calc.c,
        calc_i = calc.index,
      ...
      if (type == 'row') {
        const functionStr = `=${  formula.functionStrChange(calc_funcStr, 'add', 'row', direction, index, value)}`;
        ...
        const functionStr = `=${  formula.functionStrChange(calc_funcStr, 'add', 'col', direction, index, value)}`;
  ...
  //筛选配置变动
  const filter_select = file.filter_select;
  ...
  if (newFilterObj != null && newFilterObj.filter != null) {
    ...
    for (const k in newFilterObj.filter) {
      const f_rowhidden = newFilterObj.filter[k].rowhidden;

      for (const n in f_rowhidden) {
        cfg['rowhidden'][n] = 0;
      }
    }
  }
  ...
  if (luckysheetFreezen.freezenhorizontaldata != null && type == 'row') {
    ...
    const freezen_top = Store.visibledatarow[freezen_row_st] - 2 - freezen_scrollTop + Store.columnHeaderHeight;
  ...
  if (luckysheetFreezen.freezenverticaldata != null && type == 'column') {
    ...
    const freezen_left = Store.visibledatacolumn[freezen_col_st] - 2 - freezen_scrollLeft + Store.rowHeaderWidth;
  ...
  if (dataVerification != null) {
    for (const key in dataVerification) {
  ...
  if (type == 'row') {
    ...
    if (cfg['borderInfo'] && cfg['borderInfo'].length > 0) {
      ...
      for (let i = 0; i < cfg['borderInfo'].length; i++) {
        const rangeType = cfg['borderInfo'][i].rangeType;
  ...
  if (type == 'row') {
    ...
    const row = Store.visibledatarow[range[0].row[1]],
...
function luckysheetextendData(rowlen, newData) {
  ...
  const collen = d[0].length;
  const addNullData = datagridgrowth([], rowlen, collen);
...
function luckysheetdeletetable(type, st, ed, sheetIndex) {
    ...
    for (let i = 0; i < calcChain.length; i++) {
      ...
      if (type == 'row') {
        if (calc_r < st || calc_r > ed) {
          const functionStr = `=${  formula.functionStrChange(calc_funcStr, 'del', 'row', null, st, slen)}`;
        ...
        if (calc_c < st || calc_c > ed) {
          const functionStr = `=${  formula.functionStrChange(calc_funcStr, 'del', 'col', null, st, slen)}`;
  ...
  if (type == 'row') {
    ...
    if (cfg['borderInfo'] && cfg['borderInfo'].length > 0) {
      ...
      for (let i = 0; i < cfg['borderInfo'].length; i++) {
        ...
        if (rangeType == 'range') {
          ...
          const row_index = cfg['borderInfo'][i].value.row_index;
    ...
    if (cfg['borderInfo'] && cfg['borderInfo'].length > 0) {
      ...
      for (let i = 0; i < cfg['borderInfo'].length; i++) {
        ...
        if (rangeType == 'range') {
          ...
          const col_index = cfg['borderInfo'][i].value.col_index;
    ...
    for (let r = 0; r < d.length; r++) {
      const row = [].concat(JSON.parse((JSON.stringify(d[r]))));
      //删除该行选中列
      row.splice(st, slen);
...
function luckysheetDeleteCell(type, str, edr, stc, edc, sheetIndex) {
  ...
  const rlen = edr - str + 1;
  const clen = edc - stc + 1;
  const cfg = $.extend(true, {}, Store.config);
  ...
  if (filter_select != null && JSON.stringify(filter_select) != '{}') {
    ...
    const f_r1 = filter_select.row[0],
      f_r2 = filter_select.row[1];
    ...
    if (type == 'moveUp') {
      if (f_c1 >= stc && f_c2 <= edc) {
        ...
        if (newFilterObj.filter_select != null && filter != null) {
          for (const k in filter) {
            const f_stc = newFilterObj.filter_select.column[0];
            const f_edc = newFilterObj.filter_select.column[1];
            let f_cindex = filter[k].cindex;
  ...
  if (cfg['borderInfo'] && cfg['borderInfo'].length > 0) {
    ...
    for (let i = 0; i < cfg['borderInfo'].length; i++) {
      ...
      if (rangeType == 'range') {
        ...
        for (let j = 0; j < borderRange.length; j++) {
          const bd_r1 = borderRange[j].row[0],
            bd_r2 = borderRange[j].row[1],
    ...
    if (str > r2 || edr < r1 || stc > c2) {
      ...
      if (stc <= c1 && edc >= c2) {
        if (str > r1 && edr < r2) {
          ...
          const range = [{ row: [r1, str - 1], column: [c1, c2] }];
          newRange = newRange.concat(range);
        } else if (edr < r2) {
          const range = [{ row: [edr + 1, r2], column: [c1, c2] }];
          newRange = newRange.concat(range);
        }
    ...
    if (stc > c2 || edc < c1 || str > r2) {
      ...
      if (str <= r1 && edr >= r2) {
        if (stc > c1 && edc < c2) {
          ...
          const range = [{ row: [r1, r2], column: [c1, stc - 1] }];
          newRange = newRange.concat(range);
        } else if (edc < c2) {
          const range = [{ row: [r1, r2], column: [edc + 1, c2] }];
          newRange = newRange.concat(range);
        }
      } else if (str > r1 && edr < r2) {
        if (stc <= c1 && edc >= c2) {
