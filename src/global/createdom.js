...
export default function luckysheetcreatedom(colwidth, rowheight, data, menu, title) {
  ...
  const addControll = `<button id="luckysheet-bottom-add-row" class="btn btn-default">${locale_info.add}</button><input id="luckysheet-bottom-add-row-input" type="text" class="luckysheet-datavisual-config-input luckysheet-mousedown-cancel" placeholder="${luckysheetConfigsetting.addRowCount || 100}"><span style="font-size: 14px;">${ locale_info.row }</span><span style="font-size: 14px;color: #9c9c9c;">(${locale_info.addLast})</span>`;
  const backControll = ` <button id="luckysheet-bottom-bottom-top" class="btn btn-default" style="">${ locale_info.backTop }</button>`;
  ...
  const pageControll = ` <span id="luckysheet-bottom-page-info" style="font-size: 14px;color: #f34141;">${ pageInfo }</span> <button id="luckysheet-bottom-page-next" class="btn btn-danger" style="">下一页</button>`;
  const pageControll2 = ` <span id="luckysheet-bottom-page-info" style="font-size: 14px;color: #f34141;">${pageInfo}</span>`;
