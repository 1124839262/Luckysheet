...
const method = {
  ...
  addDataAjax: function(param, index, url, func){
    ...
    $.ajax({
      ...
      success: function(d) {
        ...
        const newData = dataset.celldata;
  ...
  reload: function(param, index, url, func){
    ...
    const arg = {'gridKey' : server.gridKey, 'index': index};
    ...
    $.post(url, param, function (d) {
      ...
      const data = sheetmanage.buildGridData(file);
  ...
  clearSheetByIndex: function(i){
    ...
    const sheetfile = Store.luckysheetfile[index];
  ...
  clear: function(index){
    ...
    if(index == 'all'){
      for(let i = 0; i < Store.luckysheetfile.length; i++){
        const sheetfile = Store.luckysheetfile[i];
  ...
  destroy:function(){
    ...
    const defaultStore = $.extend(true, {}, defaultConfig.defaultStore);
    ...
    const defaultFormula = $.extend(true, {}, defaultConfig.defaultFormula);
    for(const key in defaultFormula){
      if(key in luckysheetformula){
        luckysheetformula[key] = defaultFormula[key];
      }
    }

    const defaultSheet = $.extend(true, {}, defaultConfig.defaultSheet);
    ...
    const defaultPivotTable = $.extend(true, {}, defaultConfig.defaultPivotTable);
    for(const key in defaultPivotTable){
      if(key in pivotTable){
        pivotTable[key] = defaultPivotTable[key];
      }
    }

    const defaultImage = $.extend(true, {}, defaultConfig.defaultImage);
    ...
    const defaultDataVerification = $.extend(true, {}, defaultConfig.defaultDataVerification);
    for(const key in defaultDataVerification){
  ...
  editorChart:function(c){
    const chart_selection_color = luckyColor[0];
    const chart_id = 'luckysheetEditMode-datav-chart';
    const chart_selection_id = `${chart_id  }_selection`;
    c.chart_id = chart_id;
  ...
  createHookFunction:function(){
    ...
    if(luckysheetConfigsetting.hook && luckysheetConfigsetting.hook[hookName]!=null && (typeof luckysheetConfigsetting.hook[hookName] === 'function')){
      ...
      args.shift();
      const ret = luckysheetConfigsetting.hook[hookName].apply(this, args);
      if(ret===false){
        return false;
      }
      else{
