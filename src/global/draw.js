...
import {luckysheet_CFiconsImg, luckysheetdefaultFont, luckysheetdefaultstyle} from '../controllers/constant';
...
function luckysheetDrawgridRowTitle(scrollHeight, drawHeight, offsetTop) {
  ...
  for (let r = dataset_row_st; r <= dataset_row_ed; r++) {
    ...
    if (Store.config['rowhidden'] != null && Store.config['rowhidden'][r] != null) {
      ...
      const textMetrics = getMeasureText(r + 1, luckysheetTableContent);
      //luckysheetTableContent.measureText(r + 1);

      const horizonAlignPos = (Store.rowHeaderWidth - textMetrics.width) / 2;
...
function luckysheetDrawgridColumnTitle(scrollWidth, drawWidth, offsetLeft) {
  ...
  for (let c = dataset_col_st; c <= dataset_col_ed; c++) {
    ...
    if (Store.config['colhidden'] != null && Store.config['colhidden'][c] != null) {
      ...
      const textMetrics = getMeasureText(abc, luckysheetTableContent);
      ...
      const verticalAlignPos = Math.round(Store.columnHeaderHeight / 2);
...
function luckysheetDrawMain(
  ...
  for (let r = dataset_row_st; r <= dataset_row_ed; r++) {
    ...
    const end_r = Store.visibledatarow[r] - scrollHeight;
    ...
    for (let c = dataset_col_st; c <= dataset_col_ed; c++) {
      ...
      const end_c = Store.visibledatacolumn[c] - scrollWidth;
  ...
  //动态数组公式计算
  const dynamicArray_compute = dynamicArrayCompute(
  ...
  //表格渲染区域 溢出单元格配置保存
  const cellOverflowMap = getCellOverflowMap(
    luckysheetTableContent,
    dataset_col_st,
    dataset_col_ed,
    dataset_row_st,
  ...
  for (let cud = 0; cud < cellupdate.length; cud++) {
    ...
    const r = item.r,
      ...
      end_r = item.end_r,
      end_c = item.end_c;
    const firstcolumnlen = item.firstcolumnlen;
    ...
    if (Store.flowdata[r][c] == null) {
      ...
      if (value == null || value.toString().length == 0) {
        ...
        //sparklines渲染
        const borderfix = menuButton.borderfix(Store.flowdata, r, c);
        const cellsize = [
  ...
  if (Store.config['borderInfo'] != null && Store.config['borderInfo'].length > 0) {
    //边框渲染
    const borderLeftRender = function(style, color, start_r, start_c, end_r, end_c, offsetLeft, offsetTop, canvas) {
      const linetype = style;

      const m_st = start_c - 2 + bodrder05 + offsetLeft;
      const m_ed = start_r + offsetTop - 1;
      const line_st = start_c - 2 + bodrder05 + offsetLeft;
      const line_ed = end_r - 2 + bodrder05 + offsetTop;
      canvas.save();
    ...
    const borderRightRender = function(style, color, start_r, start_c, end_r, end_c, offsetLeft, offsetTop, canvas) {
      const linetype = style;

      const m_st = end_c - 2 + bodrder05 + offsetLeft;
      const m_ed = start_r + offsetTop - 1;
      const line_st = end_c - 2 + bodrder05 + offsetLeft;
      const line_ed = end_r - 2 + bodrder05 + offsetTop;
      canvas.save();
      menuButton.setLineDash(canvas, linetype, 'v', m_st, m_ed, line_st, line_ed);
    ...
    const borderBottomRender = function(style, color, start_r, start_c, end_r, end_c, offsetLeft, offsetTop, canvas) {
      const linetype = style;

      const m_st = start_c - 2 + bodrder05 + offsetLeft;
      const m_ed = end_r - 2 + bodrder05 + offsetTop;
      const line_st = end_c - 2 + bodrder05 + offsetLeft;
      const line_ed = end_r - 2 + bodrder05 + offsetTop;
      canvas.save();
    ...
    const borderTopRender = function(style, color, start_r, start_c, end_r, end_c, offsetLeft, offsetTop, canvas) {
      const linetype = style;

      const m_st = start_c - 2 + bodrder05 + offsetLeft;
      const m_ed = start_r - 1 + bodrder05 + offsetTop;
      const line_st = end_c - 2 + bodrder05 + offsetLeft;
      const line_ed = start_r - 1 + bodrder05 + offsetTop;
      canvas.save();
      menuButton.setLineDash(canvas, linetype, 'h', m_st, m_ed, line_st, line_ed);
    ...
    for (const x in borderInfoCompute) {
      ...
      const bd_r = x.substr(0, x.indexOf('_'));
      const bd_c = x.substr(x.indexOf('_') + 1);
      ...
      if (borderOffset[`${bd_r  }_${  bd_c}`]) {
        const start_r = borderOffset[`${bd_r  }_${  bd_c}`].start_r;
        const start_c = borderOffset[`${bd_r  }_${  bd_c}`].start_c;
        const end_r = borderOffset[`${bd_r  }_${  bd_c}`].end_r;
        const end_c = borderOffset[`${bd_r  }_${  bd_c}`].end_c;

        const cellOverflow_colInObj = cellOverflow_colIn(
          cellOverflowMap,
          bd_r,
          bd_c,
          dataset_col_st,
...
//sparklines渲染
const sparklinesRender = function(r, c, offsetX, offsetY, canvasid, ctx) {
  if (Store.flowdata[r] == null || Store.flowdata[r][c] == null) {
    return;
  }
...
const nullCellRender = function(
  ...
  const checksAF = alternateformat.checksAF(r, c, af_compute); //交替颜色
  const checksCF = conditionformat.checksCF(r, c, cf_compute); //条件格式
  ...
  if (`${r  }_${  c}` in dynamicArray_compute) {
    const value = dynamicArray_compute[`${r  }_${  c}`].v;

    luckysheetTableContent.fillStyle = '#000000';
    //文本宽度和高度
    const fontset = luckysheetdefaultFont();
    luckysheetTableContent.font = fontset;

    //水平对齐 (默认为1，左对齐)
    const horizonAlignPos = start_c + 4 + offsetLeft;

    //垂直对齐 (默认为2，下对齐)
    const verticalFixed = browser.luckysheetrefreshfixed();
    const verticalAlignPos = end_r + offsetTop - 2;
  ...
  //此单元格 与  溢出单元格关系
  const cellOverflow_colInObj = cellOverflow_colIn(cellOverflowMap, r, c, dataset_col_st, dataset_col_ed);
...
const cellRender = function(
  ...
  const cell = Store.flowdata[r][c];
  const cellWidth = end_c - start_c - 2;
  const cellHeight = end_r - start_r - 2;
  const space_width = 2,
    space_height = 2; //宽高方向 间隙

  //水平对齐
  const horizonAlign = menuButton.checkstatus(Store.flowdata, r, c, 'ht');
  //垂直对齐
  const verticalAlign = menuButton.checkstatus(Store.flowdata, r, c, 'vt');
  ...
  const dataVerification = dataVerificationCtrl.dataVerification;
  ...
  //数据验证 复选框
    ...
    const pos_x = start_c + offsetLeft;
    const pos_y = start_r + offsetTop + 1;
    ...
    const measureText = getMeasureText(value, luckysheetTableContent);
    ...
    if (
      ...
      const x = start_c + offsetLeft + space_width;
      const y = start_r + offsetTop + space_height;
      const w = cellWidth - space_width * 2;
      const h = cellHeight - space_height * 2;

      const valueType = checksCF['dataBar']['valueType'];
      const valueLen = checksCF['dataBar']['valueLen'];
      const format = checksCF['dataBar']['format'];

      if (valueType == 'minus') {
        //负数
        const minusLen = checksCF['dataBar']['minusLen'];

        if (format.length > 1) {
          //渐变
          const my_gradient = luckysheetTableContent.createLinearGradient(
        ...
        //正数
        const plusLen = checksCF['dataBar']['plusLen'];

        if (plusLen == 1) {
          if (format.length > 1) {
            //渐变
            const my_gradient = luckysheetTableContent.createLinearGradient(x, y, x + w * valueLen, y);
    ...
    const textInfo = getCellTextInfo(cell, luckysheetTableContent, {
      cellWidth: cellWidth,
...
//溢出单元格渲染
const cellOverflowRender = function(
  r,
  c,
  stc,
  edc,
  ...
  const end_c = Store.visibledatacolumn[edc] - scrollWidth;
  ...
  for (let r = row_st; r <= row_end; r++) {
    ...
    for (let c = 0; c < data[r].length; c++) {
      ...
      if (
        ...
        //水平对齐
        const horizonAlign = menuButton.checkstatus(data, r, c, 'ht');

        const textMetricsObj = getCellTextInfo(cell, canvas, {
          r: r,
          c: c,
        });
        let textMetrics = 0;
        ...
        const start_c = c - 1 < 0 ? 0 : Store.visibledatacolumn[c - 1];
        const end_c = Store.visibledatacolumn[c];
        ...
        if (end_c - start_c < textMetrics) {
          if (horizonAlign == '0') {
            //居中对齐
            const trace_forward = cellOverflow_trace(r, c, c - 1, 'forward', horizonAlign, textMetrics);
            const trace_backward = cellOverflow_trace(r, c, c + 1, 'backward', horizonAlign, textMetrics);
            ...
            //左对齐
            const trace = cellOverflow_trace(r, c, c + 1, 'backward', horizonAlign, textMetrics);
            stc = c;
            ...
            //右对齐
            const trace = cellOverflow_trace(r, c, c - 1, 'forward', horizonAlign, textMetrics);
            edc = c;
...
function cellOverflow_trace(r, curC, traceC, traceDir, horizonAlign, textMetrics) {
  ...
  const start_traceC = traceC - 1 < 0 ? 0 : Store.visibledatacolumn[traceC - 1];
  const end_traceC = Store.visibledatacolumn[traceC];
...
function cellTextRender(textInfo, ctx, option) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const txt = typeof word.content === 'object' ? word.content.m : word.content;
