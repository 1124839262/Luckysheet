...
import {isInlineStringCell, isInlineStringCT} from '../controllers/inlineString';
...
export function getdatabyselectionD(d, range) {
  ...
  const dynamicArray_compute = dynamicArrayCompute(Store.luckysheetfile[getSheetIndex(Store.currentSheetIndex)]['dynamicArray']);
  const data = [];
...
export function getcellvalue(r, c, data, type) {
  ...
  if (r != null && c != null) {
    ...
    const newData = data[0].map(function(col, i) {
...
export function datagridgrowth(data, addr, addc, iscallback) {
  ...
  const coladd = [];//需要额外增加的空列
  for (let c = 0; c < addc; c++) {
    coladd.push(null);
  }

  const rowadd = [];//完整的一个空行
...
export function getOrigincell(r, c, i) {
  ...
  if (i == null) {
    ...
    const sheet = sheetmanage.getSheetByIndex(i);
    data = sheet.data;
  }
...
export function getRealCellValue(r, c){
  ...
  if(value == null){
    ...
    if(value==null){
      const ct = getcellvalue(r, c, null, 'ct');
