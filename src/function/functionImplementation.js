...
import {diff, isdatatype, isdatetime} from '../global/datecontroll';
...
const functionImplementation = {
  ...
  'AGE_BY_IDCARD': function() {
    ...
    try {
      ...
      const age = cuurentdate.diff(birthday, 'years');
  ...
  'STAR_BY_IDCARD': function() {
    ...
    try {
      ...
      const d = new Date(1999, month, day, 0, 0, 0);
      const arr = [];
  ...
  'ANIMAL_BY_IDCARD': function() {
    ...
    try {
      ...
      const list = new Array('猪', '鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊', '猴', '鸡', '狗');
      const index = (parseInt(birthday.getFullYear()) + 9) % 12;
  ...
  'MOD': function() {
    ...
    try {
      ...
      //计算结果
      const modulus = Math.abs(number % divisor);
  ...
  'SUBTOTAL': function() {
    ...
    try {
      ...
      const data_function_num = arguments[0];
      let function_num;
  ...
  'COUNTIF': function() {
    ...
    try {
      ...
      //条件
      const data_criteria = arguments[1];
      let criteria;
  ...
  'SERIESSUM': function() {
    ...
    try {
      ...
      //与 x 的每个连续乘幂相乘的一组系数
      const data_coefficients = arguments[3];
      let coefficients = [];
  ...
  'SUMIFS': function() {
    ...
    try {
      ...
      const results = new Array(rangeData.length);
  ...
  'COUNTIFS': function() {
    ...
    try {
      const args = arguments;
      luckysheet_getValue(args);
      const results = new Array(formula.getRangeArray(args[0])[0].length);
      for(var i = 0;i < results.length; i++){
        results[i] = true;
      }
  ...
  'RANK_EQ': function() {
    ...
    try {
      ...
      const index = ref_n.indexOf(number);
  ...
  'PERCENTRANK_EXC': function() {
    ...
    try {
      ...
      const uniques = window.luckysheet_function.UNIQUE.f(ref_n)[0];

      const n = ref_n.length;
  ...
  'COVARIANCE_P': function() {
    ...
    try {
      ...
      //计算
      const mean1 = jStat.mean(data_x);
      const mean2 = jStat.mean(data_y);
  ...
  'TRIMMEAN': function() {
    ...
    try {
      ...
      const trim = window.luckysheet_function.FLOOR.f(dataArr_n.length * percent, 2) / 2;
  ...
  'Z_TEST': function() {
    ...
    try {
      ...
      //计算
      const n = dataArr_n.length;
      const mean = window.luckysheet_function.AVERAGE.f.apply(window.luckysheet_function.AVERAGE, dataArr_n);
  ...
  'PROB': function() {
    ...
    try {
      ...
      for(var i = 0; i < data_x_range.length; i++){
        const num_x_range = data_x_range[i];
        const num_prob_range = data_prob_range[i];
  ...
  'T_TEST': function() {
    ...
    try {
      ...
      if(type == 1){
        ...
        const diff_mean = Math.abs(jStat.mean(diff_arr));
        const diff_sd = func_methods.standardDeviation_s(diff_arr);
        ...
        const s_x = func_methods.variance_s(data_x);
        const s_y = func_methods.variance_s(data_y);
  ...
  'INDIRECT': function() {
    ...
    try {
      ...
      if(formula.iscelldata(ref_text)){
        const cellrange = formula.getcellrange(ref_text);
        const row = cellrange.row[0], col = cellrange.column[0];
        ...
        if (formula.execFunctionGlobalData != null) {
          const ef = formula.execFunctionGlobalData[`${row}_${col}_${Store.calculateSheetIndex}`];
          if(ef!=null){
            value = ef.v;
          }
        }
  ...
  'OFFSET': function() {
    ...
    try {
      ...
      const reference = arguments[0].startCell;
      const sheetName = arguments[0].sheetName;
      ...
      for(let r = cellRow0; r <= cellRow1; r++){
        ...
        for(let c = cellCol0; c <= cellCol1; c++){
          if(formula.execFunctionGlobalData != null && formula.execFunctionGlobalData[`${r}_${c}_${Store.calculateSheetIndex}`]!=null){
            const ef = formula.execFunctionGlobalData[`${r}_${c}_${Store.calculateSheetIndex}`];
            if(ef!=null){
              rowArr.push(ef.v);
            }
            else{
  ...
  'HLOOKUP': function() {
    ...
    try {
      ...
      for(var c = 0;c < range[0].length;c++){
        const matchv = getcellvalue(0, c, range);
        const showv = getcellvalue(index - 1, c, range);
    ...
    try {
      ...
      if(typeof(searchkey) === 'string'){ //字符串直接判断是否相等
        ...
        if(rangeNow.length != 0){
          ...
          const index = rangeNow.indexOf(searchkey);
          if(index == (rangeNow.length -1)){
            return [formula.error.na, '找不到对应参数'];
          }else{
            const mat = rangeNow[index + 1];
            if(arguments[2]){
  ...
  'INDEX': function() {
    ...
    try {
      ...
      const rowlen = array.length, collen = array[0].length;
      ...
      if(isReference){
        ...
        const row_index = cellRow0 + row_num - 1, column_index = cellCol0 + column_num - 1;
    ...
    try {
      ...
      if(getObjType(data_result) == 'array'){
        ...
        if(getObjType(data_result.data) == 'array'){
          const result = func_methods.getCellDataDyadicArr(data_result.data, 'number');
  ...
  'TIME': function() {
    ...
    try {
      ...
      //计算
      const time = dayjs().set({'hour': hour, 'minute': minute, 'second': second});
  ...
  'EOMONTH': function() {
    ...
    try {
      ...
      //计算
      const date = dayjs(start_date).add(months + 1, 'months').set('date', 1).subtract(1, 'days');
      const mask = genarate(dayjs(date).format('YYYY-MM-DD H:mm:ss'));
  ...
  'EDATE': function() {
    ...
    try {
      ...
      //计算
      const date = dayjs(start_date).add(months, 'months');
      const mask = genarate(dayjs(date).format('YYYY-MM-DD h:mm:ss'));
  ...
  'SECOND': function() {
    ...
    try {
      ...
      const result = dayjs(time_text).seconds();
  ...
  'MINUTE': function() {
    ...
    try {
      ...
      const result = dayjs(time_text).minutes();
  ...
  'HOUR': function() {
    ...
    try {
      ...
      const result = dayjs(time_text).hours();
  ...
  'NETWORKDAYS_INTL': function() {
    ...
    try {
      ...
      //计算
      const days = dayjs(end_date).diff(dayjs(start_date), 'days') + 1;
      let total = days;
  ...
  'ISOWEEKNUM': function() {
    ...
    try {
      //用于日期和时间计算的日期
      const date = func_methods.getCellDate(arguments[0]);
      if(valueIsError(date)){
        return date;
      }
  ...
  'WEEKNUM': function() {
    ...
    try {
      ...
      const inc = dayjs(serial_number).isoWeekday() >= week_start ? 1 : 0;
      const result = dayjs(serial_number).isoWeeks() + inc;
  ...
  'WEEKDAY': function() {
    ...
    try {
      ...
      //计算
      const result = WEEK_TYPES[return_type][dayjs(serial_number).day()];
  ...
  'DAYS': function() {
    ...
    try {
      ...
      //计算
      const result = dayjs(end_date).diff(dayjs(start_date), 'days');
  ...
  'DAYS360': function() {
    ...
    try {
      ...
      //计算
      const sm = dayjs(start_date).month();
      ...
      if (method) {
        ...
        const smd = dayjs().set({ 'year': dayjs(start_date).year(), 'month': sm + 1, 'date': 0 }).date();
        const emd = dayjs().set({ 'year': dayjs(end_date).year(), 'month': em + 1, 'date': 0 }).date();
      ...
      const result = 360 * dayjs(end_date).diff(dayjs(start_date), 'years') + 30 * (em - sm) + (ed - sd);
  ...
  'DATE': function() {
    ...
    try {
      ...
      const date = dayjs().set({ 'year': year, 'month': month - 1, 'date': day });
  ...
  'DATEVALUE': function() {
    ...
    try {
      ...
      const result = genarate(date_text)[2];
  ...
  'DATEDIF': function() {
    ...
    try {
      ...
      const startDate = dayjs(arguments[0]);
      const endDate = dayjs(arguments[1]);
  ...
  'YEARFRAC': function() {
    ...
    try {
      ...
      const sm = dayjs(start_date).month() + 1;
      const sy = dayjs(start_date).year();
      let ed = dayjs(end_date).date();
      const em = dayjs(end_date).month() + 1;
      const ey = dayjs(end_date).year();
  ...
  'RATE': function() {
    ...
    try {
      ...
      while (iter < iterMax && !close) {
        const t1 = Math.pow(rate + 1, nper);
        const t2 = Math.pow(rate + 1, nper - 1);

        const f1 = fv + t1 * pv + pmt * (t1 - 1) * (rate * type + 1) / rate;
        const f2 = nper * t2 * pv - pmt * (t1 - 1) *(rate * type + 1) / Math.pow(rate,2);
        const f3 = nper * pmt * t2 * (rate * type + 1) / rate + pmt * (t1 - 1) * type / rate;
  ...
  'FINDB': function() {
    ...
    try {
      ...
      const strArr = within_text.split('');
