...
const init = function init(matrix){
  const sylvester_matrix = function() {};

  sylvester_matrix.precision = 1e-6;

  sylvester_matrix.create = function(elements) {
    const M = new sylvester_matrix();
    return M.setElements(elements);
  };
  const $M = sylvester_matrix.create;
  ...
  sylvester_matrix.prototype = {
    ...
    minor: function(a, b, c, d) {
      ...
      const rows = this.elements.length, cols = this.elements[0].length;
      while (ni--) { i = c - ni - 1;
        elements[i] = [];
        nj = d;
        while (nj--) { j = d - nj - 1;
    ...
    diagonal: function() {
      ...
      const els = [], n = this.elements.length;
      for (let i = 0; i < n; i++) {
    ...
    augment: function(matrix) {
      ...
      let M = matrix.elements || matrix;
      if (typeof(M[0][0]) === 'undefined') { M = sylvester_matrix.create(M).elements; }
      const T = this.dup(), cols = T.elements[0].length;
    ...
    inverse: function() {
      ...
      const M = this.augment(sylvester_matrix.I(n)).toRightTriangular();
...
export function inverse(els){
  const M = matrix.sylvester_matrix.create(els);
