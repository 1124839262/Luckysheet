...
import {error, isRealNum, valueIsError} from '../global/validate';
...
import {ABCatNum, getObjType} from '../utils/util';
    ...
    if(getObjType(fp) == 'array' && getObjType(tp) == 'array'){
      ...
      if(getObjType(fp[0]) == 'array' && getObjType(tp[0]) == 'array'){
        ...
        if(fp.length == tp.length && fp[0].length == tp[0].length){
          ...
          const rowlen = fp.length;
          const collen = tp[0].length;
          ...
          const rowlen = tp.length;
          const collen = fp[0].length;
    ...
    if(getObjType(fp) == 'array' && getObjType(tp) == 'array'){
      ...
      if(getObjType(fp[0]) == 'array' && getObjType(tp[0]) == 'array'){
        ...
        if(fp.length == tp.length && fp[0].length == tp[0].length){
          ...
          const tp_inverse = inverse(tp);

          const rowlen = fp.length;
          const collen = tp_inverse[0].length;
    ...
    if(getObjType(fp) == 'array' && getObjType(tp) == 'array'){
      ...
      if(getObjType(fp[0]) == 'array' && getObjType(tp[0]) == 'array'){
        ...
        for(let m = 0; m < fp.length; m++){
          ...
          for(let n = 0; n < fp[m].length; n++){
            const value = booleanOperation(fp[m][n], sp, tp[m][n]);
            rowArr.push(value);
          }
        ...
        for(let m = 0; m < fp.length; m++){
          ...
          for(let n = 0; n < fp[m].length; n++){
            const value = booleanOperation(fp[m][n], sp, tp[n]);
            rowArr.push(value);
          }
        ...
        for(let m = 0; m < tp.length; m++){
          ...
          for(let n = 0; n < tp[m].length; n++){
            const value = booleanOperation(fp[n], sp, tp[m][n]);
            rowArr.push(value);
          }
        ...
        for(let n = 0; n < fp.length; n++){
          const value = booleanOperation(fp[n], sp, tp[n]);
          result.push(value);
        }
      }
      ...
      if(getObjType(fp[0]) == 'array'){
        for(let m = 0; m < fp.length; m++){
          ...
          for(let n = 0; n < fp[m].length; n++){
            const value = booleanOperation(fp[m][n], sp, tp);
            rowArr.push(value);
          }
        ...
        for(let n = 0; n < fp.length; n++){
          const value = booleanOperation(fp[n], sp, tp);
          result.push(value);
        }
      }
      ...
      if(getObjType(tp[0]) == 'array'){
        for(let m = 0; m < tp.length; m++){
          ...
          for(let n = 0; n < tp[m].length; n++){
            const value = booleanOperation(fp, sp, tp[m][n]);
            rowArr.push(value);
          }
        ...
        for(let n = 0; n < tp.length; n++){
          const value = booleanOperation(fp, sp, tp[n]);
          result.push(value);
        }
      }
  ...
  if (rangetxt.indexOf(':') == -1) {
    ...
    if (!isNaN(row) && !isNaN(col)) {
      ...
      if (formula.execFunctionGlobalData != null) {
        const ef = formula.execFunctionGlobalData[`${row}_${col}_${sheetIndex}`];
        if(ef!=null){
          ret = ef;
        }
      }
    ...
    const ret = getdatabyselectionD(sheetdata, {
      'row': row,
      'column': col
    });
    ...
    if(formula.execFunctionGlobalData!=null){
      for(let r=row[0];r<=row[1];r++){
        for(let c=col[0];c<=col[1];c++){
          const ef = formula.execFunctionGlobalData[`${r}_${c}_${sheetIndex}`];
          if(ef!=null){
    ...
    //范围的长宽
    const rowl = row[1] - row[0] + 1;
    const coll = col[1] - col[0] + 1;
    const retAll= {
      'sheetName': sheettxt,
